...
from app.services.persona_bundle_writer import write_promotion_items_to_bundle  # noqa: E402
from app.services.persona_promotion_extractor import extract_canonical_promotion_items  # noqa: E402
from app.services.persona_promotion_utils import normalize_selected_promotion_items  # noqa: E402
...
def _json_request(method: str, url: str, payload: dict[str, Any] | None = None, timeout: float = 30.0) -> Any:
    body = None
    headers = control_plane_headers({"Accept": "application/json"})
...
def update_local_sync_state(
    ...
    *,
    ...
    remote_error = "local_persona_sync_failed" if state == "failed" and error_message else None
    payload = {
        "metadata": {
            # Only content-free sync receipts leave the machine. Canonical
            # persona bytes and absolute private-state paths stay local.
            "bundle_storage": (bundle_write or {}).get("bundle_storage") if state == "synced" else None,
