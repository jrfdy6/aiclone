...
- The next source-expansion phase should reuse the parent `Media Intake System` instead of creating a separate transcript stack in the social workspace:
  ...
  7c. the next cleanup on that same path is planner freshness metadata: production now returns the routed media overlay correctly, but `weekly_plan.generated_at` still reflects the older base-plan artifact. The planner timestamp/readout should be reconciled with the augmented long-form route state so the API and UI do not imply a stale planner when the routed media slice is current.
