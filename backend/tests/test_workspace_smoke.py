...
""",
    ...
    def test_workspace_snapshot_route(self) -> None:
        ...
        with patch.object(
            workspace_snapshot_module.workspace_snapshot_service,
            "get_linkedin_os_snapshot",
            return_value=persisted_snapshot,
        ) as snapshot_mock:
...
""",
    ...
    def test_load_doc_entries_discovers_operating_and_workspace_docs(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            source_of_truth = tmp_path / "SOURCE_OF_TRUTH.md"
            source_of_truth.write_text("# Source of Truth\n", encoding="utf-8")
            (sop_dir / "openclaw_retired.md").write_text("# Retired\n", encoding="utf-8")
            ...
            with patch.object(
                ...
                return_value=[(source_of_truth, "Start Here"), (standalone, "Operating Docs")],
            ), patch.object(
                workspace_snapshot_module,
                "ROOT",
                tmp_path,
        ...
        source_path = next((path for path in paths if path.endswith("SOURCE_OF_TRUTH.md")), None)
        ...
        self.assertEqual(entries[0]["path"], source_path)
        self.assertEqual(entries[0].get("authority"), "binding")
        self.assertFalse(any("openclaw" in path.lower() for path in paths))
...
""",
    ...
    def test_brain_ingest_long_form_route_queues_signed_local_action(self) -> None:
        now = datetime.now(timezone.utc)
        card = PMCard(
            id="brain-long-form-card",
            title="Brain: ingest long-form source locally",
            owner="Jean-Claude",
            status="todo",
            source="brain_local_action:long_form_ingest",
        ...
        self.assertEqual(payload.get("action"), "long_form_ingest")
        self.assertEqual(payload.get("card_id"), "brain-long-form-card")
        self.assertEqual(payload.get("job_id"), "brain-long-form-card")
        request = enqueue.call_args.args[1]["request"]
        self.assertEqual(request["url"], "https://www.youtube.com/watch?v=brain123")
        self.assertEqual(request["title"], "Brain-owned ingest test")
