...
class LocalCodexGenerationServiceTest(unittest.TestCase):
    ...
    def test_same_worker_resumes_its_running_job_after_restart(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            previous = os.environ.get("LOCAL_CODEX_JOB_STORE_DIR")
            os.environ["LOCAL_CODEX_JOB_STORE_DIR"] = tmp_dir
            try:
                created = create_codex_job(
                    workspace_slug="linkedin-content-os",
                    requested_by="johnnie_fields",
                    request_payload={"topic": "durable handoffs"},
                    context_packet={"prompt": "write 3 options", "planned_option_briefs": []},
                    idempotency_key="resume-after-crash",
                )
                first_claim = claim_next_codex_job(
                    worker_id="mac-feezie-codex-bridge",
                    workspace_slug="linkedin-content-os",
                )
                resumed_claim = claim_next_codex_job(
                    worker_id="mac-feezie-codex-bridge",
                    workspace_slug="linkedin-content-os",
                )
                competing_claim = claim_next_codex_job(
        ...
        self.assertEqual(first_claim["id"], created["id"])
        self.assertEqual(resumed_claim["id"], created["id"])
        self.assertEqual(resumed_claim["claimed_by"], "mac-feezie-codex-bridge")
