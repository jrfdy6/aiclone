...
class WorkLifecycleServiceTests(unittest.TestCase):
    def test_build_work_lifecycle_report_derives_stages_from_existing_state(self) -> None:
        ...
        standups = [
            SimpleNamespace(
                ...
                payload={
                    ...
                    "meeting_held": False,
                    "observed_at": "2026-04-25T19:00:00Z",
                },
        ...
        ), patch.object(work_lifecycle_service, "list_standups", return_value=standups), patch.object(
            work_lifecycle_service,
            "is_verified_meeting_record",
            side_effect=lambda payload, **_kwargs: payload.get("record_kind") == "standup",
        ):
        ...
        self.assertEqual(((payload.get("standups") or {}).get("counts_by_stage") or {}).get("promoted"), 0)
        self.assertEqual(((payload.get("standups") or {}).get("counts_by_stage") or {}).get("routed"), 1)
        self.assertEqual(
            ((payload.get("workspace_cycle_evaluations") or {}).get("counts_by_stage") or {}).get("interpreted"),
            1,
        )
        self.assertEqual(
            ((payload.get("unverified_standup_records") or {}).get("counts_by_stage") or {}).get("captured"),
            1,
        )
        self.assertEqual(payload["clock"]["authority"], "ai_clone_utc")
