...
class EmailOpsServiceTests(unittest.TestCase):
    def test_persisted_list_and_read_never_sync_seed_or_write(self) -> None:
        ...
        with (
            patch.object(email_ops_service, "load_persisted_threads", return_value=[thread]) as load,
            patch.object(email_ops_service, "gmail_connection_status", side_effect=AssertionError("network status called")) as status,
            patch.object(email_ops_service, "gmail_account_email", side_effect=AssertionError("network account called")) as account,
            patch.object(email_ops_service, "fetch_gmail_threads", side_effect=AssertionError("network fetch called")) as fetch,
            patch.object(email_ops_service, "_fetch_live_gmail_threads", side_effect=AssertionError("live sync called")) as live_sync,
            patch.object(email_ops_service, "_seed_threads", side_effect=AssertionError("seed called")) as seed,
            patch.object(email_ops_service, "_refresh_codex_job_threads", side_effect=AssertionError("refresh called")) as refresh,
            patch.object(email_ops_service, "_write_threads", side_effect=AssertionError("cache write called")) as write,
            patch.object(email_ops_service, "replace_persisted_threads", side_effect=AssertionError("state write called")) as replace,
        ):
            response = list_persisted_threads(workspace_key="agc", needs_human=False, limit=10)
            loaded = get_persisted_thread(thread.id)

        self.assertEqual(load.call_count, 2)
        self.assertEqual(response.total, 1)
        self.assertEqual([item.id for item in response.items], [thread.id])
        self.assertEqual(response.items[0].messages, [])
        self.assertIsNotNone(loaded)
        self.assertEqual(loaded.id, thread.id)
        self.assertEqual(len(loaded.messages), 1)
        for forbidden in (status, account, fetch, live_sync, seed, refresh, write, replace):
            forbidden.assert_not_called()

    def test_persisted_list_returns_empty_without_seeding(self) -> None:
        from app.services import email_ops_service

        with (
            patch.object(email_ops_service, "load_persisted_threads", return_value=[]),
            patch.object(email_ops_service, "_seed_threads", side_effect=AssertionError("seed called")) as seed,
            patch.object(email_ops_service, "_write_threads", side_effect=AssertionError("write called")) as write,
            patch.object(email_ops_service, "gmail_connection_status", side_effect=AssertionError("network called")) as status,
        ):
            response = list_persisted_threads()
            missing = get_persisted_thread("missing")
