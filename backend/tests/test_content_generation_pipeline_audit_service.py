...
def _fake_context(claims: list[str], *, reservoir_count: int, retrieval_support_count: int) -> SimpleNamespace:
    return SimpleNamespace(
        ...
        content_reservoir_chunks=[{"chunk": "x"}] * reservoir_count,
        audit={
            "retrieval": {
                ...
                "content_reservoir_candidates": {
                    "count": reservoir_count,
                    "counts_by_memory_role": {"proof": max(0, reservoir_count - 1), "ambient": 1 if reservoir_count else 0},
                },
...
class ContentGenerationPipelineAuditServiceTests(unittest.TestCase):
    def test_audit_flags_snapshot_drift_and_source_mode_collapse(self) -> None:
        persona_only = _fake_context(["claim-a", "claim-b"], reservoir_count=0, retrieval_support_count=0)
        selected_source = _fake_context(["claim-a", "claim-b"], reservoir_count=4, retrieval_support_count=0)
        recent_signals = _fake_context(["claim-a", "claim-b"], reservoir_count=5, retrieval_support_count=0)
