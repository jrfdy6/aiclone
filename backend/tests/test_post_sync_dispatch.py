...
class PostSyncDispatchTests(unittest.TestCase):
    ...
    def test_build_report_skips_workspace_fallback_when_active_card_exists(self) -> None:
        ...
        standups = [
            {
                ...
                "payload": {
                    ...
                    "standup_sections": {
                        "opportunities_created": [
                            "Latest workspace briefing can be converted into the next bounded SOP."
                        ]
                    },
                },
            }
        ...
        self.assertEqual(dispatch_payload["candidate_titles"], ["Draft the next AGC SOP from the latest workspace briefing"])

    def test_build_report_reuses_active_concrete_workspace_lane_when_latest_standup_carries_it_forward(self) -> None:
        standup_payloads: list[dict] = []
        card_patch_payloads: list[tuple[str, dict]] = []
        standups = [
            {
                "id": "easy-standup-2",
                "workspace_key": "easyoutfitapp",
                "status": "completed",
                "created_at": "2026-04-28T20:51:04Z",
                "payload": {
                    "standup_kind": "workspace_sync",
                    "artifact_deltas": [
                    ...
                    "standup_sections": {
                        "opportunities_created": [
                            "The traffic baseline lane is still the current next move until the host proof lands."
                        ]
                    },
                },
            }
        ...
        cards = [
            {
                ...
                "status": "todo",
                "source": "post-sync-dispatch:older-standup",
                "link_type": "standup",
                "link_id": "older-standup",
                "payload": {
                    "workspace_key": "easyoutfitapp",
                    "created_from_standup_id": "older-standup",
                    "completion_contract": {"source": "post_sync_dispatch"},
                    "execution": {
                        "state": "queued",
                ...
                "created_at": "2026-04-26T09:10:23Z",
                "updated_at": "2026-04-26T09:10:23Z",
            }
        ]

        def fake_fetch(url: str, *, method: str = "GET", payload: dict | None = None):
            if method == "PATCH" and "/api/pm/cards/" in url:
                card_patch_payloads.append((url, payload or {}))
                return {
                    "id": "easy-active-card",
                    "title": "Capture the first Easy Outfit App traffic baseline proof",
                    "status": "todo",
        ...
        with mock.patch.object(self.dispatch, "_fetch_json", side_effect=fake_fetch), mock.patch.object(
            self.dispatch, "_now", return_value=self.dispatch.datetime.fromisoformat("2026-04-28T21:00:00+00:00")
        ):
            report = self.dispatch.build_report("https://example.test", lookback_days=2, limit=80, sync_live=True)

        self.assertEqual(report["created_count"], 0)
        self.assertEqual(report["standups"][0]["linked_card_ids"], ["easy-active-card"])
        self.assertEqual(report["standups"][0]["reused_card_ids"], ["easy-active-card"])
        self.assertEqual(len(card_patch_payloads), 1)
        carry_forward_payload = card_patch_payloads[0][1]["payload"]
        self.assertEqual(carry_forward_payload["latest_carry_forward_standup_id"], "easy-standup-2")
        self.assertIn("easy-standup-2", carry_forward_payload["carry_forward_standup_ids"])
        dispatch_payload = standup_payloads[0]["payload"]["post_sync_dispatch"]
        self.assertEqual(dispatch_payload["status"], "ok")
        self.assertEqual(dispatch_payload["reused_card_ids"], ["easy-active-card"])

    def test_build_report_refreshes_generic_workspace_placeholder_when_concrete_focus_arrives(self) -> None:
        standup_payloads: list[dict] = []
        card_patch_payloads: list[tuple[str, dict]] = []
        standups = [
            {
        ...
        cards = [
            {
                "id": "fusion-placeholder-card",
                "title": "Define next concrete opportunity for Fusion Os",
                "status": "review",
                "source": "post-sync-dispatch:older-standup",
                "link_type": "standup",
                "link_id": "older-standup",
                "payload": {
                    "workspace_key": "fusion-os",
                    "dispatch_source_title": "Define next concrete opportunity for Fusion OS",
                    "completion_contract": {"source": "post_sync_dispatch"},
                    "execution": {
                        "state": "review",
                        "manager_agent": "Jean-Claude",
                ...
                "updated_at": "2026-04-25T18:15:00Z",
            }
        ]
        ...
        self.assertEqual(report["created_count"], 0)
        self.assertEqual(report["standups"][0]["linked_card_ids"], ["fusion-placeholder-card"])
        self.assertEqual(report["standups"][0]["refreshed_card_ids"], ["fusion-placeholder-card"])
        self.assertEqual(len(card_patch_payloads), 1)
        card_payload = card_patch_payloads[0][1]
        self.assertEqual(card_payload["title"], "Ship a Leadership POV post in the next cycle so the weekly mix stays balanced")
        self.assertEqual(card_payload["status"], "todo")
        self.assertEqual(card_payload["link_id"], "fusion-standup-2")
        dispatch_payload = standup_payloads[0]["payload"]["post_sync_dispatch"]
        self.assertEqual(dispatch_payload["status"], "ok")
        self.assertEqual(dispatch_payload["refreshed_card_ids"], ["fusion-placeholder-card"])
