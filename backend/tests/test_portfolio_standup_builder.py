...
class PortfolioStandupBuilderTests(unittest.TestCase):
    ...
    def test_targets_include_work_life_tools_workspace_sync(self) -> None:
        target = next(
            item
            for item in self.builder.TARGETS
            if item.workspace_key == "work-life-tools"
        )

        self.assertEqual(target.standup_kind, "workspace_sync")
        self.assertEqual(target.max_age_hours, 72)

    def test_prep_path_accepts_sanitized_repo_relative_path(self) -> None:
        relative_path = Path("memory/standup-prep/latest.json")
        entry = {
            "payload": {
                "prep_json_path": relative_path.as_posix(),
            }
        }

        resolved = self.builder._prep_path_from_standup(entry)

        self.assertEqual(resolved, relative_path)
        self.assertFalse(resolved.is_absolute())
        self.assertEqual(
            (ROOT / resolved).resolve(),
            ROOT / "memory" / "standup-prep" / "latest.json",
        )
