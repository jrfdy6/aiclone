    ...
    with _patch_collectors(
        {
            ...
            "_collect_workspace_review_decisions": RuntimeError(
                "/" + "Users/example/private/control_plane.env"
            ),
        }
    ):
...
def test_standup_collector_uses_one_read_and_latest_actionable_item_per_workspace() -> None:
    ...
    latest = SimpleNamespace(
        ...
        payload={
            "summary": "A supplier decision is waiting.",
            "source_paths": ["memory/latest.md"],
            "record_kind": "standup",
            "meeting_held": True,
            "evaluation_only": False,
...
def test_standup_collector_ignores_newer_cycle_plan_and_uses_verified_meeting() -> None:
    meeting = SimpleNamespace(
        id="verified-meeting",
        workspace_key="agc",
        blockers=["Verified blocker"],
        needs=[],
        source="standup_agent_meeting",
        payload={
            "summary": "Verified meeting state.",
            "record_kind": "standup",
            "meeting_held": True,
            "evaluation_only": False,
    ...
    cycle_plan = SimpleNamespace(
        id="newer-cycle-plan",
        workspace_key="agc",
        blockers=["Suppressed planning blocker"],
        needs=["Owner decision that was not ratified"],
        source="standup_prep",
        payload={
    ...
    with patch.object(
        service.standup_service,
        "list_standups",
        return_value=[cycle_plan, meeting],
    ), patch.object(
        service,
        "is_verified_meeting_record",
        return_value=True,
    ...
    assert [item.id for item in result.items] == ["standup:verified-meeting"]
    assert "Verified blocker" in result.items[0].why_it_matters
    assert "Suppressed planning blocker" not in str(result.items)


def test_standup_collector_ranks_by_ai_clone_observation_not_persistence_time() -> None:
    def meeting(meeting_id: str, observed_at: str, created_at: datetime, blocker: str) -> SimpleNamespace:
    ...
    late_old = meeting(
        ...
        NOW,
        "Old replayed blocker",
    )
    semantic_new = meeting(
        "semantic-new",
    ...
    with patch.object(
        service.standup_service,
        "list_standups",
        return_value=[late_old, semantic_new],
    ), patch.object(
        service,
        "is_verified_meeting_record",
        return_value=True,
    ...
    assert [item.id for item in result.items] == ["standup:semantic-new"]
    assert result.items[0].updated_at == datetime(2026, 8, 26, 12, 0, tzinfo=timezone.utc)
...
def test_owner_revision_requires_notes_and_delegates_to_existing_contract() -> None:
    ...
    with patch.object(service, "_find_decision_and_action", return_value=(decision, decision.actions[0])):
        with pytest.raises(service.ExecutiveDecisionActionError, match="disabled by default"):
            service.execute_executive_decision_action(
                decision.id,
                "revise",
                ExecutiveDecisionActionRequest(confirmed=True, notes="Must not write."),
            )
