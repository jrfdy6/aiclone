...
class ContentGenerationContextSourceModeTests(unittest.TestCase):
    ...
    def test_selected_source_mode_blends_content_safe_operator_lessons_with_reservoir(self) -> None:
        core_chunk = {
        ...
        self.assertEqual(context.content_reservoir_chunks, [reservoir_chunk])
        self.assertEqual(
            context.content_signal_source,
            "content_reservoir+content_safe_operator_lessons",
        )
        self.assertEqual(len(context.content_signal_chunks), 2)
        self.assertEqual(
            [
                (item.get("metadata") or {}).get("source_kind")
                for item in context.content_signal_chunks
            ],
            ["content_reservoir", "content_safe_operator_lessons"],
        )
        self.assertEqual(
            (context.content_signal_chunks[1].get("metadata") or {}).get("source_kind"),
        ...
        self.assertEqual(
            context.audit["retrieval"]["content_signal_source"],
            "content_reservoir+content_safe_operator_lessons",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            context.audit["retrieval"]["content_reservoir_candidates"]["count"],
            1,
        )
    ...
    def test_recent_signals_mode_blends_recent_reservoir_and_safe_operator_lessons(self) -> None:
        core_chunk = {
            "chunk": "Workflow clarity matters because operator trust breaks when review disappears.",
            "metadata": {
                "bundle_path": "identity/claims.md",
        ...
        reservoir_chunk = {
            "chunk": "A recent source showed that visible review keeps agent orchestration trustworthy.",
            "metadata": {
                "source_kind": "content_reservoir",
                "source_lane": "content_reservoir",
                "memory_role": "proof",
        ...
        safe_chunk = {
            "chunk": "A public-safe operator lesson connected explicit review to better AI adoption.",
            "metadata": {
                "source_kind": "content_safe_operator_lessons",
                "source_lane": "content_safe_operator_lessons",
                "memory_role": "proof",
        ...
        with (
            ...
            patch("app.services.content_generation_context_service.retrieve_bundle_example_chunks", return_value=[]),
            patch(
                "app.services.content_generation_context_service.retrieve_content_safe_operator_lesson_chunks",
                return_value=[safe_chunk],
            ) as safe_mock,
            patch(
                "app.services.content_generation_context_service.retrieve_content_reservoir_chunks",
                return_value=[reservoir_chunk],
            ) as reservoir_mock,
        ):
            context = build_content_generation_context(
                user_id="johnnie_fields",
        ...
        self.assertEqual(context.content_reservoir_chunks, [reservoir_chunk])
        self.assertEqual(context.content_signal_chunks, [reservoir_chunk, safe_chunk])
        self.assertEqual(
            context.content_signal_source,
            "content_reservoir+content_safe_operator_lessons",
        )
