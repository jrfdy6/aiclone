...
class BrainControlPlaneServiceTests(unittest.TestCase):
    def test_local_runner_previews_overlay_without_replacing_full_pipeline_snapshot_types(self) -> None:
        canonical = {
            "source_assets": {"counts": {"total": 1}, "items": [{"asset_id": "canonical"}]},
            "content_reservoir": {"counts": {"total": 1}},
            "long_form_routes": {"route_counts": {"source_only": 1}},
        }
        previews = {
            "brain_source_assets_preview": {
                "generated_at": "2026-07-20T12:00:00Z",
                "counts": {"total": 20},
                "items": [{"asset_id": "local-preview", "title": "Newest local asset"}],
            },
            "brain_content_reservoir_summary": {"counts": {"total": 45}},
            "brain_long_form_routes_summary": {"route_counts": {"belief_evidence": 7}},
        }
        with patch.object(brain_control_plane_service, "list_snapshot_payloads", return_value=previews):
            overlaid = brain_control_plane_service._overlay_local_runner_previews(canonical)

        self.assertEqual(overlaid["source_assets"]["counts"]["total"], 20)
        self.assertEqual(overlaid["source_assets"]["items"][0]["asset_id"], "local-preview")
        self.assertEqual(overlaid["content_reservoir"]["counts"]["total"], 45)
        self.assertEqual(overlaid["long_form_routes"]["route_counts"]["belief_evidence"], 7)

    def test_empty_persisted_workspace_snapshot_never_falls_back_to_runtime_refresh(self) -> None:
        with patch.object(brain_control_plane_service, "list_automations", return_value=[]), patch.object(
            ...
            "fetch_metrics",
            return_value={"captures": {}, "vectors": {}, "recent_captures": []},
        ), patch.object(
        ...
        self.assertEqual(payload.get("workspace_snapshot"), {"workspace_files": [], "doc_entries": []})

    def test_build_brain_control_plane_uses_deployed_workspace_snapshot(self) -> None:
        workspace_snapshot = {
    ...
    def test_build_brain_control_plane_returns_signal_and_source_summary_without_heavy_fanout(self) -> None:
        workspace_snapshot = {
            "doc_entries": [],
            "workspace_files": [],
        ...
        with patch.object(brain_control_plane_service, "list_automations", return_value=[]), patch.object(
            ...
            "app.services.open_brain_service.fetch_health",
        ) as health_read, patch(
            "app.services.portfolio_workspace_snapshot_service.build_portfolio_workspace_snapshot",
        ) as portfolio_read:
            payload = brain_control_plane_service.build_brain_control_plane()

        health_read.assert_not_called()
        portfolio_read.assert_not_called()
        self.assertNotIn("telemetry_health", payload)
        self.assertNotIn("portfolio_snapshot", payload)
        self.assertNotIn("portfolio_workspace_count", payload.get("summary") or {})
        self.assertNotIn("portfolio_attention_count", payload.get("summary") or {})
    ...
    def test_source_intelligence_index_loader_uses_fallback_candidates(self) -> None:
        ...
        self.assertEqual((payload or {}).get("source_ref"), "knowledge/source-intelligence/index.json.txt")
        self.assertEqual((payload or {}).get("source_mode"), "deployed_snapshot")
        self.assertNotIn(str(Path(temp_dir)), str(payload))
