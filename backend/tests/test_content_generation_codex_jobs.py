...
class ContentGenerationCodexJobsRouteTest(unittest.TestCase):
    ...
    def test_source_card_is_persisted_public_safe_and_part_of_cache_identity(self) -> None:
        source_card = {
            "item_key": "brief-item-1",
            "brief_id": "brief-2026-07-20",
            "origin_type": "daily_brief_item",
            "origin_id": "brief-item-1",
            "owner_reaction": "This explains why judgment matters more than tool volume.",
            "title": "AI literacy is judgment literacy",
            "summary": "The best operators build judgment before they add more automation.",
            "hook": "More tools do not create better judgment.",
            "source_url": "https://example.com/ai-judgment?private_tracking=1",
            "source_path": "/Users/neo/private/briefs/ai-judgment.md",
            "priority_lane": "ai",
            "source_kind": "daily_brief",
            "route_reason": "This sharpens the public argument for operator judgment.",
            "target_file": "knowledge/persona/feeze/identity/VOICE_PATTERNS.md",
            "section": "What changed overnight",
            "provenance": {
                "author": "Example Author",
                "platform": "web",
                "published_at": "2026-07-19",
                "source_path": "/Users/neo/another/private/path.md",
            },
        }
        payload = {
            "user_id": "johnnie_fields",
            "topic": "judgment literacy",
            "context": "Use the source card and keep the claim practical.",
            "content_type": "linkedin_post",
            "category": "value",
            "tone": "expert_direct",
            "audience": "tech_ai",
        ...
        with patch("app.services.local_codex_context_cache_service.get_snapshot_payload", return_value={}), patch.object(
            ...
            first_complete = self.client.post(
                f"/api/content-generation/codex-jobs/{first_create.json()['job_id']}/complete",
                headers={"X-Local-Codex-Token": "test-token"},
                json={
                    "worker_id": "local-bridge",
                    "model": "local-template-v1",
            ...
            second_payload = {
                **payload,
                "source_card": {**source_card, "item_key": "brief-item-2", "origin_id": "brief-item-2"},
            }
            second_create = self.client.post("/api/content-generation/codex-jobs", json=second_payload)
            self.assertEqual(second_create.status_code, 200)
            second_claim = self.client.post(
                "/api/content-generation/codex-jobs/claim-next",
                headers={"X-Local-Codex-Token": "test-token"},
                json={"worker_id": "local-bridge-2", "workspace_slug": "linkedin-content-os"},
            )
        ...
        self.assertEqual(first_claim_payload["request_payload"]["source_card"], source_card)
        self.assertEqual(first_claim_payload["context_packet"]["source_card"], source_card)
        self.assertEqual(second_claim_payload["request_payload"]["source_card"]["item_key"], "brief-item-2")
        self.assertNotEqual(
            first_claim_payload["context_packet"]["cache_key"],
            second_claim_payload["context_packet"]["cache_key"],
        )
        prompt = first_claim_payload["context_packet"]["prompt"]
        self.assertIn("AI literacy is judgment literacy", prompt)
        self.assertIn("This explains why judgment matters more than tool volume.", prompt)
        self.assertIn("https://example.com/ai-judgment", prompt)
        self.assertNotIn("private_tracking", prompt)
        self.assertNotIn(source_card["source_path"], prompt)
        self.assertNotIn(source_card["target_file"], prompt)
        self.assertNotIn("/Users/neo/another/private/path.md", prompt)
        built_context = builder.call_args_list[0].kwargs["context"]
        self.assertIn("Selected source card", built_context)
        self.assertNotIn(source_card["source_path"], built_context)
        self.assertNotIn(source_card["target_file"], built_context)

    def test_send_to_review_validates_option_and_uses_completed_server_result(self) -> None:
        payload = {
            "user_id": "johnnie_fields",
            "topic": "workflow clarity",
            "context": "Use the operator angle.",
        ...
        with patch("app.services.local_codex_context_cache_service.get_snapshot_payload", return_value={}), patch.object(
            ...
            create_response = self.client.post("/api/content-generation/codex-jobs", json=payload)
        job_id = create_response.json()["job_id"]
        ...
        with patch.object(
            ...
            return_value={
                "card_id": "pm-card-1",
                "queue_id": owner_review_item["queue_id"],
                "duplicate": False,
                "item": owner_review_item,
        ...
        self.assertTrue(review_response.json()["owner_review_required"])
        self.assertEqual(review_response.json()["approval_status"], "owner_review_required")
        self.assertEqual(review_response.json()["publish_posture"], "owner_review_required")
        create_review.assert_called_once()
        self.assertEqual(create_review.call_args.kwargs["option_text"], _fake_result_payload()["options"][1])
        self.assertEqual(create_review.call_args.kwargs["option_index"], 1)
