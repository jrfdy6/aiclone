...
import build_standup_prep as standup_prep  # type: ignore


class WorkspaceRegistryLegacyTests(unittest.TestCase):
    def test_standup_prep_resolves_work_life_tools_from_canonical_registry(self) -> None:
        registry = standup_prep._read_registry()

        self.assertIn("work-life-tools", registry)
        self.assertEqual(
            standup_prep._workspace_root("work-life-tools", registry),
            REPO_ROOT / "workspaces" / "work-life-tools",
        )
    ...
    def test_written_legacy_registry_can_be_loaded_as_map(self) -> None:
        ...
        self.assertEqual(
            registry["work-life-tools"]["filesystem_path"],
            str(REPO_ROOT / "workspaces" / "work-life-tools"),
        )
