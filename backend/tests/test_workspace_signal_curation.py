...
class WorkspaceSignalCurationTests(unittest.TestCase):
    ...
    def test_api_runners_load_raw_execution_source_card(self) -> None:
        expected = {"id": "card-raw-1", "payload": {"execution": {"state": "queued"}}}

        for runner in (self.jean, self.workspace_agent):
            with self.subTest(runner=runner.__name__):
                with mock.patch.object(runner, "_fetch_json", return_value=expected) as fetch:
                    card = runner._load_card(
                        {"mode": "api"},
    ...
    def test_workspace_idle_pm_update_turns_briefing_into_bounded_sop_when_no_focus_line_exists(self) -> None:
        updates = self.build_standup._build_pm_updates(
            [],
            "agc",
            "jean-claude",
        ...
        self.assertEqual(len(updates), 1)
        self.assertEqual(updates[0]["title"], "Draft the next AGC SOP from the latest workspace briefing")
        self.assertIn("bounded artifact", updates[0]["payload"]["acceptance_criteria"][0])

    def test_workspace_idle_pm_update_prefers_staged_sop_followthrough(self) -> None:
        updates = self.build_standup._build_pm_updates(
            [],
            "fusion-os",
            "jean-claude",
            {"available": True, "cards": []},
            {
                "latest_sop_path": "/tmp/fusion_sop.json",
                "latest_briefing_path": "/tmp/fusion_briefing.md",
                "execution_log_path": "/tmp/fusion_execution_log.md",
                "audience_feedback": {},
            },
        ...
        self.assertEqual(len(updates), 1)
        self.assertEqual(updates[0]["title"], "Refine the staged Fusion OS SOP into one runnable execution packet")
    ...
    def test_workspace_pm_updates_reuse_existing_standup_lane_before_parallel_scope(self) -> None:
        updates = self.build_standup._build_pm_updates(
            [],
            "fusion-os",
            "jean-claude",
            {
                ...
                "cards": [
                    {
                        "id": "pm-card-42",
                        "title": "Draft the next Fusion OS SOP from the latest workspace briefing",
                        "status": "review",
                        "workspace_key": "fusion-os",
                        "link_type": "standup",
                        "payload": {"created_from_standup_id": "standup-old"},
                    }
                ],
            },
            {
        ...
        self.assertEqual(len(updates), 1)
        self.assertEqual(updates[0]["title"], "Draft the next Fusion OS SOP from the latest workspace briefing")
        self.assertEqual(updates[0]["payload"]["carry_forward_action"], "reuse_existing_lane")
        self.assertEqual(updates[0]["payload"]["pm_card_id"], "pm-card-42")
        self.assertTrue(updates[0]["payload"]["carry_forward_required"])

    def test_workspace_pm_updates_refresh_placeholder_lane_in_place(self) -> None:
        updates = self.build_standup._build_pm_updates(
            [],
            "agc",
            "jean-claude",
            {
                ...
                "cards": [
                    {
                        "id": "pm-card-77",
                        "title": "Define next concrete opportunity for AGC",
                        "status": "ready",
                        "workspace_key": "agc",
                        "link_type": "standup",
                        "payload": {"created_from_standup_id": "standup-old"},
        ...
        self.assertEqual(len(updates), 1)
        self.assertEqual(updates[0]["title"], "Draft the next AGC SOP from the latest workspace briefing")
        self.assertEqual(updates[0]["payload"]["carry_forward_action"], "refresh_existing_lane")
        self.assertEqual(updates[0]["payload"]["pm_card_id"], "pm-card-77")
    ...
    def test_standup_ready_chronicle_drops_chat_derived_upload_digest_residue(self) -> None:
        cleaned = self.build_standup._standup_ready_chronicle_entries(
            [
                {
                    "workspace_key": "linkedin-os",
                    "source": "codex-history",
                    "decisions": [
                        "Uploaded image Uploaded image Uploaded image Uploaded image Uploaded image Uploaded image Uploaded image Uploaded image Uploaded image Uploaded image I want you to digest all the information on all of these photos including 200 more so t...",
                        "We need to convert it into a clean knowledge base your system can actually use.",
                        "What I recommend you upload The best option is: Original Google Sheet export as .xlsx or .csv Original Google Docs / PDFs instead of photos If those are unavailable, upload the photos in batches of 20-40 images at a time Photos are workable.",
                    ],
                    "blockers": [
                        "Recurring themes include: Remaining flat on projections Program overview webinar timing Team transitioning focus to January New admissions counselors starting Email template updates Financial aid webinar planning Improving handoff between teams.",
                        "You were helping ACs navigate conversations and manager-facing issues.",
                    ],
                    "follow_ups": [
                        "We need to convert it into a clean knowledge base your system can actually use.",
                        "Examples: Webinar initiative ownership Decisions out of SLA Technical issues from campus causing deferrals.",
                    ],
                    "project_updates": [
                        "From the photos you shared, this looks like a Student Engagement / Admissions Success Playbook with two major types of information.",
                    ],
                }
            ],
    ...
    def test_standup_ready_chronicle_drops_standup_review_residue(self) -> None:
        cleaned = self.build_standup._standup_ready_chronicle_entries(
            [
                {
                    "workspace_key": "linkedin-os",
                    "source": "codex-history",
                    "project_updates": [
                        "now I feel like i would like to have all of my stand ups to be more productive.look at the executive stand up: FEEZIE OS Standup FEEZIE OS · Apr 27, 4:49 AM completed feezie-os Generated from standup prep",
                        "Attendee Snapshot 3 observed attendees 0 inferred from room defaults 3 shown in the meeting reader Round 1: Jean-Claude",
                        "Agenda starts with Check whether the next public move strengthens Feeze's brand, career narrative, and trust before optimizing for content output.",
                    ],
                    "identity_signals": [
                        "Signal - Recent Codex discussion for `linkedin-os`: Uploaded image Uploaded image Uploaded image"
                    ],
                    "outcomes": [
                        "Conversation Evidence Outcomes Raw should not become the next standup signal."
                    ],
                }
            ],
            workspace_key="feezie-os",
    ...
    def test_build_bundle_keeps_executive_context_separate_from_workspace_context(self) -> None:
        with tempfile.TemporaryDirectory(prefix="ai-clone-fusion-os-test-") as temp_dir:
            project_root = Path(temp_dir)
            workspace_root = project_root / "workspaces" / "fusion-os-test"
            ...
            with (
                mock.patch.object(self.jean, "WORKSPACE_ROOT", project_root),
                mock.patch.object(self.jean, "build_workspace_memory_contract", side_effect=fake_memory_contract),
            ):
    ...
    def test_workspace_execution_contract_requires_local_citations(self) -> None:
        ...
        self.assertTrue(
            any("/tmp/manager.md" in item for item in contract["artifacts_expected"])
        )
        self.assertTrue(
            any("/tmp/workspace.md" in item for item in contract["artifacts_expected"])
        )

    def test_workspace_agent_select_entry_prefers_recent_standup_backed_queue(self) -> None:
        selected = self.workspace_agent._select_entry(
            [
                {
                    **RUNNABLE_GATE_FIELDS,
                    "card_id": "older-residue",
                    "workspace_key": "fusion-os",
                    ...
                    "queued_at": "2026-04-27T15:00:00Z",
                },
                {
                    **RUNNABLE_GATE_FIELDS,
                    "card_id": "fresh-standup",
                    "workspace_key": "fusion-os",
                    "execution_mode": "delegated",
                    "execution_state": "queued",
                    "target_agent": "Fusion Systems Operator",
                    "source": "post-sync-dispatch:fusion-standup-2",
                    "link_type": "standup",
                    "queued_at": "2026-04-27T19:47:12Z",
                },
            ],
            workspace_key="fusion-os",
            target_agent="Fusion Systems Operator",
        ...
        self.assertEqual(selected["card_id"], "fresh-standup")

    def test_workspace_agent_select_entry_skips_running_lane_with_existing_packet_during_automatic_poll(self) -> None:
        selected = self.workspace_agent._select_entry(
            [
                {
                    **RUNNABLE_GATE_FIELDS,
                    "card_id": "already-running",
                    "workspace_key": "fusion-os",
                    "execution_mode": "delegated",
    ...
    def test_workspace_agent_select_entry_allows_manual_rerun_of_running_lane_with_existing_packet(self) -> None:
        selected = self.workspace_agent._select_entry(
            [
                {
                    **RUNNABLE_GATE_FIELDS,
        ...
        self.assertIsNotNone(selected)
        assert selected is not None
        self.assertEqual(selected["card_id"], "already-running")

    def test_jean_claude_select_next_entry_prefers_recent_standup_backed_queue(self) -> None:
        selected = self.jean._select_next_entry(
            [
                {
                    **RUNNABLE_GATE_FIELDS,
                    "card_id": "older-residue",
    ...
    def test_morning_brief_stays_bounded_with_long_brain_and_ops_signals(self) -> None:
        long_signal = "material operator signal " * 40
        memory_payload = {
            "memory_context": {},
            "memory_diagnostics": {"memory/persistent_state.md": {}},
            "source_paths": [],
        }
        runtime_context = {
            "pm_context": {"cards": []},
        ...
        brain_context = {"source_paths": []}

        with (
            mock.patch.object(self.morning_brief, "_material_entries", return_value=[]),
            mock.patch.object(self.morning_brief, "build_memory_tail_context", return_value=memory_payload),
            mock.patch.object(self.morning_brief, "load_runtime_context", return_value=runtime_context),
            mock.patch.object(self.morning_brief, "build_heartbeat_report", return_value={}),
            mock.patch.object(self.morning_brief, "render_heartbeat_summary", return_value="heartbeat"),
            mock.patch.object(self.morning_brief, "build_brain_automation_context", return_value=brain_context),
            mock.patch.object(self.morning_brief, "_what_changed_lines", return_value=[long_signal, long_signal]),
            mock.patch.object(self.morning_brief, "_why_it_matters_lines", return_value=[long_signal, long_signal]),
            mock.patch.object(self.morning_brief, "_action_lines", return_value=["Owner: Neo", f"Next: {long_signal}"]),
            mock.patch.object(self.morning_brief, "_routing_lines", return_value=["Workspace: shared_ops", f"Route: {long_signal}"]),
            mock.patch.object(self.morning_brief, "portfolio_attention_lines", return_value=[long_signal, long_signal]),
            mock.patch.object(self.morning_brief, "brain_signal_lines", return_value=[long_signal, long_signal]),
            mock.patch.object(self.morning_brief, "_source_lines", return_value=[f"Artifact: `{long_signal}`"]),
            mock.patch.object(self.morning_brief, "source_intelligence_lines", return_value=[long_signal]),
            mock.patch.object(self.morning_brief, "_alert_lines", return_value=[long_signal, long_signal]),
        ):
            payload = self.morning_brief.build_brief_payload()

        markdown = payload["markdown"]
        report = self.digest_quality.validate_digest("morning_daily_brief", markdown)
        self.assertLessEqual(len(markdown), 1800)
        self.assertTrue(report.ok, report.issues)
        self.assertEqual(markdown.split("Brain Context:\n", 1)[1].split("Source:\n", 1)[0].count("\n- "), 1)
    ...
    def test_load_automation_context_uses_codex_registry_and_private_ledger(self) -> None:
        class _Report:
            def model_dump(self, mode: str = "json") -> dict[str, object]:
                return {"mismatch_count": 0, "action_required_count": 0, "run_count": 1}

        run = SimpleNamespace(id="run-1")
        automation = SimpleNamespace(name="Fusion Feedback Refresh")
        build_report = mock.Mock(return_value=_Report())
        list_automations = mock.Mock(return_value=[automation])
        context = self.build_standup._load_automation_context(
            {
                "build_mismatch_report": build_report,
                "list_automation_runs": lambda: [run],
        ...
        self.assertTrue(context["available"])
        self.assertFalse(context["fallback_active"])
        self.assertEqual(context["source"], "codex_registry+codex_run_ledger")
        self.assertEqual(context["primary_source"], "codex_registry+codex_run_ledger")
        self.assertEqual(context["job_names"], ["Fusion Feedback Refresh"])
        list_automations.assert_called_once_with(runs=[run])
        build_report.assert_called_once_with(automations=[automation], runs=[run])

    def test_load_automation_context_falls_back_to_private_codex_ledger(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ledger = Path(temp_dir) / "all.jsonl"
            ledger.write_text(
                json.dumps(
                    {
            ...
            with mock.patch.object(self.build_standup, "CODEX_RUN_LEDGER_PATH", ledger):
                context = self.build_standup._load_automation_context({"automation_error": "registry import failed"})

        self.assertTrue(context["available"])
        self.assertTrue(context["fallback_active"])
        self.assertEqual(context["source"], "codex_run_ledger")
        self.assertEqual(context["fallback_reason"], "codex_registry_unavailable")
        self.assertEqual(context["job_names"], ["Fusion Feedback Refresh"])

    def test_standup_sanitizes_retired_runtime_and_chat_channel_history(self) -> None:
        memory_line = self.build_standup._normalize_memory_content(
            "Codex and OpenClaw need to stay aligned through Discord heartbeat summaries."
        )
        pm_title = self.build_standup._normalize_pm_title("Align OpenClaw and Codex workflow sync")
        health_title = self.build_standup._normalize_pm_title("Heartbeat and Discord summaries are weak")

        for value in (memory_line, pm_title, health_title):
            lowered = value.lower()
            self.assertNotIn("openclaw", lowered)
            self.assertNotIn("discord", lowered)
        self.assertEqual(pm_title, "Complete the Codex-native workflow cutover")
        self.assertEqual(health_title, "Make automation health summaries diagnostically useful")

    def test_durable_memory_context_uses_codex_memory_index(self) -> None:
        indexed = [
            {
                "path": "memory/fusion_notes.md",
                "title": "Fusion notes",
                "score": 0.92,
                "excerpt": "A durable Fusion decision.",
            }
        ]
        with mock.patch.object(self.durable, "search_index", return_value=indexed) as search_mock:
            results, error = self.durable._run_codex_index_search("fusion", 2)

        search_mock.assert_called_once_with("fusion", limit=2)
        self.assertIsNone(error)
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0]["path"], "memory/fusion_notes.md")
