...
class ThinTriggerScriptTests(unittest.TestCase):
    ...
    def test_pm_card_request_uses_workspace_defaults_for_delegated_workspace(self) -> None:
        args = types.SimpleNamespace(
            ...
            repo_path="/Users/neo/Documents/Codex/AI-Clone",
            branch="main",
            sop_path="",
            briefing_path="",
        )
        ...
        self.assertEqual(payload["payload"]["completion_contract"]["source"], "codex_native_remote_queue")
        self.assertTrue(payload["payload"]["completion_contract"]["autostart"])
        self.assertEqual(payload["payload"]["acceptance_criteria"], ["Card appears in delegated queue."])
        self.assertEqual(payload["payload"]["artifacts_expected"], ["dispatch note"])
        ...
        self.assertTrue(str(payload["payload"]["trigger_key"]).startswith("codex:"))
