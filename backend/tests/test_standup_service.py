...
class StandupServiceTests(unittest.TestCase):
    ...
    def test_promote_standup_refreshes_existing_carry_forward_lane_in_place(self) -> None:
        now = datetime.now(timezone.utc)
        standup_entry = StandupEntry(
            id="standup-234",
            owner="Jean-Claude",
        ...
        existing = PMCard(
            ...
            owner="Jean-Claude",
            status="review",
            source="post-sync-dispatch:old-standup",
            link_type="standup",
            link_id="standup-old",
        ...
        updated_cards: list[object] = []

        def _fake_create_standup(_: StandupCreate):
            return standup_entry

        def _fake_update_card(card_id: str, payload):
            self.assertEqual(card_id, "pm-card-existing")
        ...
        with (
            patch.object(standup_service, "create_standup", side_effect=_fake_create_standup),
            patch.object(standup_service.pm_card_service, "get_card", return_value=existing),
            patch.object(standup_service.pm_card_service, "find_card_by_signature", return_value=None),
            patch.object(standup_service.pm_card_service, "find_active_card_by_title", return_value=None),
            patch.object(standup_service.pm_card_service, "update_card", side_effect=_fake_update_card),
            patch.object(standup_service.pm_card_service, "create_card") as create_card_mock,
        ):
            result = standup_service.promote_standup(
                StandupPromotionRequest(
                    owner="Jean-Claude",
                    ...
                    summary="Carry forward the existing Fusion lane.",
                    pm_updates=[
                        {
                            "workspace_key": "fusion-os",
                            "scope": "workspace",
                            "owner_agent": "Jean-Claude",
                            "title": "Refine the staged Fusion OS SOP into one runnable execution packet",
                            "status": "todo",
                            "reason": "Carry-forward pressure refreshed the placeholder lane.",
                            "payload": {
                                "pm_card_id": "pm-card-existing",
                                "created_from_prep_id": "prep-456",
                                ...
                                "carry_forward_summary": "Carry the Fusion lane first.",
                            },
                        }
                    ],
                )
        ...
        self.assertEqual(len(result.created_cards), 0)
        self.assertEqual(len(result.existing_cards), 1)
        self.assertEqual(result.existing_cards[0].title, "Refine the staged Fusion OS SOP into one runnable execution packet")
        self.assertEqual(result.existing_cards[0].status, "todo")
        self.assertIn("standup-234", result.existing_cards[0].payload.get("carry_forward_standup_ids") or [])
        self.assertEqual(result.existing_cards[0].payload.get("latest_carry_forward_standup_id"), "standup-234")
