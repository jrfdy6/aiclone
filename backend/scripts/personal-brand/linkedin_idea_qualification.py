...
IDEA_QUALIFICATION_VERSION = "2026-08-15-source-freshness-v2"
...
def _source_freshness(item: dict[str, Any], *, now: datetime | None = None) -> dict[str, Any]:
    """Return the same bounded freshness semantics used by FEEZIE generation.

    Social-feed items are trend inputs unless upstream explicitly classifies
    them as evergreen.  A capture timestamp is useful provenance, but it must
    not make an old published source look new.
    """

    now_utc = (now or datetime.now(timezone.utc)).astimezone(timezone.utc)
    published_at = _parse_source_datetime(item.get("published_at"))
    observed_at = _parse_source_datetime(item.get("observed_at") or item.get("captured_at"))
    ...
    if dated_at is None:
        path_match = re.search(r"(?:^|/)(\d{4}-\d{2}-\d{2})__", clean_text(item.get("source_path")))
        if path_match:
            dated_at = _parse_source_datetime(path_match.group(1))
            date_origin = "source_filename"

    temporality = clean_text(item.get("source_temporality")).lower()
    if not temporality:
        temporality = "trend" if _derive_source_kind(item) == "external_signal" else "unknown"
    declared_state = clean_text(item.get("freshness_state")).lower() or None
    age_days: int | None = None
    if dated_at is not None:
        age_days = max(0, int((now_utc - dated_at).total_seconds() // 86400))
...
def normalize_feed_item_to_idea_candidate(
    item: dict[str, Any],
    *,
    ...
    freshness = _source_freshness(item, now=now)
    return IdeaCandidate(
        ...
        source_published_at=clean_text(freshness.get("published_at")),
        source_observed_at=clean_text(freshness.get("observed_at")),
        source_temporality=clean_text(freshness.get("temporality")),
        freshness_state=clean_text(freshness.get("state")),
        source_age_days=freshness.get("age_days"),
        current_claim_allowed=bool(freshness.get("current_claim_allowed")),
...
def _evaluate_source_freshness(candidate: IdeaCandidate) -> QualificationTest:
    if candidate.source_kind in {"own_thinking", "existing_backlog"}:
        return QualificationTest(True, "source freshness is not applicable to owned thinking")
    state = clean_text(candidate.freshness_state).lower() or "unknown"
    age_evidence = f"state={state}"
    if candidate.source_age_days is not None:
        age_evidence += f"; age_days={candidate.source_age_days}"
    if state in {"current", "aging", "dated_recent", "evergreen"}:
        return QualificationTest(True, "source is current enough for idea admission", evidence=age_evidence)
    if state in {"stale", "dated_stale"}:
        return QualificationTest(
            False,
            "source is older than the 30-day net-new recommendation window",
            evidence=age_evidence,
        )
    return QualificationTest(
        False,
        "source freshness is unverified and cannot support a net-new recommendation",
        evidence=age_evidence,
    )
...
def qualify_idea(candidate: IdeaCandidate) -> IdeaQualificationReport:
    ...
    source_freshness = _evaluate_source_freshness(candidate)
    core_tests = {
        "sharpness": sharpness,
    ...
    if not failure_dimensions:
        ...
        not source_freshness.passed
        and sharpness.passed
        and non_obviousness.passed
        and proof_potential.passed
        and strategic_relevance.passed
    ):
        route = "latent"
        latent_reason = "wrong_timing"
        salvageable = True
        suggested_fix = (
            "Keep the idea as background evidence until a current trigger appears or the source is "
            "explicitly and defensibly classified as evergreen."
        )
    elif (
    ...
    return IdeaQualificationReport(
        ...
        source_freshness={
            "temporality": candidate.source_temporality,
            "state": candidate.freshness_state,
            "published_at": candidate.source_published_at or None,
            "observed_at": candidate.source_observed_at or None,
            "age_days": candidate.source_age_days,
            "current_claim_allowed": candidate.current_claim_allowed,
        },
...
def _markdown_for_payload(payload: dict[str, Any]) -> str:
    ...
    for route in ("pass", "latent", "discard"):
        ...
        for item in route_items[:12]:
            ...
            lines.append(
                f"- Source freshness: {freshness.get('state') or 'unknown'}"
                + (f" ({freshness.get('age_days')} days)" if freshness.get("age_days") is not None else "")
            )
...
def archived_reaction_draft_paths(workspace_dir: Path) -> set[str]:
    """Logical draft paths retired by the state-first quarantine manifest.

    A private-state archive move must continue to shadow a same-named legacy
    repository fallback.  Otherwise the retired draft reappears on the next
    read and a later quarantine run may move the repository artifact itself.
    """
    ...
    return {
        workspace_source_path(item.get("archived_from"))
        for item in (manifest.get("items") or [])
        if isinstance(item, dict) and workspace_source_path(item.get("archived_from"))
    }
...
def active_draft_indexes(workspace_dir: Path) -> tuple[dict[str, dict[str, Any]], dict[str, dict[str, Any]]]:
    ...
    for path in generated_workspace_files(workspace_dir, "drafts", "*.md"):
        ...
        if workspace_source_path(relative_path.as_posix()) in archived_paths:
            continue
        metadata = {
...
def quarantine_stale_reaction_drafts(workspace_dir: Path) -> dict[str, Any]:
    ...
    already_archived_from = {
        workspace_source_path(item.get("archived_from"))
        for item in records
        if workspace_source_path(item.get("archived_from"))
    }
    ...
    for path in sorted(generated_drafts_root.glob("*.md")):
        ...
        source_logical_path = workspace_source_path(source_relative.as_posix())
        if source_logical_path in already_archived_from:
            continue
        ...
        record = {
            ...
            "archived_from": source_logical_path,
            "source_path": source_path,
