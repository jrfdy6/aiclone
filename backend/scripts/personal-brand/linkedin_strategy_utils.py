...
def load_rejected_source_index(workspace_dir: Path | None = None) -> tuple[set[str], dict[str, dict[str, Any]]]:
    ...
    feedback_path = generated_workspace_read_path(
        resolved_workspace,
        "analytics/feed_feedback.jsonl",
    )
...
def infer_primary_lane(item: dict[str, Any]) -> str:
    ...
    topic_tags = {clean_text(tag).lower() for tag in item.get("topic_tags") or [] if clean_text(tag)}

    # Match semantic tag tokens, never arbitrary substrings.  The historical
    # substring check routed ordinary words such as ``said``, ``email``, and
    # ``Portland`` into the AI lane merely because their spelling contains
    # ``ai``.  Source tags are intentionally small, so an explicit vocabulary
    # is both easier to audit and safer than fuzzy inference here.
    tag_tokens = {
        token
        for tag in topic_tags
        for token in re.findall(r"[a-z0-9]+", tag)
    }
    ai_phrases = (
    ...
    if "ai" in tag_tokens or "llm" in tag_tokens or any(phrase in topic_tags for phrase in ai_phrases):
        return "ai"
    if "leadership" in tag_tokens:
...
def load_queue_items(workspace_dir: Path | None = None) -> list[dict[str, Any]]:
    resolved_workspace = workspace_dir or workspace_root()
    queue_path = generated_workspace_read_path(resolved_workspace, "drafts/queue_01.md")
