...
@router.post("/ops-workspace-context/sync")
def publish_ops_workspace_context_projection(
    payload: OpsWorkspaceContextProjectionSyncRequest,
):
    projection = validate_ops_workspace_context_projection(payload.projection)
    request_generated_at = _parse_generated_at(payload.generated_at)
    projection_generated_at = _parse_generated_at(str(projection["generated_at"]))
    if request_generated_at != projection_generated_at:
        raise HTTPException(
            status_code=422,
            detail="Workspace-context projection receipt time does not match its envelope.",
        )
    semantic_observed_at = _parse_generated_at(str(projection["observed_at"]))
    try:
        stored_snapshot, stored = upsert_snapshot_monotonic(
            ...
            metadata={
                "source": "codex_local_runner",
                "projection": "ops_workspace_context",
            },
        )
    except Exception as exc:
        raise HTTPException(
            status_code=503,
            detail="Workspace-context projection storage is unavailable.",
        ) from exc
    if stored_snapshot is None:
        raise HTTPException(
            status_code=503,
    ...
    try:
        current = validate_ops_workspace_context_projection(
            stored_snapshot.get("payload")
        )
    except Exception as exc:
        raise HTTPException(
            status_code=503,
            detail="Stored workspace-context projection is invalid.",
        ) from exc
    requested_hash = hashlib.sha256(
        json.dumps(
            projection,
    ...
    requested_semantic_hash = ops_workspace_context_projection_semantic_sha256(
        projection
    )
    current_semantic_hash = ops_workspace_context_projection_semantic_sha256(current)
    current_observed_at = _parse_generated_at(str(current["observed_at"]))
    if stored:
        disposition = "stored"
    elif current_semantic_hash == requested_semantic_hash:
        disposition = "idempotent_same_cycle"
    elif current_observed_at > semantic_observed_at:
        disposition = "retained_newer"
    else:
        disposition = "retained_same_observation_conflict"
    return {
        "stored": stored,
        "disposition": disposition,
        "workspace_key": OPS_WORKSPACE_CONTEXT_WORKSPACE_KEY,
        "snapshot_type": OPS_WORKSPACE_CONTEXT_SNAPSHOT_TYPE,
        "payload_sha256": current_hash,
        "requested_payload_sha256": requested_hash,
        "semantic_payload_sha256": current_semantic_hash,
