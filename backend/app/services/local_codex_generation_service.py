...
def claim_next_codex_job(*, worker_id: str, workspace_slug: str | None = None) -> dict[str, Any] | None:
    with _LOCK:
        ...
        resumable_jobs = [
            job
            for job in jobs
            if _normalize_status(job.get("status")) == "running"
            and _job_matches_workspace(job, workspace_slug)
            and str(job.get("claimed_by") or "") == worker_id
        ]
        if resumable_jobs:
            # launchd restarts keep a stable worker id. Returning that worker's
            # unfinished claim lets it resume after a process crash without
            # opening the job to a second worker or creating duplicate work.
            resumable_jobs.sort(key=lambda item: str(item.get("claimed_at") or item.get("created_at") or ""))
            return dict(resumable_jobs[0])
        pending_jobs = [
            job
            for job in jobs
