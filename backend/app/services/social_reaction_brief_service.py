...
class SocialReactionBriefService:
    def build(
        ...
        *,
        ...
        owner_perspective: dict[str, Any],
    ) -> dict[str, Any]:
        article_view = _ensure_period(article_understanding.get("article_view"))
        owner_view = _ensure_period(owner_perspective.get("personal_reaction"))
        tension = _ensure_period(
            owner_perspective.get("pushback")
            or owner_perspective.get("what_matters_most")
            or article_understanding.get("world_stakes")
        )
        lived_addition = _normalize_inline_text(owner_perspective.get("lived_addition"))
        if lived_addition:
            content_angle = "lived_translation"
        elif _normalize_inline_text(owner_perspective.get("pushback")):
            content_angle = "qualified_disagreement"
        elif _normalize_inline_text(owner_perspective.get("agree_with")) and _normalize_inline_text(owner_perspective.get("pushback")):
        ...
        return {
            ...
            "stance_rationale": _ensure_period(owner_perspective.get("stance_rationale")),
            ...
            "audience_posture": _normalize_inline_text(owner_perspective.get("audience_posture")),
            "role_posture": _normalize_inline_text(owner_perspective.get("role_posture")),
            "do_not_overstate": do_not_overstate,
            "bridge_intent": "use_lived_addition" if lived_addition else "connect_belief_to_world_signal",
            "priority_signal": _ensure_period(owner_perspective.get("what_matters_most") or article_understanding.get("world_stakes")),
            "brief_confidence": 8.1,
            "selection_rationale": _ensure_period(
                f"The brief combines the article view, the the owner-side reaction, and the lane={lane_id} posture into one drafting packet."
            ),
            "draft_guidance": {
