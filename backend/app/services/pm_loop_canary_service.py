...
def _execution_queue_alignment_check(active_cards: list[Any], queue_entries: list[Any]) -> dict[str, Any]:
    observed_queue_card_ids = {str(entry.card_id) for entry in queue_entries}
    queue_card_ids = {
        str(entry.card_id)
        for entry in queue_entries
        if str(entry.execution_state or "").strip().lower() in {"queued", "running"}
    }
    ...
    return {
        ...
        "observed_count": len(observed_queue_card_ids),
