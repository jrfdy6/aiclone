...
def list_automations() -> List[Automation]:
    ...
    automations: List[Automation] = [
        ...
        Automation(
            ...
            description="OpenClaw-local automation that runs on the machine to discover fresh watchlist videos, download audio, transcribe with Whisper, and register the result into the shared Brain source system.",
            type="scheduled",
            status="active",
            ...
            isolation=True,
            last_run_at=_dt(hours_ago=2),
            next_run_at=datetime.utcnow().replace(second=0, microsecond=0) + timedelta(hours=2),
            metrics={
                ...
                "media_stack": "yt-dlp + ffmpeg + whisper",
            ...
            instructions=_instructions(
                ...
                "Download audio + transcribe locally when the media runtime is available",
                "Register each source into the shared Brain long-form ingest lane and refresh the LinkedIn OS snapshots",
            ),
            notes="Runs inside the OpenClaw workspace on the local machine because Railway does not have the media runtime required for transcript capture.",
        ),
        Automation(
            id="rolling_docs",
