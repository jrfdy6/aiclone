...
def list_automations() -> List[Automation]:
    ...
    automations: List[Automation] = [
        ...
        Automation(
            ...
            description="Runs on the local machine to discover fresh watchlist videos, download audio, transcribe with Whisper, and register the result into the shared Brain source system.",
            type="scheduled",
            status="active",
            ...
            isolation=True,
            last_run_at=_dt(hours_ago=1),
            next_run_at=datetime.utcnow().replace(second=0, microsecond=0) + timedelta(minutes=30),
            metrics={
            ...
            notes="Runs locally because Railway does not have the media runtime required for transcript capture.",
        ),
        Automation(
            id="rolling_docs",
