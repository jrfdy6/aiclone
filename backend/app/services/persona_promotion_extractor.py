...
from app.persona_promotion_targets import TARGET_INITIATIVES
...
def _infer_positioning_signal(item: dict[str, Any]) -> str | None:
    ...
    if _contains_any(text, ("ai", "agent", "prompt", "model")):
        return "Strengthens the owner's positioning as an AI systems operator grounded in real execution."
    if _contains_any(text, ("workflow", "process", "system")):
        return "Strengthens the owner's positioning as an operator who can connect systems thinking to real implementation."
    return "Strengthens the owner's positioning with a concrete, reusable operating signal."
