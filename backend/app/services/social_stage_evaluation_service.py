...
class SocialStageEvaluationService:
    def evaluate_variant(
        ...
        *,
        ...
        owner_perspective: dict[str, Any] | None,
        ...
        owner_perspective = owner_perspective or {}
        ...
        owner_perspective_score = 4.0
        if _normalize_inline_text(owner_perspective.get("agree_with")):
            owner_perspective_score += 1.5
        if _normalize_inline_text(owner_perspective.get("pushback")):
            owner_perspective_score += 1.3
        if _normalize_inline_text(owner_perspective.get("lived_addition")):
            owner_perspective_score += 1.3
        if _normalize_inline_text(owner_perspective.get("what_matters_most")):
            owner_perspective_score += 1.1

        reaction_brief_score = 4.0
        for field in ("article_view", "owner_view", "tension", "content_angle"):
        ...
        for field, value in {
            ...
            "owner_perspective": owner_perspective.get("personal_reaction"),
        ...
        if not _normalize_inline_text(owner_perspective.get("lived_addition")):
        ...
        return {
            ...
            "owner_perspective_score": _round_score(owner_perspective_score),
