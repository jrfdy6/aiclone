...
def _safe_feezie_publication_performance(workspace_key: str, root_slug: str) -> dict[str, Any] | None:
    if canonicalize_workspace_key(workspace_key, default=workspace_key) != "feezie-os":
        return None
    ...
    for key in _workspace_snapshot_keys(workspace_key, root_slug):
        try:
            ...
            payloads = {}
        if summary is None and isinstance(payloads.get("publication_performance_summary"), dict):
            summary = payloads["publication_performance_summary"]
        if status is None and isinstance(payloads.get("publication_performance_status"), dict):
            status = payloads["publication_performance_status"]
    ...
    recent = summary.get("recent_publications") if isinstance(summary, dict) and isinstance(summary.get("recent_publications"), list) else []
    due_actions: dict[str, int] = {}
    for publication in recent[:10]:
        if not isinstance(publication, dict):
            continue
        for action in publication.get("due_actions") or []:
            code = _clean_text(action)[:80]
            if code:
                due_actions[code] = due_actions.get(code, 0) + 1
    completeness = (
        summary.get("feedback_completeness")
        if isinstance(summary, dict) and isinstance(summary.get("feedback_completeness"), dict)
        else {}
    )
    aggregate_due_actions = {
        "record_24h_metrics": int((completeness.get("metrics_24h_recorded") or {}).get("missing") or 0)
        if isinstance(completeness.get("metrics_24h_recorded"), dict)
        else 0,
        "record_7d_metrics": int((completeness.get("metrics_7d_recorded") or {}).get("missing") or 0)
        if isinstance(completeness.get("metrics_7d_recorded"), dict)
        else 0,
        "record_owner_assessment": max(
            0,
            int(counts.get("confirmed_publications") or 0) - int(counts.get("owner_assessments") or 0),
        ),
    }
    for action, count in aggregate_due_actions.items():
        if count:
            due_actions[action] = max(due_actions.get(action, 0), count)
    gaps = summary.get("actionable_gaps") if isinstance(summary, dict) and isinstance(summary.get("actionable_gaps"), list) else []
    safe_gaps = [
        {
            "code": _clean_text(item.get("code"))[:120],
            "severity": _clean_text(item.get("severity"))[:20],
            "actionable": item.get("actionable") is True,
            "next_action": sanitize_brain_text(_clean_text(item.get("next_action"))[:260]),
        }
        for item in gaps[:6]
        if isinstance(item, dict) and _clean_text(item.get("code"))
    ]
    topic_mix = summary.get("rolling_topic_mix") if isinstance(summary, dict) and isinstance(summary.get("rolling_topic_mix"), dict) else {}
    intent_mix = summary.get("rolling_intent_mix") if isinstance(summary, dict) and isinstance(summary.get("rolling_intent_mix"), dict) else {}
    status_state = _clean_text((status or {}).get("state")) or ("fresh" if summary else "missing")
    return {
        "schema_version": "portfolio_publication_performance/v1",
        "state": status_state,
        "generated_at": (summary or {}).get("generated_at"),
        "confirmed_publications": int(counts.get("confirmed_publications") or 0),
        "complete_feedback_posts": int(counts.get("complete_feedback_posts") or 0),
        "owner_assessments": int(counts.get("owner_assessments") or 0),
        "approved_unpublished": int(counts.get("approved_unpublished") or 0),
        "due_actions": due_actions,
        "due_action_count": sum(due_actions.values()),
        "topic_mix": {
            "status": _clean_text(topic_mix.get("status"))[:40],
            "sample_size": int(topic_mix.get("sample_size") or 0),
            "deficit_total": sum(int(value or 0) for value in (topic_mix.get("deficits") or {}).values())
            if isinstance(topic_mix.get("deficits"), dict)
            else 0,
        },
        "intent_mix": {
            "status": _clean_text(intent_mix.get("status"))[:40],
            "sample_size": int(intent_mix.get("sample_size") or 0),
            "deficit_total": sum(int(value or 0) for value in (intent_mix.get("deficits") or {}).values())
            if isinstance(intent_mix.get("deficits"), dict)
            else 0,
        },
        "actionable_gaps": safe_gaps,
        "data_policy": {
            "aggregate_only": True,
            "publication_urls_included": False,
...
def _build_workspace_summary(
    ...
    *,
    ...
    publication_performance = _safe_feezie_publication_performance(workspace_key, root_slug)
    performance_due_actions = int((publication_performance or {}).get("due_action_count") or 0)
    performance_state = str((publication_performance or {}).get("state") or "")
    performance_system_issue = performance_state in {"degraded", "corrupt"}
    if attention_summary.get("status") == "clear" and performance_due_actions:
        attention_summary = {
            **attention_summary,
            "status": "needs_host",
            "label": "Needs performance follow-up",
            "reasons": [f"{performance_due_actions} publication feedback action(s) are due."],
            "needs_operator": True,
        }
    elif attention_summary.get("status") == "clear" and performance_system_issue:
        attention_summary = {
            **attention_summary,
            "status": "system_issue",
            "label": "Performance truth needs repair",
            "reasons": [f"The publication-performance projection is {performance_state}."],
    ...
    return {
        ...
        "counts": {
            ...
            "performance_due_actions": performance_due_actions,
        },
        "active_blockers": active_blockers,
        ...
        "has_system_issue": bool(attention_summary.get("has_system_issue") or performance_system_issue),
        "needs_brain_attention": bool(
            attention_summary.get("needs_operator")
            or attention_summary.get("has_system_issue")
            or performance_due_actions
            or performance_system_issue
        ),
