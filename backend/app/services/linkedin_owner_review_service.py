from __future__ import annotations

import hashlib
import json
import re
from datetime import datetime, timezone
from pathlib import Path
from typing import Any

from app.models import (
    LinkedinPerformanceLocalActionRequest,
    PMCardCreate,
    PMCardDispatchRequest,
    PMCardUpdate,
)
from app.services import pm_card_service, workspace_snapshot_service
from app.services.brain_local_action_queue_service import enqueue_brain_local_action
from app.services.feezie_positioning_contract_service import load_feezie_strategy_contract
from app.services.linkedin_performance_ledger_service import linkedin_content_version_sha256
from app.services.pm_execution_contract_service import build_execution_contract
from runtime_paths import workspace_state_root

QUEUE_HEADING_RE = re.compile(r"^###\s+(FEEZIE-\d+)\s*-\s*(.+)$", flags=re.MULTILINE)
PACKET_HEADING_RE = re.compile(r"^##\s+(FEEZIE-\d+)\s+[—-]\s+(.+)$", flags=re.MULTILINE)

STATUS_MAP = {
    "approve": "approved",
    "revise": "revision_requested",
    "park": "parked",
}

APPROVAL_STATUS_MAP = {
    "approve": "owner_approved",
    "revise": "owner_revision_requested",
    "park": "owner_parked",
}

PUBLISH_POSTURE_MAP = {
    "approve": "approved",
    "revise": "owner_review_required",
    "park": "hold_private",
}

PACKET_CHECKBOX_LINES = {
    "approve": "Approve for scheduling",
    "revise": "Revise (note specifics below)",
    "park": "Park for later",
}

PM_WORKSPACE_KEY = "linkedin-os"
OWNER_REVIEW_CARD_SOURCE = "codex_native:workspace-owner-review"
LEGACY_OWNER_REVIEW_CARD_SOURCE = "openclaw:workspace-owner-review"
OWNER_REVIEW_LINK_TYPE = "owner_review"
SUPPLEMENTAL_OWNER_REVIEW_SOURCE_KINDS = {"latent_transform"}
SUPPLEMENTAL_OWNER_REVIEW_ID_PREFIX = "LATENT"
GENERATED_OWNER_REVIEW_SOURCE_KIND = "codex_generation"
GENERATED_OWNER_REVIEW_ID_PREFIX = "FEEZIE-CODEX"
ACTIVE_OWNER_REVIEW_CARD_STATUSES = {"review", "todo", "in_progress"}
OWNER_REVIEW_LIFECYCLE_JOIN_SCHEMA = "feezie_owner_review_lifecycle_join/v1"
OWNER_REVIEW_LIFECYCLE_EVENT_SCHEMA = "feezie_owner_review_lifecycle_event/v1"
GENERATED_CRITIC_DIMENSIONS = {"truth", "safety", "intent", "voice", "hook"}
GENERATED_HOOK_VARIANT_COUNT = 8
DETERMINISTIC_QUALITY_GATE_VERSION = "feezie_deterministic_quality_gate/v2"
SELECTED_OPTION_QUALITY_VERSION = "feezie_selected_option_quality_gate/v1"
FEEZIE_REVISION_CONTRACT_VERSION = "feezie_critic_guided_revision_contract/v1"
FEEZIE_REVISION_RECEIPT_VERSION = "feezie_revision_execution_receipt/v1"
SELECTED_REVISION_VERIFICATION_VERSION = "feezie_selected_revision_verification/v1"
APPROVABLE_EMPLOYER_SAFETY = {"pass", "owner_review_required"}


class LinkedinOwnerReviewNotFoundError(ValueError):
    """The requested owner-review item does not exist in live or PM history."""


class LinkedinOwnerReviewConflictError(ValueError):
    """The requested decision conflicts with durable review or critic truth."""


def _require_legacy_owner_review_compatibility(legacy_compatibility: bool) -> None:
    """Fail closed before the retired owner-review lane can mutate any state."""

    if legacy_compatibility is not True:
        raise LinkedinOwnerReviewConflictError(
            "The historical FEEZIE owner-review writer is disabled by default; "
            "use the canonical integrated-content lifecycle or explicitly enable "
            "the rollback-only compatibility path."
        )


def _is_owner_review_source(value: Any) -> bool:
    return str(value or "").strip() in {OWNER_REVIEW_CARD_SOURCE, LEGACY_OWNER_REVIEW_CARD_SOURCE}


def _owner_review_link_id() -> str | None:
    return None


def _linkedin_root() -> Path:
    return workspace_snapshot_service._discover_linkedin_root()


def _queue_path(root: Path) -> Path:
    return root / "drafts" / "queue_01.md"


def _drafts_root(root: Path) -> Path:
    return root / "drafts"


def _private_feezie_drafts_root(root: Path) -> Path | None:
    """Return private generated drafts only for the canonical FEEZIE workspace."""

    try:
        canonical_root = workspace_snapshot_service._discover_linkedin_root().resolve()
    except Exception:
        return None
    if root.resolve() != canonical_root:
        return None
    return workspace_state_root("feezie-os") / "drafts"


def _owner_review_draft_paths(root: Path) -> list[tuple[str, Path]]:
    """Return state-first drafts, with canonical state shadowing repo aliases."""

    roots: list[Path] = []
    private_root = _private_feezie_drafts_root(root)
    if private_root is not None:
        roots.append(private_root)
    roots.append(_drafts_root(root))
    selected: dict[str, tuple[str, Path]] = {}
    for drafts_root in roots:
        if not drafts_root.is_dir():
            continue
        for path in sorted(drafts_root.glob("*.md")):
            selected.setdefault(path.name, (f"drafts/{path.name}", path))
    return [selected[name] for name in sorted(selected)]


def _resolve_current_owner_review_draft(root: Path, raw_path: Any) -> Path | None:
    cleaned = _clean_markdown_value(str(raw_path or ""))
    workspace_prefix = "workspaces/linkedin-content-os/"
    if cleaned.startswith(workspace_prefix):
        cleaned = cleaned[len(workspace_prefix) :]
    relative = Path(cleaned)
    if (
        not cleaned
        or relative.is_absolute()
        or ".." in relative.parts
        or not relative.parts
        or relative.parts[0] != "drafts"
    ):
        return None
    candidates: list[Path] = []
    private_root = _private_feezie_drafts_root(root)
    if private_root is not None:
        candidates.append(private_root.parent / relative)
    candidates.append(root / relative)
    return next((path for path in candidates if path.is_file()), None)


def _latest_owner_packet(root: Path) -> Path | None:
    packets = sorted(_drafts_root(root).glob("feezie_owner_review_packet_*.md"))
    return packets[-1] if packets else None


def _execution_log_path(root: Path) -> Path:
    return root / "memory" / "execution_log.md"


def _read_text(path: Path | None) -> str:
    if path is None or not path.exists():
        return ""
    return path.read_text(encoding="utf-8")


def _clean_markdown_value(value: str) -> str:
    cleaned = value.strip()
    if cleaned.startswith("`") and cleaned.endswith("`"):
        cleaned = cleaned[1:-1]
    return cleaned


def _compact_slug(value: str) -> str:
    return re.sub(r"[^a-z0-9]+", "-", value.lower()).strip("-")


def _supplemental_queue_id(title: str, draft_rel_path: str) -> str:
    title_slug = _compact_slug(title)
    short_title = "-".join(part for part in title_slug.split("-") if part)[:36].strip("-")
    digest = hashlib.sha1(draft_rel_path.encode("utf-8")).hexdigest()[:6].upper()
    if short_title:
        return f"{SUPPLEMENTAL_OWNER_REVIEW_ID_PREFIX}-{short_title.upper()}-{digest}"
    return f"{SUPPLEMENTAL_OWNER_REVIEW_ID_PREFIX}-{digest}"


def _owner_review_identity_key(
    *,
    queue_id: str,
    entry_kind: str,
    source_kind: str,
    idea_id: Any = None,
    source_path: Any = None,
    source_url: Any = None,
    title: Any = None,
) -> str:
    normalized_queue_id = str(queue_id or "").strip()
    if str(entry_kind or "").strip() != "supplemental":
        return normalized_queue_id
    normalized_source_kind = _compact_slug(str(source_kind or "").strip()) or "supplemental"
    for label, raw_value in (
        ("idea", idea_id),
        ("source_path", source_path),
        ("source_url", source_url),
        ("title", title),
    ):
        normalized_value = _compact_slug(str(raw_value or "").strip())
        if normalized_value:
            return f"{normalized_source_kind}:{label}:{normalized_value}"
    if normalized_queue_id:
        return f"{normalized_source_kind}:queue:{_compact_slug(normalized_queue_id)}"
    return ""


def _owner_review_identity_key_for_item(item: dict[str, Any]) -> str:
    return _owner_review_identity_key(
        queue_id=str(item.get("queue_id") or "").strip(),
        entry_kind=str(item.get("entry_kind") or "queue").strip(),
        source_kind=str(item.get("source_kind") or "feezie_queue").strip(),
        idea_id=item.get("idea_id"),
        source_path=item.get("source_path"),
        source_url=item.get("source_url"),
        title=item.get("title"),
    )


def _owner_review_identity_key_from_payload(owner_review_payload: dict[str, Any]) -> str:
    explicit = str(owner_review_payload.get("identity_key") or "").strip()
    if explicit:
        return explicit
    return _owner_review_identity_key(
        queue_id=str(owner_review_payload.get("queue_id") or "").strip(),
        entry_kind=str(owner_review_payload.get("entry_kind") or "queue").strip(),
        source_kind=str(owner_review_payload.get("source_kind") or "feezie_queue").strip(),
        idea_id=owner_review_payload.get("idea_id"),
        source_path=owner_review_payload.get("source_path"),
        source_url=owner_review_payload.get("source_url"),
        title=owner_review_payload.get("title"),
    )


def _owner_review_trigger_key(identity_key: str) -> str:
    return f"owner-review:{identity_key}"


def _section_bounds(text: str, heading_re: re.Pattern[str], queue_id: str) -> tuple[int, int] | None:
    matches = list(heading_re.finditer(text))
    for index, match in enumerate(matches):
        if match.group(1).strip() != queue_id:
            continue
        end = matches[index + 1].start() if index + 1 < len(matches) else len(text)
        return match.start(), end
    return None


def _split_queue_blocks(section: str) -> list[tuple[str, str, str]]:
    matches = list(QUEUE_HEADING_RE.finditer(section))
    blocks: list[tuple[str, str, str]] = []
    for index, match in enumerate(matches):
        start = match.end()
        end = matches[index + 1].start() if index + 1 < len(matches) else len(section)
        blocks.append((match.group(1).strip(), match.group(2).strip(), section[start:end].strip()))
    return blocks


def _parse_bullet_fields(block: str) -> tuple[dict[str, str], dict[str, list[str]]]:
    fields: dict[str, str] = {}
    lists: dict[str, list[str]] = {}
    current_key: str | None = None
    for raw_line in block.splitlines():
        line = raw_line.rstrip()
        stripped = line.strip()
        if stripped.startswith("- ") and ":" in stripped and not line.startswith("  "):
            key, value = stripped[2:].split(":", 1)
            current_key = key.strip().lower()
            fields[current_key] = _clean_markdown_value(value)
            continue
        if current_key and stripped.startswith("- ") and line.startswith("  "):
            lists.setdefault(current_key, []).append(_clean_markdown_value(stripped[2:]))
            continue
        if current_key and line.startswith("  ") and stripped:
            existing = fields.get(current_key, "")
            fields[current_key] = f"{existing}\n{_clean_markdown_value(stripped)}".strip()
    return fields, lists


def _parse_frontmatter(text: str) -> tuple[dict[str, str], str]:
    if not text.startswith("---\n"):
        return {}, text
    end = text.find("\n---\n", 4)
    if end == -1:
        return {}, text
    frontmatter_text = text[4:end]
    body = text[end + 5 :]
    fields: dict[str, str] = {}
    for raw_line in frontmatter_text.splitlines():
        if ":" not in raw_line:
            continue
        key, value = raw_line.split(":", 1)
        fields[key.strip()] = value.strip().strip('"').strip("'")
    return fields, body


def _yaml_scalar(value: str) -> str:
    if not value:
        return '""'
    if re.search(r"[:#\s]", value):
        return json.dumps(value)
    return value


def _set_frontmatter_fields(text: str, updates: dict[str, str]) -> str:
    if not text.startswith("---\n"):
        return text
    end = text.find("\n---\n", 4)
    if end == -1:
        return text
    frontmatter_lines = text[4:end].splitlines()
    body = text[end + 5 :]
    keys_seen: set[str] = set()
    updated_lines: list[str] = []
    for line in frontmatter_lines:
        if ":" not in line:
            updated_lines.append(line)
            continue
        key, _ = line.split(":", 1)
        normalized_key = key.strip()
        if normalized_key in updates:
            updated_lines.append(f"{normalized_key}: {_yaml_scalar(updates[normalized_key])}")
            keys_seen.add(normalized_key)
        else:
            updated_lines.append(line)
    for key, value in updates.items():
        if key in keys_seen:
            continue
        updated_lines.append(f"{key}: {_yaml_scalar(value)}")
    updated_frontmatter = "\n".join(updated_lines)
    return f"---\n{updated_frontmatter}\n---\n{body}"


def _extract_section_body(text: str, heading: str) -> str:
    pattern = re.compile(rf"^##\s+{re.escape(heading)}\s*$", flags=re.MULTILINE)
    match = pattern.search(text)
    if not match:
        return ""
    start = match.end()
    next_match = re.search(r"^##\s+", text[start:], flags=re.MULTILINE)
    end = start + next_match.start() if next_match else len(text)
    return text[start:end].strip()


def _extract_packet_section(packet_text: str, queue_id: str) -> str:
    bounds = _section_bounds(packet_text, PACKET_HEADING_RE, queue_id)
    if bounds is None:
        return ""
    start, end = bounds
    return packet_text[start:end].strip()


def _parse_packet_decision(section: str) -> tuple[str | None, str | None, str | None]:
    current_decision: str | None = None
    for decision, label in PACKET_CHECKBOX_LINES.items():
        if re.search(rf"^- \[[xX]\]\s+{re.escape(label)}\s*$", section, flags=re.MULTILINE):
            current_decision = decision
            break
    recommendation_match = re.search(r"\*\*Recommended action:\*\*\s*(.+)", section)
    notes_match = re.search(r"\*\*Revision notes \(if needed\):\*\*\n(.*?)(?:\n---\n|\Z)", section, flags=re.DOTALL)
    notes = notes_match.group(1).strip() if notes_match else ""
    if notes == "---":
        notes = ""
    return current_decision, recommendation_match.group(1).strip() if recommendation_match else None, notes or None


def _normalize_relative_path(root: Path, raw_path: str) -> str:
    cleaned = _clean_markdown_value(raw_path)
    if not cleaned:
        return ""
    if cleaned.startswith("(") and cleaned.endswith(")"):
        cleaned = cleaned[1:-1]
    if cleaned.startswith("drafts/"):
        return cleaned
    try:
        return Path(cleaned).resolve().relative_to(root).as_posix()
    except Exception:
        return cleaned


def _draft_path_from_status(root: Path, status_value: str) -> tuple[str, Path | None]:
    match = re.search(r"\(([^)]+)\)", status_value)
    if not match:
        return "", None
    relative_path = _normalize_relative_path(root, match.group(1))
    if not relative_path:
        return "", None
    return relative_path, root / relative_path


def _assessment_recommendation_text(item: dict[str, Any]) -> str:
    return re.sub(r"\*+", "", str(item.get("packet_recommendation") or "")).strip()


def _build_owner_review_assessment(item: dict[str, Any]) -> dict[str, Any]:
    recommendation_text = _assessment_recommendation_text(item)
    recommendation_lower = recommendation_text.lower()
    source_kind = str(item.get("source_kind") or "").strip()
    latent_reason = str(item.get("latent_reason") or "").strip()
    first_pass_draft = str(item.get("first_pass_draft") or "").strip()
    proof_anchors = [str(anchor).strip() for anchor in (item.get("proof_anchors") or []) if str(anchor).strip()]
    draft_owner_notes = [str(note).strip() for note in (item.get("draft_owner_notes") or []) if str(note).strip()]
    revision_goals = [str(goal).strip() for goal in (item.get("revision_goals") or []) if str(goal).strip()]

    decision = "revise"
    confidence = "medium"
    reasons: list[str] = []
    missing_items: list[str] = []

    if recommendation_text:
        if "park" in recommendation_lower:
            decision = "park"
            confidence = "medium"
            reasons.append("The owner packet already leans toward parking this instead of advancing it.")
        elif "approve" in recommendation_lower and "after" not in recommendation_lower:
            decision = "approve"
            confidence = "high"
            reasons.append("The owner packet already recommends approval or scheduling.")
        elif "revise" in recommendation_lower or "after" in recommendation_lower:
            decision = "revise"
            confidence = "medium"
            reasons.append("The owner packet still frames this as conditional rather than schedule-ready.")

    if source_kind == "latent_transform":
        decision = "revise"
        confidence = "high" if first_pass_draft and revision_goals else "medium"
        reasons.append("This came from the latent transform lane, so it is still being translated into your angle and audience context.")
        if latent_reason == "needs_context_translation":
            missing_items.append("The audience consequence is still implied rather than clearly named.")

    if not first_pass_draft:
        missing_items.append("There is no first-pass draft attached yet.")
    elif len(first_pass_draft.split()) < 70:
        missing_items.append("The first-pass draft is still short enough that the final angle may not be fully expressed.")

    if not proof_anchors:
        missing_items.append("No proof anchors are attached yet.")

    if not str(item.get("core_angle") or "").strip():
        missing_items.append("The core angle is not stated explicitly.")

    if not str(item.get("why_now") or "").strip():
        missing_items.append("The timing or audience consequence is not stated explicitly.")

    if draft_owner_notes:
        reasons.append("The draft already carries explicit owner notes, so the system should not assume it is frictionless.")

    if revision_goals:
        reasons.append("There are named revision goals, which usually means this is closer to revise than approve.")

    if decision == "approve" and missing_items:
        confidence = "medium"
    elif decision == "park" and len(missing_items) >= 2:
        confidence = "high"
    elif decision == "revise" and (revision_goals or source_kind == "latent_transform" or len(missing_items) >= 2):
        confidence = "high"
    elif decision == "revise" and not missing_items:
        confidence = "medium"

    if decision == "approve":
        summary = "System suggestion: approve if the hook and proof feel publicly defensible on one clean read."
        fallback_action = "If you still want to add a story beat, sharper audience consequence, or cleaner proof line, choose Revise instead."
    elif decision == "park":
        summary = "System suggestion: park this unless you already know exactly how it becomes stronger in the next cycle."
        fallback_action = "Only keep it alive if the angle matters strategically and you know what concrete proof or timing change would rescue it."
    else:
        summary = "System suggestion: revise before this moves forward."
        fallback_action = "Approve only if you can already defend the claim, proof, and audience consequence without adding anything material."

    return {
        "suggested_decision": decision,
        "confidence": confidence,
        "summary": summary,
        "reasons": reasons[:3],
        "missing_items": missing_items[:4],
        "fallback_action": fallback_action,
    }


def _build_owner_decision_scaffold(item: dict[str, Any], assessment: dict[str, Any]) -> dict[str, Any]:
    title = str(item.get("title") or item.get("queue_id") or "this draft").strip()
    core_angle = str(item.get("core_angle") or "").strip()
    why_now = str(item.get("why_now") or "").strip()
    draft_owner_notes = [str(note).strip() for note in (item.get("draft_owner_notes") or []) if str(note).strip()]
    revision_goals = [str(goal).strip() for goal in (item.get("revision_goals") or []) if str(goal).strip()]
    missing_items = [str(entry).strip() for entry in (assessment.get("missing_items") or []) if str(entry).strip()]
    suggested_decision = str(assessment.get("suggested_decision") or "revise").strip() or "revise"

    must_verify = [
        "Read the first-pass draft, not just the PM summary or recommendation.",
    ]
    if core_angle:
        must_verify.append(f"Confirm the draft clearly lands the stated angle: {core_angle}")
    if why_now:
        must_verify.append(f"Confirm the draft makes the current stakes or audience consequence legible: {why_now}")
    if item.get("proof_anchors"):
        must_verify.append("Check that at least one proof anchor or lived proof line is strong enough to defend publicly.")

    approve_when = [
        "The draft is publishable on one clean read without adding anything material.",
        "The claim, proof, and audience consequence feel publicly defensible as written.",
    ]
    if core_angle:
        approve_when.append("The first paragraph and close still support the named core angle.")
    if not missing_items:
        approve_when.append("No obvious gaps remain in the current owner packet.")

    revise_when = [
        "You want one more concrete proof line, story beat, or audience consequence before this moves forward.",
    ]
    revise_when.extend(missing_items[:3])
    revise_when.extend(revision_goals[:3])

    optional_strengtheners = list(dict.fromkeys(draft_owner_notes[:3]))
    if optional_strengtheners:
        revise_when.append("The current draft is close, but you want one of the optional strengtheners applied first.")

    park_when = [
        "You do not want to say this publicly right now, even after a revision pass.",
        "The angle is strategically weak enough that it should leave the active queue instead of consuming more iteration.",
    ]
    if suggested_decision == "park":
        park_when.insert(0, "The current packet already leans toward parking rather than advancing this draft.")

    if suggested_decision == "approve":
        neo_answer_contract = "Approve if it feels publishable on one clean read; otherwise revise with one concrete reason."
    elif suggested_decision == "park":
        neo_answer_contract = "Park unless you already know the concrete proof, timing, or angle change that would rescue it."
    else:
        neo_answer_contract = "Revise unless you can already defend the claim, proof, and audience consequence without adding anything material."

    return {
        "decision_question": f"Should `{title}` move forward now, be revised, or be parked?",
        "recommended_decision": suggested_decision,
        "must_verify": must_verify[:4],
        "approve_when": list(dict.fromkeys(approve_when))[:4],
        "revise_when": list(dict.fromkeys(revise_when))[:4],
        "park_when": list(dict.fromkeys(park_when))[:3],
        "optional_strengtheners": optional_strengtheners,
        "neo_answer_contract": neo_answer_contract,
    }


def _serialize_item(root: Path, queue_id: str, title: str, fields: dict[str, str], list_fields: dict[str, list[str]], packet_path: Path | None) -> dict[str, Any]:
    status_value = fields.get("status", "")
    draft_rel_path, draft_path = _draft_path_from_status(root, status_value)
    draft_text = _read_text(draft_path)
    frontmatter, draft_body = _parse_frontmatter(draft_text)
    packet_text = _read_text(packet_path)
    packet_section = _extract_packet_section(packet_text, queue_id)
    current_decision, recommendation, decision_notes = _parse_packet_decision(packet_section)
    if not current_decision:
        current_decision = frontmatter.get("owner_decision") or None
    current_notes = decision_notes or frontmatter.get("owner_review_notes") or None
    item = {
        "queue_id": queue_id,
        "title": title,
        "lane": fields.get("lane", ""),
        "format": fields.get("format", ""),
        "core_angle": fields.get("core angle", ""),
        "why_now": fields.get("why now", ""),
        "status": status_value.split("(", 1)[0].strip(),
        "approval_status": _clean_markdown_value(fields.get("approval status", "")),
        "draft_path": draft_rel_path,
        "owner_packet_path": packet_path.relative_to(root).as_posix() if packet_path and packet_path.exists() else None,
        "proof_anchors": list_fields.get("proof anchors", []),
        "draft_body": draft_body.strip(),
        "first_pass_draft": _extract_section_body(draft_body, "First-pass draft"),
        "draft_owner_notes": [
            line[2:].strip()
            for line in _extract_section_body(draft_body, "Owner notes").splitlines()
            if line.strip().startswith("- ")
        ],
        "packet_section": packet_section,
        "packet_recommendation": recommendation,
        "current_decision": current_decision,
        "current_notes": current_notes,
        "publish_posture": frontmatter.get("publish_posture", ""),
        "reviewed_at": frontmatter.get("owner_reviewed_at") or None,
        "created_at": frontmatter.get("created_at") or None,
        "entry_kind": "queue",
        "source_kind": frontmatter.get("source_kind", "feezie_queue"),
        "source_url": frontmatter.get("source_url") or None,
        "source_path": frontmatter.get("source_path") or None,
        "idea_id": frontmatter.get("idea_id") or None,
    }
    item["identity_key"] = _owner_review_identity_key_for_item(item)
    item["system_assessment"] = _build_owner_review_assessment(item)
    item["decision_scaffold"] = _build_owner_decision_scaffold(item, item["system_assessment"])
    return item


def _serialize_supplemental_owner_review_item(
    root: Path,
    draft_path: Path,
    *,
    draft_rel_path: str | None = None,
) -> dict[str, Any] | None:
    draft_text = _read_text(draft_path)
    if not draft_text:
        return None
    frontmatter, draft_body = _parse_frontmatter(draft_text)
    source_kind = str(frontmatter.get("source_kind") or "").strip()
    if source_kind not in SUPPLEMENTAL_OWNER_REVIEW_SOURCE_KINDS:
        return None
    if str(frontmatter.get("publish_posture") or "").strip() != "owner_review_required":
        return None

    draft_rel_path = draft_rel_path or draft_path.relative_to(root).as_posix()
    title = str(frontmatter.get("title") or draft_path.stem).strip()
    queue_id = _supplemental_queue_id(title, draft_rel_path)
    why_this_exists_fields, _ = _parse_bullet_fields(_extract_section_body(draft_body, "Why this draft exists"))
    source_signal_fields, _ = _parse_bullet_fields(_extract_section_body(draft_body, "Source signal"))
    transform_fields, _ = _parse_bullet_fields(_extract_section_body(draft_body, "Transform brief"))
    revision_goals = [
        line[2:].strip()
        for line in _extract_section_body(draft_body, "Revision goals").splitlines()
        if line.strip().startswith("- ")
    ]
    owner_notes = [
        line[2:].strip()
        for line in _extract_section_body(draft_body, "Owner notes").splitlines()
        if line.strip().startswith("- ")
    ]
    proof_anchors: list[str] = []
    source_file = str(source_signal_fields.get("source file") or "").strip()
    proof_prompt = str(transform_fields.get("proof prompt") or "").strip()
    if source_file:
        proof_anchors.append(f"Source file: {source_file}")
    if proof_prompt:
        proof_anchors.append(f"Proof prompt: {proof_prompt}")

    item = {
        "queue_id": queue_id,
        "title": title,
        "lane": str(frontmatter.get("lane") or ""),
        "format": str(frontmatter.get("draft_kind") or "owner_review"),
        "draft_kind": str(frontmatter.get("draft_kind") or ""),
        "core_angle": str(transform_fields.get("proposed angle") or ""),
        "why_now": str(source_signal_fields.get("why it matters") or why_this_exists_fields.get("priority lane") or ""),
        "status": "owner_review_draft",
        "approval_status": "owner_review_required",
        "draft_path": draft_rel_path,
        "owner_packet_path": None,
        "proof_anchors": proof_anchors,
        "draft_body": draft_body.strip(),
        "first_pass_draft": _extract_section_body(draft_body, "First-pass transformed draft") or _extract_section_body(draft_body, "First-pass draft"),
        "draft_owner_notes": owner_notes,
        "packet_section": "",
        "packet_recommendation": str(transform_fields.get("promotion rule") or "Revise before promotion into the main FEEZIE queue."),
        "current_decision": frontmatter.get("owner_decision") or None,
        "current_notes": frontmatter.get("owner_review_notes") or None,
        "publish_posture": str(frontmatter.get("publish_posture") or ""),
        "reviewed_at": frontmatter.get("owner_reviewed_at") or None,
        "created_at": frontmatter.get("created_at") or None,
        "entry_kind": "supplemental",
        "source_kind": source_kind,
        "source_url": frontmatter.get("source_url") or None,
        "source_path": frontmatter.get("source_path") or source_file or None,
        "idea_id": frontmatter.get("idea_id") or None,
        "summary": source_signal_fields.get("source summary") or "",
        "revision_goals": revision_goals,
        "latent_reason": frontmatter.get("latent_reason") or None,
        "transform_type": frontmatter.get("transform_type") or None,
        "generated_by": frontmatter.get("generated_by") or None,
    }
    item["identity_key"] = _owner_review_identity_key_for_item(item)
    item["system_assessment"] = _build_owner_review_assessment(item)
    item["decision_scaffold"] = _build_owner_decision_scaffold(item, item["system_assessment"])
    return item


def _list_supplemental_owner_review_items(root: Path) -> list[dict[str, Any]]:
    items: list[dict[str, Any]] = []
    for draft_rel_path, draft_path in _owner_review_draft_paths(root):
        if draft_path.name == "README.md" or draft_path.name.startswith("queue_") or draft_path.name.startswith("feezie_owner_review_packet_"):
            continue
        item = _serialize_supplemental_owner_review_item(
            root,
            draft_path,
            draft_rel_path=draft_rel_path,
        )
        if item is not None:
            items.append(item)
    return items


def _item_from_owner_review_payload(owner_review_payload: dict[str, Any]) -> dict[str, Any] | None:
    queue_id = str(owner_review_payload.get("queue_id") or "").strip()
    if not queue_id:
        return None
    title = str(owner_review_payload.get("title") or queue_id).strip()
    decision = str(owner_review_payload.get("current_decision") or owner_review_payload.get("decision") or "").strip().lower()
    item = {
        "queue_id": queue_id,
        "title": title,
        "lane": str(owner_review_payload.get("lane") or ""),
        "format": str(owner_review_payload.get("format") or ""),
        "core_angle": str(owner_review_payload.get("core_angle") or ""),
        "why_now": str(owner_review_payload.get("why_now") or ""),
        "status": str(owner_review_payload.get("status") or STATUS_MAP.get(decision) or ""),
        "approval_status": str(owner_review_payload.get("approval_status") or APPROVAL_STATUS_MAP.get(decision) or ""),
        "draft_path": str(owner_review_payload.get("draft_path") or ""),
        "owner_packet_path": owner_review_payload.get("owner_packet_path"),
        "proof_anchors": list(owner_review_payload.get("proof_anchors") or []),
        "draft_body": "",
        "first_pass_draft": str(owner_review_payload.get("first_pass_draft") or ""),
        "draft_owner_notes": list(owner_review_payload.get("draft_owner_notes") or []),
        "packet_section": "",
        "packet_recommendation": str(owner_review_payload.get("packet_recommendation") or ""),
        "current_decision": decision or None,
        "current_notes": owner_review_payload.get("current_notes") or owner_review_payload.get("notes"),
        "publish_posture": str(owner_review_payload.get("publish_posture") or PUBLISH_POSTURE_MAP.get(decision) or ""),
        "reviewed_at": owner_review_payload.get("reviewed_at"),
        "created_at": owner_review_payload.get("created_at"),
        "entry_kind": str(owner_review_payload.get("entry_kind") or "queue"),
        "source_kind": str(owner_review_payload.get("source_kind") or "feezie_queue"),
        "source_url": owner_review_payload.get("source_url"),
        "source_path": owner_review_payload.get("source_path"),
        "idea_id": owner_review_payload.get("idea_id"),
        "identity_key": _owner_review_identity_key_from_payload(owner_review_payload),
        "summary": str(owner_review_payload.get("summary") or ""),
        "revision_goals": list(owner_review_payload.get("revision_goals") or []),
        "latent_reason": owner_review_payload.get("latent_reason"),
        "transform_type": owner_review_payload.get("transform_type"),
        "generated_by": owner_review_payload.get("generated_by"),
        "generation_job_id": owner_review_payload.get("generation_job_id"),
        "generation_option_index": owner_review_payload.get("generation_option_index"),
        "source_card": dict(owner_review_payload.get("source_card") or {}) if isinstance(owner_review_payload.get("source_card"), dict) else {},
        "intent": owner_review_payload.get("intent"),
        "canonical_pillar": owner_review_payload.get("canonical_pillar"),
        "career_signal": owner_review_payload.get("career_signal"),
        "employer_proximity": owner_review_payload.get("employer_proximity"),
        "employer_safety": owner_review_payload.get("employer_safety"),
        "proof_posture": owner_review_payload.get("proof_posture"),
        "audience": owner_review_payload.get("audience"),
        "audience_consequence": owner_review_payload.get("audience_consequence"),
        "distinct_thesis": owner_review_payload.get("distinct_thesis"),
        "generation_receipt": (
            dict(owner_review_payload.get("generation_receipt") or {})
            if isinstance(owner_review_payload.get("generation_receipt"), dict)
            else {}
        ),
    }
    system_assessment = owner_review_payload.get("system_assessment")
    item["system_assessment"] = system_assessment if isinstance(system_assessment, dict) else _build_owner_review_assessment(item)
    decision_scaffold = owner_review_payload.get("decision_scaffold")
    item["decision_scaffold"] = (
        decision_scaffold if isinstance(decision_scaffold, dict) else _build_owner_decision_scaffold(item, item["system_assessment"])
    )
    return item


def _parse_iso_datetime(value: Any) -> datetime | None:
    raw = str(value or "").strip()
    if not raw:
        return None
    if raw.endswith("Z"):
        raw = f"{raw[:-1]}+00:00"
    try:
        parsed = datetime.fromisoformat(raw)
    except ValueError:
        return None
    if parsed.tzinfo is None:
        parsed = parsed.replace(tzinfo=timezone.utc)
    return parsed.astimezone(timezone.utc)


def _owner_review_item_sort_key(item: dict[str, Any]) -> tuple[int, float | str]:
    created_at = _parse_iso_datetime(item.get("created_at"))
    if created_at is not None:
        return (1, created_at.timestamp())
    return (0, str(item.get("draft_path") or ""))


def _dedupe_owner_review_items(items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    selected: dict[str, dict[str, Any]] = {}
    for item in items:
        identity_key = _owner_review_identity_key_for_item(item) or str(item.get("queue_id") or "").strip()
        if not identity_key:
            continue
        current = selected.get(identity_key)
        if current is None or _owner_review_item_sort_key(item) > _owner_review_item_sort_key(current):
            selected[identity_key] = item
    return list(selected.values())


def _record_owner_review_override(
    overrides: dict[str, dict[str, Any]],
    *,
    keys: list[str],
    decision: str,
    notes: str | None,
    reviewed_at: Any,
    sort_key: datetime,
) -> None:
    entry = {
        "decision": decision,
        "notes": notes,
        "reviewed_at": reviewed_at,
        "_sort_key": sort_key,
    }
    for key in keys:
        normalized_key = str(key or "").strip()
        if not normalized_key:
            continue
        current = overrides.get(normalized_key)
        if current is not None and sort_key <= current["_sort_key"]:
            continue
        overrides[normalized_key] = entry


def _list_pm_active_owner_review_decisions() -> dict[str, dict[str, Any]]:
    decisions: dict[str, dict[str, Any]] = {}
    try:
        cards = pm_card_service.list_cards(limit=250)
    except Exception:
        return decisions
    for card in cards:
        if not _is_owner_review_source(card.source):
            continue
        if str(card.link_type or "") != OWNER_REVIEW_LINK_TYPE:
            continue
        if str(card.status or "").lower() not in ACTIVE_OWNER_REVIEW_CARD_STATUSES:
            continue
        payload = dict(card.payload or {})
        owner_review_payload = payload.get("owner_review") if isinstance(payload.get("owner_review"), dict) else {}
        if not isinstance(owner_review_payload, dict):
            continue
        queue_id = str(owner_review_payload.get("queue_id") or "").strip()
        identity_key = _owner_review_identity_key_from_payload(owner_review_payload)
        decision = str(owner_review_payload.get("decision") or "").strip().lower()
        if decision not in STATUS_MAP or (not queue_id and not identity_key):
            continue
        reviewed_at = _parse_iso_datetime(owner_review_payload.get("reviewed_at"))
        card_updated_at = getattr(card, "updated_at", None)
        sort_key = reviewed_at or (card_updated_at if isinstance(card_updated_at, datetime) else None) or datetime.min.replace(tzinfo=timezone.utc)
        _record_owner_review_override(
            decisions,
            keys=[identity_key, queue_id],
            decision=decision,
            notes=str(owner_review_payload.get("notes") or "").strip() or None,
            reviewed_at=owner_review_payload.get("reviewed_at"),
            sort_key=sort_key,
        )
    return decisions


def _apply_pm_owner_review_decision_overrides(items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    overrides = _list_pm_active_owner_review_decisions()
    if not overrides:
        return items
    updated_items: list[dict[str, Any]] = []
    for item in items:
        queue_id = str(item.get("queue_id") or "").strip()
        identity_key = _owner_review_identity_key_for_item(item)
        override = overrides.get(identity_key) or overrides.get(queue_id)
        if override is None:
            updated_items.append(item)
            continue
        decision = str(override.get("decision") or "").strip().lower()
        updated = dict(item)
        updated["current_decision"] = decision
        updated["approval_status"] = APPROVAL_STATUS_MAP.get(decision, updated.get("approval_status"))
        updated["publish_posture"] = PUBLISH_POSTURE_MAP.get(decision, updated.get("publish_posture"))
        updated["status"] = STATUS_MAP.get(decision, updated.get("status"))
        if override.get("notes"):
            updated["current_notes"] = override.get("notes")
        if override.get("reviewed_at"):
            updated["reviewed_at"] = override.get("reviewed_at")
        updated_items.append(updated)
    return updated_items


def _list_pm_pending_owner_review_items() -> list[dict[str, Any]]:
    items: list[dict[str, Any]] = []
    try:
        cards = pm_card_service.list_cards(limit=250)
    except Exception:
        return items
    for card in cards:
        if not _is_owner_review_source(card.source):
            continue
        if str(card.link_type or "") != OWNER_REVIEW_LINK_TYPE:
            continue
        if str(card.status or "").lower() not in {"review", "todo", "in_progress"}:
            continue
        payload = dict(card.payload or {})
        owner_review_payload = payload.get("owner_review") if isinstance(payload.get("owner_review"), dict) else {}
        if not isinstance(owner_review_payload, dict):
            continue
        if str(owner_review_payload.get("sync_state") or "").strip() != "pending_owner_review":
            continue
        if str(owner_review_payload.get("decision") or "").strip():
            continue
        item = _item_from_owner_review_payload(owner_review_payload)
        if item is not None and not _is_development_only_owner_review_item(item):
            items.append(item)
    return items


def _find_pm_pending_owner_review_item(queue_id: str) -> dict[str, Any] | None:
    normalized_queue_id = str(queue_id or "").strip()
    if not normalized_queue_id:
        return None
    return next(
        (item for item in _list_pm_pending_owner_review_items() if str(item.get("queue_id") or "").strip() == normalized_queue_id),
        None,
    )


def _is_missing_owner_review_artifact_error(exc: ValueError) -> bool:
    message = str(exc)
    return (
        "Draft file is missing" in message
        or "does not have a draft path" in message
        or "is not present in" in message
    )


def _is_supplemental_owner_review_item(item: dict[str, Any]) -> bool:
    return str(item.get("entry_kind") or "").strip() == "supplemental"


def _is_generated_owner_review_item(item: dict[str, Any]) -> bool:
    return (
        str(item.get("entry_kind") or "").strip() == "generated"
        and str(item.get("source_kind") or "").strip() == GENERATED_OWNER_REVIEW_SOURCE_KIND
    )


def _is_development_only_owner_review_item(item: dict[str, Any]) -> bool:
    if str(item.get("publish_posture") or "").strip().lower() == "development_only":
        return True
    if str(item.get("draft_kind") or "").strip().lower() == "development_brief":
        return True
    draft_path = _resolve_current_owner_review_draft(
        _linkedin_root(),
        item.get("draft_path"),
    )
    if draft_path is not None:
        frontmatter, _ = _parse_frontmatter(_read_text(draft_path))
        if (
            str(frontmatter.get("publish_posture") or "").strip().lower() == "development_only"
            or str(frontmatter.get("draft_kind") or "").strip().lower() == "development_brief"
            or str(frontmatter.get("promotion_state") or "").strip().lower()
            == "needs_generation_and_critic"
        ):
            return True
    receipt = item.get("generation_receipt")
    classification = (
        receipt.get("candidate_classification")
        if isinstance(receipt, dict) and isinstance(receipt.get("candidate_classification"), dict)
        else {}
    )
    return str(classification.get("publish_posture") or "").strip().lower() == "development_only"


def _selected_option_quality_receipt(
    *,
    option_index: int,
    quality_gate: dict[str, Any],
) -> dict[str, Any]:
    """Project one deterministic option without turning sibling failure into a veto."""

    selected_option_index = option_index + 1
    schema_version = str(quality_gate.get("schema_version") or "").strip()
    base = {
        "schema_version": SELECTED_OPTION_QUALITY_VERSION,
        "selected_option_index": selected_option_index,
        "source_quality_gate_schema_version": schema_version or "legacy_aggregate",
    }
    if not schema_version:
        legacy_passed = quality_gate.get("passed") is True
        legacy_failures = [
            str(item).strip()
            for item in (quality_gate.get("failed_reasons") or [])
            if str(item).strip()
        ]
        return {
            **base,
            "passed": legacy_passed,
            "shared_constraints_passed": legacy_passed,
            "selected_option_passed": legacy_passed,
            "batch_all_options_passed": legacy_passed,
            "selection_admission_passed": legacy_passed,
            "score": None,
            "threshold": None,
            "failed_reasons": [] if legacy_passed else legacy_failures or ["legacy_aggregate_quality_gate_failed"],
        }

    invalid_reasons: list[str] = []
    if schema_version != DETERMINISTIC_QUALITY_GATE_VERSION:
        invalid_reasons.append("unsupported_quality_gate_schema")

    def _bounded_int(value: Any, *, minimum: int, maximum: int) -> int | None:
        if (
            isinstance(value, bool)
            or not isinstance(value, int)
            or value < minimum
            or value > maximum
        ):
            return None
        return value

    def _reasons(value: Any) -> list[str] | None:
        if not isinstance(value, list):
            return None
        if any(not isinstance(item, str) for item in value):
            return None
        parsed = [item.strip() for item in value]
        if any(not item for item in parsed) or len(set(parsed)) != len(parsed):
            return None
        return parsed

    required_count = _bounded_int(
        quality_gate.get("required_option_count"),
        minimum=1,
        maximum=10,
    )
    evaluated_count = _bounded_int(
        quality_gate.get("evaluated_option_count"),
        minimum=0,
        maximum=10,
    )
    if required_count is None or evaluated_count is None or evaluated_count != required_count:
        invalid_reasons.append("option_count_coverage_invalid")

    shared = quality_gate.get("shared_constraints") if isinstance(quality_gate.get("shared_constraints"), dict) else {}
    shared_passed = shared.get("passed") is True
    shared_failures = _reasons(shared.get("failed_reasons"))
    if shared_failures is None:
        invalid_reasons.append("shared_failed_reasons_invalid")
        shared_failures = []
    if (
        not isinstance(shared.get("passed"), bool)
        or shared_passed != (not shared_failures and evaluated_count == required_count)
        or shared.get("required_option_count") != required_count
        or shared.get("evaluated_option_count") != evaluated_count
    ):
        invalid_reasons.append("shared_constraints_invalid")

    option_results: dict[int, dict[str, Any]] = {}
    raw_results = quality_gate.get("option_results")
    if not isinstance(raw_results, list):
        invalid_reasons.append("option_results_missing")
    else:
        for raw_result in raw_results:
            if not isinstance(raw_result, dict):
                invalid_reasons.append("option_result_invalid")
                continue
            result_index = _bounded_int(raw_result.get("option_index"), minimum=1, maximum=10)
            score = _bounded_int(raw_result.get("score"), minimum=0, maximum=100)
            threshold = _bounded_int(raw_result.get("threshold"), minimum=1, maximum=100)
            failures = _reasons(raw_result.get("failed_reasons"))
            result_passed = raw_result.get("passed")
            if result_index is None or result_index in option_results:
                invalid_reasons.append("option_result_index_invalid_or_duplicate")
                continue
            if score is None or threshold is None or failures is None or not isinstance(result_passed, bool):
                invalid_reasons.append(f"option_{result_index}_receipt_invalid")
                failures = failures or []
                result_passed = False
            expected_passed = (
                score is not None
                and threshold is not None
                and score >= threshold
                and not failures
            )
            if isinstance(raw_result.get("passed"), bool) and result_passed != expected_passed:
                invalid_reasons.append(f"option_{result_index}_passed_inconsistent")
            option_results[result_index] = {
                "passed": bool(result_passed),
                "score": int(score or 0),
                "threshold": int(threshold or 0),
                "failed_reasons": failures,
            }

    expected_indices = set(range(1, (required_count or 0) + 1))
    if set(option_results) != expected_indices:
        invalid_reasons.append("option_result_coverage_invalid")
    selected = option_results.get(selected_option_index) or {}
    selected_passed = selected.get("passed") is True
    computed_batch_passed = shared_passed and bool(option_results) and all(
        result["passed"] for result in option_results.values()
    )
    computed_selection_passed = shared_passed and any(
        result["passed"] for result in option_results.values()
    )
    if (
        not isinstance(quality_gate.get("passed"), bool)
        or quality_gate.get("passed") != computed_batch_passed
        or not isinstance(quality_gate.get("selection_admission_passed"), bool)
        or quality_gate.get("selection_admission_passed") != computed_selection_passed
    ):
        invalid_reasons.append("quality_gate_aggregate_invalid")
    aggregate_failures = _reasons(quality_gate.get("failed_reasons"))
    expected_aggregate_failures = list(
        dict.fromkeys(
            [
                f"option_{index}_{reason}"
                for index in sorted(option_results)
                for reason in option_results[index]["failed_reasons"]
            ]
            + shared_failures
        )
    )
    if aggregate_failures != expected_aggregate_failures:
        invalid_reasons.append("aggregate_failed_reasons_inconsistent")
    contamination = quality_gate.get("voice_exemplar_contamination")
    if isinstance(contamination, dict) and contamination.get("passed") is not True and shared_passed:
        invalid_reasons.append("contamination_not_globally_blocking")

    valid = not invalid_reasons
    selected_failures = [
        str(item)
        for item in (selected.get("failed_reasons") or [])
        if str(item)
    ]
    failed_reasons = (
        list(dict.fromkeys(shared_failures + selected_failures))
        if valid
        else ["quality_gate_receipt_invalid"]
    )
    return {
        **base,
        "passed": valid and shared_passed and selected_passed,
        "shared_constraints_passed": valid and shared_passed,
        "selected_option_passed": valid and selected_passed,
        "batch_all_options_passed": valid and computed_batch_passed,
        "selection_admission_passed": valid and computed_selection_passed,
        "score": selected.get("score") if valid else None,
        "threshold": selected.get("threshold") if valid else None,
        "failed_reasons": failed_reasons,
    }


def _require_generated_approval_readiness(item: dict[str, Any]) -> None:
    """Fail closed unless a generated exact copy still has a verified critic receipt."""

    if not _is_generated_owner_review_item(item):
        return
    receipt = item.get("generation_receipt")
    if not isinstance(receipt, dict) or receipt.get("schema_version") != "feezie_generation_receipt/v1":
        raise LinkedinOwnerReviewConflictError(
            "Generated draft approval is blocked because its generation receipt is missing or invalid."
        )

    quality_gate = receipt.get("quality_gate") if isinstance(receipt.get("quality_gate"), dict) else {}
    readiness = (
        receipt.get("editorial_readiness")
        if isinstance(receipt.get("editorial_readiness"), dict)
        else {}
    )
    critic_verification = (
        receipt.get("critic_verification")
        if isinstance(receipt.get("critic_verification"), dict)
        else {}
    )
    option_review = receipt.get("option_review") if isinstance(receipt.get("option_review"), dict) else {}
    selected_option_quality = (
        receipt.get("selected_option_quality")
        if isinstance(receipt.get("selected_option_quality"), dict)
        else None
    )
    revision_contract = (
        receipt.get("revision_contract")
        if isinstance(receipt.get("revision_contract"), dict)
        else None
    )
    revision_verification = (
        receipt.get("revision_verification")
        if isinstance(receipt.get("revision_verification"), dict)
        else None
    )
    blocking_reasons: list[str] = []
    exact_copy = str(item.get("first_pass_draft") or "")
    receipt_digest = str(receipt.get("content_version_sha256") or "").strip().lower()
    if (
        re.fullmatch(r"[a-f0-9]{64}", receipt_digest) is None
        or not exact_copy.strip()
        or linkedin_content_version_sha256(exact_copy) != receipt_digest
    ):
        blocking_reasons.append("selected_content_digest_not_verified")
    if revision_contract is not None:
        expected_revision_contract = {
            "schema_version": FEEZIE_REVISION_CONTRACT_VERSION,
            "enabled": True,
            "trigger": "non_ready_after_initial_blind_critic",
            "revision_calls_per_non_ready_option": 1,
            "model_retries_per_revision": 0,
            "preserve_ready_sibling_exactly": True,
            "fresh_blind_critic_required_after_revision": True,
        }
        expected_revision_keys = {
            "schema_version",
            "source_contract_schema_version",
            "source_receipt_schema_version",
            "status",
            "failure_code",
            "selected_option_index",
            "action",
            "attempt_count",
            "original_post_sha256",
            "final_post_sha256",
            "changed",
            "error_code",
            "final_pair_sha256",
            "initial_critic_receipt_sha256",
            "final_critic_receipt_sha256",
            "final_critic_call_count",
            "selected_copy_sha256",
            "contains_sibling_option_rows",
            "contains_post_copy",
        }

        def valid_sha256(value: Any) -> bool:
            return re.fullmatch(r"[a-f0-9]{64}", str(value or "").strip().lower()) is not None

        stored_zero_based_index = item.get("generation_option_index")
        expected_selected_index = (
            stored_zero_based_index + 1
            if isinstance(stored_zero_based_index, int) and not isinstance(stored_zero_based_index, bool)
            else None
        )
        revision_valid = (
            revision_contract == expected_revision_contract
            and isinstance(revision_verification, dict)
            and set(revision_verification) == expected_revision_keys
            and revision_verification.get("schema_version")
            == SELECTED_REVISION_VERIFICATION_VERSION
            and revision_verification.get("source_contract_schema_version")
            == FEEZIE_REVISION_CONTRACT_VERSION
            and revision_verification.get("source_receipt_schema_version")
            == FEEZIE_REVISION_RECEIPT_VERSION
            and revision_verification.get("selected_option_index") == expected_selected_index
            and revision_verification.get("selected_option_index")
            == receipt.get("selected_option_index")
            and revision_verification.get("contains_sibling_option_rows") is False
            and revision_verification.get("contains_post_copy") is False
            and not str(revision_verification.get("failure_code") or "")
            and not str(revision_verification.get("error_code") or "")
            and valid_sha256(revision_verification.get("original_post_sha256"))
            and valid_sha256(revision_verification.get("final_post_sha256"))
            and valid_sha256(revision_verification.get("selected_copy_sha256"))
            and valid_sha256(revision_verification.get("final_pair_sha256"))
            and valid_sha256(revision_verification.get("initial_critic_receipt_sha256"))
            and valid_sha256(revision_verification.get("final_critic_receipt_sha256"))
            and revision_verification.get("selected_copy_sha256")
            == _revision_post_sha256(exact_copy)
            and revision_verification.get("final_post_sha256")
            == revision_verification.get("selected_copy_sha256")
        )
        revision_status = str((revision_verification or {}).get("status") or "").strip().lower()
        revision_action = str((revision_verification or {}).get("action") or "").strip().lower()
        if revision_status == "not_required":
            revision_valid = bool(
                revision_valid
                and revision_action == "preserved"
                and revision_verification.get("attempt_count") == 0
                and revision_verification.get("changed") is False
                and revision_verification.get("original_post_sha256")
                == revision_verification.get("final_post_sha256")
                and revision_verification.get("final_critic_call_count") == 0
                and revision_verification.get("initial_critic_receipt_sha256")
                == revision_verification.get("final_critic_receipt_sha256")
            )
        elif revision_status == "completed":
            selected_action_valid = (
                revision_action == "preserved"
                and revision_verification.get("attempt_count") == 0
                and revision_verification.get("changed") is False
                and revision_verification.get("original_post_sha256")
                == revision_verification.get("final_post_sha256")
            ) or (
                revision_action == "revised"
                and revision_verification.get("attempt_count") == 1
                and revision_verification.get("changed") is True
                and revision_verification.get("original_post_sha256")
                != revision_verification.get("final_post_sha256")
            )
            revision_valid = bool(
                revision_valid
                and selected_action_valid
                and revision_verification.get("final_critic_call_count") == 1
                and revision_verification.get("initial_critic_receipt_sha256")
                != revision_verification.get("final_critic_receipt_sha256")
            )
        else:
            revision_valid = False
        if not revision_valid:
            blocking_reasons.append("revision_lineage_not_verified")
    elif revision_verification is not None:
        blocking_reasons.append("revision_lineage_not_verified")
    classification = (
        receipt.get("candidate_classification")
        if isinstance(receipt.get("candidate_classification"), dict)
        else {}
    )
    employer_safety = str(classification.get("employer_safety") or "").strip().lower()
    if employer_safety == "blocked":
        blocking_reasons.append("employer_safety_blocked")
    elif employer_safety not in APPROVABLE_EMPLOYER_SAFETY:
        blocking_reasons.append("employer_safety_not_verified")
    v2_selected_contract = False
    if selected_option_quality is None:
        # Durable legacy receipts predate per-option deterministic admission.
        # They remain safe only under their original all-options contract.
        if str(quality_gate.get("schema_version") or "").strip():
            blocking_reasons.append("selected_option_quality_gate_not_verified")
        elif quality_gate.get("passed") is not True:
            blocking_reasons.append("deterministic_quality_gate_not_verified")
    else:
        selected_index = selected_option_quality.get("selected_option_index")
        generation_receipt_index = receipt.get("selected_option_index")
        option_review_index = option_review.get("option_index")
        stored_zero_based_index = item.get("generation_option_index")
        expected_selected_index = (
            stored_zero_based_index + 1
            if isinstance(stored_zero_based_index, int) and not isinstance(stored_zero_based_index, bool)
            else None
        )
        source_schema = str(
            selected_option_quality.get("source_quality_gate_schema_version") or ""
        ).strip()
        v2_selected_contract = source_schema == DETERMINISTIC_QUALITY_GATE_VERSION
        selected_failures = selected_option_quality.get("failed_reasons")
        selected_quality_valid = (
            selected_option_quality.get("schema_version") == SELECTED_OPTION_QUALITY_VERSION
            and isinstance(selected_index, int)
            and not isinstance(selected_index, bool)
            and selected_index >= 1
            and isinstance(generation_receipt_index, int)
            and not isinstance(generation_receipt_index, bool)
            and generation_receipt_index == selected_index
            and expected_selected_index == selected_index
            and isinstance(option_review_index, int)
            and not isinstance(option_review_index, bool)
            and option_review_index == selected_index
            and selected_option_quality.get("passed") is True
            and selected_option_quality.get("shared_constraints_passed") is True
            and selected_option_quality.get("selected_option_passed") is True
            and selected_option_quality.get("selection_admission_passed") is True
            and isinstance(selected_option_quality.get("batch_all_options_passed"), bool)
            and selected_failures == []
        )
        if source_schema == DETERMINISTIC_QUALITY_GATE_VERSION:
            score = selected_option_quality.get("score")
            threshold = selected_option_quality.get("threshold")
            shared_projection = (
                quality_gate.get("shared_constraints")
                if isinstance(quality_gate.get("shared_constraints"), dict)
                else {}
            )
            selected_quality_valid = (
                selected_quality_valid
                and quality_gate.get("schema_version") == source_schema
                and isinstance(quality_gate.get("passed"), bool)
                and quality_gate.get("passed")
                == selected_option_quality.get("batch_all_options_passed")
                and isinstance(
                    quality_gate.get("selection_admission_passed"),
                    bool,
                )
                and quality_gate.get("selection_admission_passed")
                == selected_option_quality.get("selection_admission_passed")
                and set(shared_projection)
                == {
                    "passed",
                    "failed_reasons",
                    "required_option_count",
                    "evaluated_option_count",
                }
                and shared_projection.get("passed") is True
                and shared_projection.get("passed")
                == selected_option_quality.get("shared_constraints_passed")
                and shared_projection.get("failed_reasons") == []
                and shared_projection.get("required_option_count")
                == quality_gate.get("required_option_count")
                and shared_projection.get("evaluated_option_count")
                == quality_gate.get("evaluated_option_count")
                and isinstance(score, int)
                and not isinstance(score, bool)
                and isinstance(threshold, int)
                and not isinstance(threshold, bool)
                and 0 <= score <= 100
                and 1 <= threshold <= 100
                and score >= threshold
                and option_review.get("deterministic_quality_passed") is True
                and option_review.get("deterministic_blocked") is False
            )
        elif source_schema == "legacy_aggregate":
            selected_quality_valid = (
                selected_quality_valid
                and not str(quality_gate.get("schema_version") or "").strip()
                and selected_option_quality.get("batch_all_options_passed") is True
                and quality_gate.get("passed") is True
            )
        else:
            selected_quality_valid = False
        if not selected_quality_valid:
            blocking_reasons.append("selected_option_quality_gate_not_verified")
    if v2_selected_contract:
        # V2 separates strict batch truth from selected-option admission. A
        # failed sibling may remain in aggregate diagnostics, but cannot veto a
        # selected option that independently passed the shared and critic gates.
        if str(readiness.get("critic_status") or "").strip().lower() != "completed":
            blocking_reasons.append("editorial_readiness_not_verified")
    elif (
        readiness.get("ready") is not True
        or str(readiness.get("status") or "").strip().lower() != "ready"
        or str(readiness.get("critic_status") or "").strip().lower() != "completed"
        or readiness.get("deterministic_quality_gate_passed") is not True
        or readiness.get("blocking_reasons") not in (None, [])
    ):
        # Schema-less legacy receipts stay all-options-or-none and fail closed.
        blocking_reasons.append("editorial_readiness_not_verified")
    selected_critic_blocked = (
        option_review.get("editorially_ready") is not True
        or str(option_review.get("verdict") or "").strip().lower() != "ready"
        or (
            v2_selected_contract
            and (
                option_review.get("deterministic_quality_passed") is not True
                or option_review.get("deterministic_blocked") is not False
            )
        )
    )
    if selected_critic_blocked:
        blocking_reasons.append("selected_option_not_critic_ready")
    if (
        str(critic_verification.get("status") or "").strip().lower() != "completed"
        or critic_verification.get("semantic_distinctness_passed") is not True
        or critic_verification.get("blind_review_receipt_present") is not True
    ):
        blocking_reasons.append("independent_critic_verification_missing")

    dimensions = option_review.get("dimension_scores")
    if (
        not isinstance(dimensions, dict)
        or set(dimensions) != GENERATED_CRITIC_DIMENSIONS
        or any(
            isinstance(value, bool) or not isinstance(value, int) or value < 1 or value > 10
            for value in dimensions.values()
        )
    ):
        blocking_reasons.append("five_dimension_critic_receipt_missing")
    hooks = [str(hook).strip() for hook in (option_review.get("hook_variants") or []) if str(hook).strip()]
    if len(hooks) != GENERATED_HOOK_VARIANT_COUNT or len({hook.lower() for hook in hooks}) != len(hooks):
        blocking_reasons.append("eight_hook_critic_receipt_missing")

    strategy = receipt.get("strategy_contract") if isinstance(receipt.get("strategy_contract"), dict) else {}
    generated_hash = str(strategy.get("contract_hash") or "").strip().lower()
    try:
        current_hash = str(load_feezie_strategy_contract().get("contract_hash") or "").strip().lower()
    except Exception as exc:
        raise LinkedinOwnerReviewConflictError(
            "Generated draft approval is blocked because the current FEEZIE strategy contract cannot be verified."
        ) from exc
    if not generated_hash or generated_hash != current_hash:
        blocking_reasons.append("strategy_contract_changed_since_generation")

    if blocking_reasons:
        raise LinkedinOwnerReviewConflictError(
            "Generated draft approval is blocked until writer/critic readiness is re-verified: "
            + ", ".join(dict.fromkeys(blocking_reasons))
            + "."
        )


def _owner_review_reason(item: dict[str, Any], decision: str) -> str:
    queue_id = str(item.get("queue_id") or "").strip()
    title = str(item.get("title") or queue_id).strip()
    if _is_supplemental_owner_review_item(item):
        if decision == "approve":
            return (
                f"Owner approved the latent transform draft {title}. Promote it into the next concrete FEEZIE step, "
                "then write the result back to PM and the draft artifacts."
            )
        return (
            f"Owner requested revision for the latent transform draft {title}. Apply the owner notes, update the draft, "
            "and return it to owner review."
        )
    if _is_generated_owner_review_item(item):
        if decision == "approve":
            return (
                f"Owner approved the generated FEEZIE draft {queue_id} ({title}). Package the approved copy for the next "
                "explicit release step, preserve its PM provenance, and do not publish automatically."
            )
        return (
            f"Owner requested revision for the generated FEEZIE draft {queue_id} ({title}). Revise the PM-backed first-pass "
            "draft from the owner notes and return it to owner review."
        )
    if decision == "approve":
        return (
            f"Owner approved {queue_id} ({title}). Convert the approved draft into the next concrete release step, "
            "then write the result back to PM and the FEEZIE artifacts."
        )
    return (
        f"Owner requested revision for {queue_id} ({title}). Apply the owner notes, update the draft artifacts, "
        "and return the draft to owner review."
    )


def _owner_review_card_title(item: dict[str, Any], decision: str) -> str:
    queue_id = str(item.get("queue_id") or "").strip()
    title = str(item.get("title") or queue_id).strip()
    if _is_supplemental_owner_review_item(item):
        if decision == "approve":
            return f"Promote approved latent draft - {title}"
        return f"Revise latent draft - {title}"
    if _is_generated_owner_review_item(item):
        if decision == "approve":
            return f"Package approved generated FEEZIE draft - {queue_id}"
        return f"Revise generated FEEZIE draft - {queue_id}"
    if decision == "approve":
        return f"Schedule approved FEEZIE draft - {queue_id}"
    return f"Revise FEEZIE draft - {queue_id}"


def _owner_review_instructions(item: dict[str, Any], decision: str, notes: str) -> list[str]:
    queue_id = str(item.get("queue_id") or "").strip()
    title = str(item.get("title") or queue_id).strip()
    draft_path = str(item.get("draft_path") or "").strip()
    packet_path = str(item.get("owner_packet_path") or "").strip()
    if _is_generated_owner_review_item(item):
        base = [
            f"Use the PM-backed owner_review payload for `{queue_id}` / {title} as the source of truth.",
            "Read owner_review.first_pass_draft, the source-card provenance, and the owner notes before changing execution state.",
        ]
    else:
        base = [
            f"Use `{queue_id}` / {title} as the source of truth for this follow-up.",
            f"Review the current draft at `{draft_path}` and the owner packet at `{packet_path or 'missing owner packet'}` before changing execution state.",
        ]
    if _is_supplemental_owner_review_item(item):
        if decision == "approve":
            base.extend(
                [
                    "Turn the approved latent transform into the next concrete FEEZIE move: promote it into the active publishing plan, package it for scheduling, or explicitly place it into the canonical queue if it now deserves that lane.",
                    "Write the follow-up outcome back to PM and the draft artifacts so the approval loop closes visibly.",
                    "If repo-side work is complete and the only remaining step needs host credentials or an external UI, return explicit host actions with proof requirements instead of leaving a generic follow-up.",
                ]
            )
        else:
            base.extend(
                [
                    "Revise the latent transform draft directly in the FEEZIE workspace and keep the copy grounded in proof and audience consequence.",
                    "Return the draft to owner review once the transform is specific enough to graduate out of the latent lane.",
                ]
            )
    elif _is_generated_owner_review_item(item):
        if decision == "approve":
            base.extend(
                [
                    "Package the approved copy into the next scheduling-ready or release-ready artifact, but do not publish automatically.",
                    "Return any external scheduling or publishing step as an explicit owner/host action with proof requirements.",
                    "Keep the completed job id, option index, and source-card provenance attached to the PM result.",
                ]
            )
        else:
            base.extend(
                [
                    "Revise the PM-backed first-pass draft directly from the owner notes and preserve its source grounding.",
                    "Return the revised copy to owner review; do not schedule or publish it.",
                ]
            )
    elif decision == "approve":
        base.extend(
            [
                "Turn the approved draft into the next concrete execution step: scheduling, packaging, or explicit publish follow-through.",
                "Write the follow-up outcome back to PM, the draft queue, and the execution log so the approval loop closes visibly.",
                "If the remaining work is host-only, such as LinkedIn scheduling or proof capture outside the repo, return it as explicit host actions with proof requirements.",
            ]
        )
    else:
        base.extend(
            [
                "Revise the draft directly in the FEEZIE workspace and keep the copy grounded in proof.",
                "Return the draft to owner review with updated notes once the requested changes are complete.",
            ]
        )
    if notes.strip():
        base.append(f"Owner notes: {notes.strip()}")
    return base


def _owner_review_acceptance_criteria(item: dict[str, Any], decision: str) -> list[str]:
    queue_id = str(item.get("queue_id") or "").strip()
    title = str(item.get("title") or queue_id).strip()
    if _is_supplemental_owner_review_item(item):
        if decision == "approve":
            return [
                f"`{title}` is promoted out of the latent lane into a concrete next FEEZIE step.",
                "PM write-back makes the promotion outcome explicit and bounded.",
                "Draft or queue artifacts reflect the new active state.",
            ]
        return [
            f"`{title}` is revised directly in the latent draft artifact.",
            "The revised draft is returned to owner review with concrete notes or proof added.",
            "PM write-back explains what changed and what the owner should re-check.",
        ]
    if _is_generated_owner_review_item(item):
        if decision == "approve":
            return [
                f"`{queue_id}` is packaged into a concrete release-ready next step without automatic publication.",
                "PM write-back preserves the completed job id, option index, selected draft, and source provenance.",
                "Any external publish or scheduling action remains explicit and proof-gated.",
            ]
        return [
            f"`{queue_id}` is revised from the PM-backed first-pass draft and owner notes.",
            "The revised copy is returned to owner review instead of being scheduled or published.",
            "PM write-back explains what changed and what the owner should re-check.",
        ]
    if decision == "approve":
        return [
            f"`{queue_id}` advances from approved draft into a concrete release or scheduling step.",
            "PM write-back records a bounded result with outcomes and any follow-up actions.",
            "The FEEZIE draft artifacts reflect the new post-approval state.",
        ]
    return [
        f"`{queue_id}` is revised directly in the FEEZIE draft artifacts.",
        "The revised draft is returned to owner review instead of stalling in execution.",
        "PM write-back explains what changed and what the owner should re-check.",
    ]


def _owner_review_artifacts_expected(item: dict[str, Any], decision: str) -> list[str]:
    draft_path = str(item.get("draft_path") or "").strip()
    queue_id = str(item.get("queue_id") or "").strip()
    if _is_generated_owner_review_item(item):
        if decision == "approve":
            return [
                "updated PM execution result preserving the selected generated draft and provenance",
                f"release-ready package or explicit host action for {queue_id}",
                "proof that no automatic publishing action was taken",
            ]
        return [
            "updated PM execution result containing the revised generated draft",
            "owner-review-ready revision notes",
            f"preserved generation provenance for {queue_id}",
        ]
    if decision == "approve":
        return [
            draft_path or f"updated draft artifact for {queue_id}",
            "updated PM execution result",
            "bounded scheduling, publishing, or queue artifact showing the next concrete step",
        ]
    return [
        draft_path or f"updated draft artifact for {queue_id}",
        "updated PM execution result",
        "owner-review-ready revision notes in the draft or packet artifacts",
    ]


def _build_owner_review_execution(reason: str) -> dict[str, Any]:
    defaults = pm_card_service.execution_defaults_for_workspace(PM_WORKSPACE_KEY)
    return {
        "lane": "codex",
        "state": "ready",
        "manager_agent": defaults["manager_agent"],
        "target_agent": defaults["target_agent"],
        "workspace_agent": defaults.get("workspace_agent"),
        "execution_mode": defaults["execution_mode"],
        "requested_by": "Neo",
        "assigned_runner": "jean-claude" if str(defaults["execution_mode"]) == "direct" else "codex",
        "reason": reason,
        "last_transition_at": datetime.now(timezone.utc).isoformat(),
        "source": "owner_review",
    }


def _build_owner_review_card_payload(
    item: dict[str, Any],
    *,
    decision: str,
    notes: str,
    draft_rel_path: str,
    packet_rel_path: str | None,
    reviewed_at: str | None = None,
    existing_payload: dict[str, Any] | None = None,
) -> dict[str, Any]:
    queue_id = str(item.get("queue_id") or "").strip()
    title = str(item.get("title") or queue_id).strip()
    identity_key = _owner_review_identity_key_for_item(item) or queue_id
    reason = _owner_review_reason(item, decision)
    payload = dict(existing_payload or {})
    artifact_paths = [path for path in [draft_rel_path, packet_rel_path] if path]
    contract = build_execution_contract(
        title=_owner_review_card_title(item, decision),
        workspace_key=PM_WORKSPACE_KEY,
        source="owner_review_followup",
        reason=reason,
        instructions=_owner_review_instructions(item, decision, notes),
        acceptance_criteria=_owner_review_acceptance_criteria(item, decision),
        artifacts_expected=_owner_review_artifacts_expected(item, decision),
    )
    payload.update(
        {
            "workspace_key": PM_WORKSPACE_KEY,
            "legacy_owner_review_compatibility": True,
            "source_agent": "Neo",
            "front_door_agent": "Neo",
            "trigger_origin": "owner_review",
            "trigger_key": _owner_review_trigger_key(identity_key),
            "reason": reason,
            "instructions": contract["instructions"],
            "acceptance_criteria": contract["acceptance_criteria"],
            "artifacts_expected": contract["artifacts_expected"],
            "completion_contract": contract["completion_contract"],
            "artifact_paths": artifact_paths,
            "owner_review": {
                "queue_id": queue_id,
                "identity_key": identity_key,
                "title": title,
                "decision": decision,
                "notes": notes.strip() or None,
                "lane": item.get("lane"),
                "format": item.get("format"),
                "core_angle": item.get("core_angle"),
                "why_now": item.get("why_now"),
                "status": STATUS_MAP.get(decision),
                "approval_status": APPROVAL_STATUS_MAP.get(decision),
                "draft_path": draft_rel_path,
                "owner_packet_path": packet_rel_path,
                "proof_anchors": item.get("proof_anchors") or [],
                "first_pass_draft": item.get("first_pass_draft"),
                "draft_owner_notes": item.get("draft_owner_notes") or [],
                "packet_recommendation": item.get("packet_recommendation"),
                "publish_posture": PUBLISH_POSTURE_MAP.get(decision),
                "reviewed_at": reviewed_at or datetime.now(timezone.utc).isoformat(),
                "created_at": item.get("created_at"),
                "sync_state": "owner_decision_recorded",
                "entry_kind": item.get("entry_kind"),
                "source_kind": item.get("source_kind"),
                "source_url": item.get("source_url"),
                "source_path": item.get("source_path"),
                "idea_id": item.get("idea_id"),
                "generated_by": item.get("generated_by"),
                "generation_job_id": item.get("generation_job_id"),
                "generation_option_index": item.get("generation_option_index"),
                "source_card": dict(item.get("source_card") or {}) if isinstance(item.get("source_card"), dict) else {},
                "intent": item.get("intent"),
                "canonical_pillar": item.get("canonical_pillar"),
                "career_signal": item.get("career_signal"),
                "employer_proximity": item.get("employer_proximity"),
                "employer_safety": item.get("employer_safety"),
                "proof_posture": item.get("proof_posture"),
                "audience": item.get("audience"),
                "audience_consequence": item.get("audience_consequence"),
                "distinct_thesis": item.get("distinct_thesis"),
                "generation_receipt": dict(item.get("generation_receipt") or {}) if isinstance(item.get("generation_receipt"), dict) else {},
                "summary": item.get("summary"),
                "revision_goals": item.get("revision_goals") or [],
                "latent_reason": item.get("latent_reason"),
                "transform_type": item.get("transform_type"),
                "system_assessment": item.get("system_assessment"),
                "decision_scaffold": item.get("decision_scaffold"),
            },
            "execution": _build_owner_review_execution(reason),
        }
    )
    return payload


def _pending_owner_review_card_title(item: dict[str, Any]) -> str:
    queue_id = str(item.get("queue_id") or "").strip()
    title = str(item.get("title") or queue_id).strip()
    if _is_supplemental_owner_review_item(item):
        return f"Owner review - Latent transform - {title}"
    return f"Owner review - {queue_id} - {title}"


def _pending_owner_review_reason(item: dict[str, Any]) -> str:
    queue_id = str(item.get("queue_id") or "").strip()
    title = str(item.get("title") or queue_id).strip()
    if _is_supplemental_owner_review_item(item):
        return (
            f"Owner decision needed for the latent transform draft {title}. Open the card, review the transformed draft, "
            "and choose approve, revise, or park."
        )
    return (
        f"Owner decision needed for {queue_id} ({title}). Open the card, review the draft, "
        "and choose approve, revise, or park."
    )


def _item_needs_owner_review(item: dict[str, Any]) -> bool:
    return (
        str(item.get("approval_status") or "").strip() == "owner_review_required"
        and str(item.get("publish_posture") or "").strip() == "owner_review_required"
        and not str(item.get("current_decision") or "").strip()
    )


def _build_pending_owner_review_card_payload(
    item: dict[str, Any],
    *,
    existing_payload: dict[str, Any] | None = None,
) -> dict[str, Any]:
    queue_id = str(item.get("queue_id") or "").strip()
    title = str(item.get("title") or queue_id).strip()
    identity_key = _owner_review_identity_key_for_item(item) or queue_id
    payload = dict(existing_payload or {})
    payload.pop("execution", None)
    payload.pop("latest_execution_result", None)
    payload.pop("latest_manual_review", None)
    payload.update(
        {
            "workspace_key": PM_WORKSPACE_KEY,
            "legacy_owner_review_compatibility": True,
            "source_agent": "Neo",
            "front_door_agent": "Neo",
            "trigger_origin": "owner_review",
            "trigger_key": _owner_review_trigger_key(identity_key),
            "reason": _pending_owner_review_reason(item),
            "instructions": [
                f"Review `{title}` in PM Board before any backend worker picks it up.",
                "Read the first-pass draft and attached context before deciding.",
                "Choose Approve, Revise, or Park from the PM card modal.",
            ],
            "owner_review": {
                "queue_id": queue_id,
                "identity_key": identity_key,
                "title": title,
                "lane": item.get("lane"),
                "format": item.get("format"),
                "core_angle": item.get("core_angle"),
                "why_now": item.get("why_now"),
                "status": item.get("status"),
                "approval_status": item.get("approval_status"),
                "draft_path": item.get("draft_path"),
                "owner_packet_path": item.get("owner_packet_path"),
                "proof_anchors": item.get("proof_anchors") or [],
                "first_pass_draft": item.get("first_pass_draft"),
                "draft_owner_notes": item.get("draft_owner_notes") or [],
                "packet_recommendation": item.get("packet_recommendation"),
                "current_notes": item.get("current_notes"),
                "publish_posture": item.get("publish_posture"),
                "reviewed_at": item.get("reviewed_at"),
                "created_at": item.get("created_at"),
                "sync_state": "pending_owner_review",
                "entry_kind": item.get("entry_kind"),
                "source_kind": item.get("source_kind"),
                "source_url": item.get("source_url"),
                "source_path": item.get("source_path"),
                "idea_id": item.get("idea_id"),
                "summary": item.get("summary"),
                "revision_goals": item.get("revision_goals") or [],
                "latent_reason": item.get("latent_reason"),
                "transform_type": item.get("transform_type"),
                "generated_by": item.get("generated_by"),
                "generation_job_id": item.get("generation_job_id"),
                "generation_option_index": item.get("generation_option_index"),
                "source_card": dict(item.get("source_card") or {}) if isinstance(item.get("source_card"), dict) else {},
                "intent": item.get("intent"),
                "canonical_pillar": item.get("canonical_pillar"),
                "career_signal": item.get("career_signal"),
                "employer_proximity": item.get("employer_proximity"),
                "employer_safety": item.get("employer_safety"),
                "proof_posture": item.get("proof_posture"),
                "audience": item.get("audience"),
                "audience_consequence": item.get("audience_consequence"),
                "distinct_thesis": item.get("distinct_thesis"),
                "generation_receipt": dict(item.get("generation_receipt") or {}) if isinstance(item.get("generation_receipt"), dict) else {},
                "system_assessment": item.get("system_assessment"),
                "decision_scaffold": item.get("decision_scaffold"),
            },
        }
    )
    return payload


def _pending_owner_review_card_is_current(
    existing: Any,
    *,
    title: str,
    payload: dict[str, Any],
) -> bool:
    return (
        str(getattr(existing, "title", "") or "") == title
        and str(getattr(existing, "owner", "") or "") == "Neo"
        and str(getattr(existing, "status", "") or "") == "review"
        and _is_owner_review_source(getattr(existing, "source", ""))
        and str(getattr(existing, "link_type", "") or "") == OWNER_REVIEW_LINK_TYPE
        and getattr(existing, "link_id", None) == _owner_review_link_id()
        and dict(getattr(existing, "payload", {}) or {}) == payload
    )


def _owner_review_card_sort_key(card: Any) -> datetime:
    updated_at = getattr(card, "updated_at", None)
    created_at = getattr(card, "created_at", None)
    if isinstance(updated_at, datetime):
        return updated_at
    if isinstance(created_at, datetime):
        return created_at
    return datetime.min.replace(tzinfo=timezone.utc)


def _list_active_owner_review_cards() -> list[Any]:
    try:
        cards = pm_card_service.list_cards(limit=250)
    except Exception:
        return []
    return [
        card
        for card in cards
        if _is_owner_review_source(card.source)
        and str(card.link_type or "") == OWNER_REVIEW_LINK_TYPE
        and str(card.status or "").lower() in ACTIVE_OWNER_REVIEW_CARD_STATUSES
    ]


def _owner_review_payload_from_card(card: Any) -> dict[str, Any]:
    payload = dict(getattr(card, "payload", {}) or {})
    owner_review_payload = payload.get("owner_review") if isinstance(payload.get("owner_review"), dict) else {}
    return owner_review_payload if isinstance(owner_review_payload, dict) else {}


def _owner_review_card_has_decision(card: Any) -> bool:
    return bool(str(_owner_review_payload_from_card(card).get("decision") or "").strip())


def _owner_review_card_is_pending_sync(card: Any) -> bool:
    owner_review_payload = _owner_review_payload_from_card(card)
    return (
        str(owner_review_payload.get("sync_state") or "").strip() == "pending_owner_review"
        and not str(owner_review_payload.get("decision") or "").strip()
    )


def _matching_owner_review_cards(item: dict[str, Any], active_cards: list[Any]) -> list[Any]:
    identity_key = _owner_review_identity_key_for_item(item)
    if not identity_key:
        return []
    matches = [
        card
        for card in active_cards
        if _owner_review_identity_key_from_payload(_owner_review_payload_from_card(card)) == identity_key
    ]
    return sorted(matches, key=_owner_review_card_sort_key, reverse=True)


def _generated_owner_review_queue_id(job_id: str, option_index: int) -> str:
    normalized_job_id = str(job_id or "").strip()
    if not normalized_job_id:
        raise ValueError("A completed Codex job id is required for owner review.")
    if option_index < 0:
        raise ValueError("option_index must be 0 or greater.")
    digest = hashlib.sha256(f"{normalized_job_id}:{option_index}".encode("utf-8")).hexdigest()[:12].upper()
    return f"{GENERATED_OWNER_REVIEW_ID_PREFIX}-{digest}"


def _revision_post_sha256(copy_text: Any) -> str:
    return hashlib.sha256(str(copy_text or "").strip().encode("utf-8")).hexdigest()


def _selected_revision_verification(
    *,
    option_index: int,
    option_text: str,
    context_packet: dict[str, Any],
    generation_diagnostics: dict[str, Any],
) -> dict[str, Any] | None:
    contract = (
        context_packet.get("revision_contract")
        if isinstance(context_packet.get("revision_contract"), dict)
        else None
    )
    if contract is None:
        return None
    execution = (
        generation_diagnostics.get("revision_execution")
        if isinstance(generation_diagnostics.get("revision_execution"), dict)
        else {}
    )
    rows = execution.get("options") if isinstance(execution.get("options"), list) else []
    selected_index = option_index + 1
    selected = next(
        (
            item
            for item in rows
            if isinstance(item, dict)
            and int(item.get("canonical_option_index") or 0) == selected_index
        ),
        {},
    )
    return {
        "schema_version": SELECTED_REVISION_VERIFICATION_VERSION,
        "source_contract_schema_version": str(contract.get("schema_version") or ""),
        "source_receipt_schema_version": str(execution.get("schema_version") or ""),
        "status": str(execution.get("status") or ""),
        "failure_code": str(execution.get("failure_code") or ""),
        "selected_option_index": selected_index,
        "action": str(selected.get("action") or ""),
        "attempt_count": selected.get("attempt_count"),
        "original_post_sha256": str(selected.get("original_post_sha256") or ""),
        "final_post_sha256": str(selected.get("final_post_sha256") or ""),
        "changed": selected.get("changed"),
        "error_code": str(selected.get("error_code") or ""),
        "final_pair_sha256": str(execution.get("final_pair_sha256") or ""),
        "initial_critic_receipt_sha256": str(
            execution.get("initial_critic_receipt_sha256") or ""
        ),
        "final_critic_receipt_sha256": str(
            execution.get("final_critic_receipt_sha256") or ""
        ),
        "final_critic_call_count": execution.get("final_critic_call_count"),
        "selected_copy_sha256": _revision_post_sha256(option_text),
        "contains_sibling_option_rows": False,
        "contains_post_copy": False,
    }


def _generated_option_receipt(
    *,
    option_index: int,
    option_text: str,
    context_packet: dict[str, Any],
    generation_diagnostics: dict[str, Any],
) -> dict[str, Any]:
    strategy = context_packet.get("strategy_contract") if isinstance(context_packet.get("strategy_contract"), dict) else {}
    classification = (
        context_packet.get("candidate_classification")
        if isinstance(context_packet.get("candidate_classification"), dict)
        else {}
    )
    readiness = (
        generation_diagnostics.get("editorial_readiness")
        if isinstance(generation_diagnostics.get("editorial_readiness"), dict)
        else {}
    )
    option_reviews = readiness.get("option_reviews") if isinstance(readiness.get("option_reviews"), list) else []
    readiness_option_review = next(
        (
            dict(item)
            for item in option_reviews
            if isinstance(item, dict) and int(item.get("option_index") or 0) == option_index + 1
        ),
        {},
    )
    critic_receipt = (
        generation_diagnostics.get("critic_review")
        if isinstance(generation_diagnostics.get("critic_review"), dict)
        else {}
    )
    critic_reviews = critic_receipt.get("reviews") if isinstance(critic_receipt.get("reviews"), list) else []
    critic_option_review = next(
        (
            dict(item)
            for item in critic_reviews
            if isinstance(item, dict) and int(item.get("option_index") or 0) == option_index + 1
        ),
        {},
    )
    # Retain the critic's five scores alongside the editorial decision.  The
    # readiness copy is authoritative for `editorially_ready`, while the
    # critic copy carries the independently produced dimensions and hooks.
    option_review = {**critic_option_review, **readiness_option_review}
    quality_gate = generation_diagnostics.get("quality_gate") if isinstance(generation_diagnostics.get("quality_gate"), dict) else {}
    shared_quality = (
        quality_gate.get("shared_constraints")
        if isinstance(quality_gate.get("shared_constraints"), dict)
        else None
    )
    quality_projection = {
        key: quality_gate.get(key)
        for key in (
            "schema_version",
            "passed",
            "selection_admission_passed",
            "failed_reasons",
            "required_option_count",
            "evaluated_option_count",
        )
        if quality_gate.get(key) is not None
    }
    if shared_quality is not None:
        quality_projection["shared_constraints"] = {
            key: shared_quality.get(key)
            for key in (
                "passed",
                "failed_reasons",
                "required_option_count",
                "evaluated_option_count",
            )
            if shared_quality.get(key) is not None
        }
    selected_option_quality = _selected_option_quality_receipt(
        option_index=option_index,
        quality_gate=quality_gate,
    )
    revision_verification = _selected_revision_verification(
        option_index=option_index,
        option_text=option_text,
        context_packet=context_packet,
        generation_diagnostics=generation_diagnostics,
    )
    receipt = {
        "schema_version": "feezie_generation_receipt/v1",
        "selected_option_index": option_index + 1,
        "content_version_sha256": linkedin_content_version_sha256(option_text),
        "intent": str(context_packet.get("intent") or "value"),
        "strategy_contract": {
            key: strategy.get(key)
            for key in ("schema_version", "contract_hash", "approved_at")
            if strategy.get(key) not in (None, "")
        },
        "candidate_classification": {
            key: classification.get(key)
            for key in (
                "canonical_pillar",
                "career_signal",
                "employer_proximity",
                "employer_safety",
                "proof_posture",
                "treatment",
                "publish_posture",
                "audience",
                "generation_audience",
                "audience_consequence",
                "distinct_thesis",
                "why_now",
                "development_status",
                "source_freshness",
                "classification_state",
                "missing_fields",
            )
            if classification.get(key) not in (None, "", [], {})
        },
        "technical_completion": dict(generation_diagnostics.get("technical_completion") or {}),
        "quality_gate": quality_projection,
        "selected_option_quality": selected_option_quality,
        "editorial_readiness": {
            key: readiness.get(key)
            for key in (
                "ready",
                "status",
                "critic_status",
                "ready_score_threshold",
                "deterministic_quality_gate_passed",
                "semantic_distinctness_passed",
                "blocking_reasons",
            )
            if readiness.get(key) is not None
        },
        "critic_verification": {
            "status": critic_receipt.get("status"),
            "semantic_distinctness_passed": (
                (critic_receipt.get("draft_distinctness") or {}).get("passed")
                if isinstance(critic_receipt.get("draft_distinctness"), dict)
                else None
            ),
            "blind_review_receipt_present": isinstance(
                critic_receipt.get("blind_review_receipt"),
                dict,
            ),
        },
        "option_review": option_review,
    }
    if revision_verification is not None:
        receipt["revision_contract"] = dict(context_packet.get("revision_contract") or {})
        receipt["revision_verification"] = revision_verification
    return receipt


def _generated_owner_review_item(
    *,
    job_id: str,
    option_index: int,
    option_text: str,
    request_payload: dict[str, Any],
    context_packet: dict[str, Any],
    generation_diagnostics: dict[str, Any] | None = None,
) -> dict[str, Any]:
    normalized_option = str(option_text or "").strip()
    if not normalized_option:
        raise ValueError("A non-empty completed option is required for owner review.")
    source_card = dict(request_payload.get("source_card") or {}) if isinstance(request_payload.get("source_card"), dict) else {}
    queue_id = _generated_owner_review_queue_id(job_id, option_index)
    topic = str(request_payload.get("topic") or "Generated LinkedIn draft").strip()
    title = str(source_card.get("title") or topic or queue_id).strip()
    owner_reaction = str(source_card.get("owner_reaction") or "").strip()
    route_reason = str(source_card.get("route_reason") or "").strip()
    summary = str(source_card.get("summary") or "").strip()
    source_url = str(source_card.get("source_url") or "").strip() or None
    generation_receipt = _generated_option_receipt(
        option_index=option_index,
        option_text=normalized_option,
        context_packet=context_packet,
        generation_diagnostics=dict(generation_diagnostics or {}),
    )
    classification = generation_receipt["candidate_classification"]
    proof_anchors = [
        str(value).strip()
        for value in (context_packet.get("proof_packets") or [])
        if str(value).strip()
    ][:3]
    if source_url and not proof_anchors:
        proof_anchors.append(f"Original source: {source_url}")
    item: dict[str, Any] = {
        "queue_id": queue_id,
        "identity_key": queue_id,
        "title": title[:300],
        "lane": str(classification.get("audience") or source_card.get("priority_lane") or request_payload.get("audience") or "").strip(),
        "format": str(request_payload.get("content_type") or "linkedin_post").strip(),
        "core_angle": topic,
        "why_now": str(classification.get("why_now") or classification.get("audience_consequence") or "").strip()
        or owner_reaction
        or route_reason
        or summary
        or "A completed local Codex option is ready for explicit owner judgment.",
        "status": "owner_review_draft",
        "approval_status": "owner_review_required",
        "draft_path": "",
        "owner_packet_path": None,
        "proof_anchors": proof_anchors,
        "draft_body": "",
        "first_pass_draft": normalized_option,
        "draft_owner_notes": [owner_reaction] if owner_reaction else [],
        "packet_section": "",
        "packet_recommendation": "This generated option is not cleared for scheduling or publication and needs an explicit owner decision.",
        "current_decision": None,
        "current_notes": None,
        "publish_posture": "owner_review_required",
        "reviewed_at": None,
        "created_at": datetime.now(timezone.utc).isoformat(),
        "entry_kind": "generated",
        "source_kind": GENERATED_OWNER_REVIEW_SOURCE_KIND,
        "source_url": source_url,
        "source_path": str(source_card.get("source_path") or "").strip() or None,
        "idea_id": str(source_card.get("item_key") or source_card.get("origin_id") or "").strip() or None,
        "summary": summary,
        "revision_goals": [],
        "latent_reason": None,
        "transform_type": None,
        "generated_by": "local_codex",
        "generation_job_id": str(job_id).strip(),
        "generation_option_index": option_index,
        "source_card": source_card,
        "intent": generation_receipt["intent"],
        "canonical_pillar": classification.get("canonical_pillar"),
        "career_signal": classification.get("career_signal"),
        "employer_proximity": classification.get("employer_proximity"),
        "employer_safety": classification.get("employer_safety"),
        "proof_posture": classification.get("proof_posture"),
        "audience": classification.get("audience"),
        "audience_consequence": classification.get("audience_consequence"),
        "distinct_thesis": classification.get("distinct_thesis"),
        "generation_receipt": generation_receipt,
    }
    item["system_assessment"] = _build_owner_review_assessment(item)
    item["decision_scaffold"] = _build_owner_decision_scaffold(item, item["system_assessment"])
    return item


def _owner_review_cards_for_identity(identity_key: str) -> list[Any]:
    cards = pm_card_service.list_cards(limit=250)
    matches = [
        card
        for card in cards
        if _is_owner_review_source(getattr(card, "source", None))
        and str(getattr(card, "link_type", "") or "") == OWNER_REVIEW_LINK_TYPE
        and _owner_review_identity_key_from_payload(_owner_review_payload_from_card(card)) == identity_key
    ]
    return sorted(matches, key=_owner_review_card_sort_key, reverse=True)


def ensure_generated_owner_review_item(
    *,
    legacy_compatibility: bool = False,
    job_id: str,
    option_index: int,
    option_text: str,
    request_payload: dict[str, Any] | None = None,
    context_packet: dict[str, Any] | None = None,
    generation_diagnostics: dict[str, Any] | None = None,
) -> dict[str, Any]:
    """Persist one completed Codex option in the existing PM-backed FEEZIE review lane."""

    _require_legacy_owner_review_compatibility(legacy_compatibility)
    item = _generated_owner_review_item(
        job_id=job_id,
        option_index=option_index,
        option_text=option_text,
        request_payload=dict(request_payload or {}),
        context_packet=dict(context_packet or {}),
        generation_diagnostics=dict(generation_diagnostics or {}),
    )
    identity_key = _owner_review_identity_key_for_item(item)
    matches = _owner_review_cards_for_identity(identity_key)
    decided_card = next((card for card in matches if _owner_review_card_has_decision(card)), None)
    if decided_card is not None:
        decided_payload = _owner_review_payload_from_card(decided_card)
        existing_decision = str(decided_payload.get("decision") or "").strip().lower()
        raise ValueError(
            f"Generated option {option_index} from job {job_id} already received owner decision `{existing_decision}`."
        )
    pending_card = next((card for card in matches if _owner_review_card_is_pending_sync(card)), None)
    if pending_card is not None:
        pending_payload = _owner_review_payload_from_card(pending_card)
        existing_item = _item_from_owner_review_payload(pending_payload) or item
        return {
            "card_id": str(getattr(pending_card, "id", "") or ""),
            "queue_id": str(existing_item.get("queue_id") or item["queue_id"]),
            "duplicate": True,
            "item": existing_item,
            "message": "This generated option is already waiting in FEEZIE owner review.",
        }

    card_payload = _build_pending_owner_review_card_payload(item)
    card = pm_card_service.create_card(
        PMCardCreate(
            title=_pending_owner_review_card_title(item),
            owner="Neo",
            status="review",
            source=OWNER_REVIEW_CARD_SOURCE,
            link_type=OWNER_REVIEW_LINK_TYPE,
            link_id=_owner_review_link_id(),
            payload=card_payload,
        )
    )
    stored_card_payload = dict(getattr(card, "payload", {}) or {})
    stored_owner_review_payload = _owner_review_payload_from_card(card)
    stored_decision = str(stored_owner_review_payload.get("decision") or "").strip().lower()
    if stored_decision:
        raise ValueError(
            f"Generated option {option_index} from job {job_id} already received owner decision `{stored_decision}`."
        )
    trigger_replays = int(stored_card_payload.get("trigger_replays") or 0)
    duplicate = trigger_replays > 0
    stored_item = _item_from_owner_review_payload(stored_owner_review_payload) or item
    return {
        "card_id": str(card.id),
        "queue_id": str(stored_item.get("queue_id") or item["queue_id"]),
        "duplicate": duplicate,
        "item": stored_item,
        "message": (
            "This generated option is already waiting in FEEZIE owner review."
            if duplicate
            else "Generated option sent to FEEZIE owner review. It will remain private until you decide."
        ),
    }


def _close_superseded_owner_review_card(card: Any, *, replacement: Any, rule: str, reason: str) -> str:
    payload = dict(getattr(card, "payload", {}) or {})
    payload["duplicate_resolution"] = {
        "rule": rule,
        "replacement_card_id": getattr(replacement, "id", None),
        "replacement_card_title": getattr(replacement, "title", None),
        "closed_at": datetime.now(timezone.utc).isoformat(),
        "reason": reason,
    }
    owner_review_payload = payload.get("owner_review") if isinstance(payload.get("owner_review"), dict) else {}
    if isinstance(owner_review_payload, dict):
        owner_review_payload["sync_state"] = "superseded"
        payload["owner_review"] = owner_review_payload
    updated = pm_card_service.update_card(str(getattr(card, "id")), PMCardUpdate(status="done", payload=payload))
    return str(updated.id if updated is not None else getattr(card, "id"))


def _auto_close_superseded_owner_review_cards(active_cards: list[Any]) -> list[str]:
    cards_by_identity: dict[str, list[Any]] = {}
    for card in active_cards:
        identity_key = _owner_review_identity_key_from_payload(_owner_review_payload_from_card(card))
        if not identity_key:
            continue
        cards_by_identity.setdefault(identity_key, []).append(card)

    closed_card_ids: list[str] = []
    for matches in cards_by_identity.values():
        ranked = sorted(matches, key=_owner_review_card_sort_key, reverse=True)
        decision_cards = [card for card in ranked if _owner_review_card_has_decision(card)]
        if decision_cards:
            replacement = decision_cards[0]
            for duplicate in ranked:
                if duplicate.id == replacement.id or not _owner_review_card_is_pending_sync(duplicate):
                    continue
                closed_card_ids.append(
                    _close_superseded_owner_review_card(
                        duplicate,
                        replacement=replacement,
                        rule="owner_review_pending_replaced_by_decision_autoclose",
                        reason="Closed stale pending owner-review card because a newer decision-bearing card already owns this review surface.",
                    )
                )
            continue
        pending_cards = [card for card in ranked if _owner_review_card_is_pending_sync(card)]
        if len(pending_cards) < 2:
            continue
        replacement = pending_cards[0]
        for duplicate in pending_cards[1:]:
            closed_card_ids.append(
                _close_superseded_owner_review_card(
                    duplicate,
                    replacement=replacement,
                    rule="owner_review_pending_identity_autoclose",
                    reason="Closed duplicate pending owner-review card because a newer card already represents the same draft identity.",
                )
            )
    return closed_card_ids


def sync_owner_review_pm_cards(*, legacy_compatibility: bool = False) -> dict[str, Any]:
    _require_legacy_owner_review_compatibility(legacy_compatibility)
    active_cards = _list_active_owner_review_cards()
    closed_duplicate_card_ids = _auto_close_superseded_owner_review_cards(active_cards)
    active_cards = [card for card in active_cards if str(getattr(card, "id", "")) not in set(closed_duplicate_card_ids)]
    payload = list_owner_review_items()
    created_card_ids: list[str] = []
    updated_card_ids: list[str] = []
    skipped_card_ids: list[str] = []
    pending_queue_ids: list[str] = []

    for item in payload.get("items", []):
        if not isinstance(item, dict) or not _item_needs_owner_review(item):
            continue
        queue_id = str(item.get("queue_id") or "").strip()
        title = str(item.get("title") or queue_id).strip()
        if not queue_id:
            continue
        pending_queue_ids.append(queue_id)
        identity_key = _owner_review_identity_key_for_item(item) or queue_id
        existing = pm_card_service.find_active_card_by_trigger_key(_owner_review_trigger_key(identity_key))
        if existing is None:
            matches = _matching_owner_review_cards(item, active_cards)
            existing = matches[0] if matches else None
        card_payload = _build_pending_owner_review_card_payload(
            item,
            existing_payload=dict(existing.payload or {}) if existing is not None else None,
        )
        card_title = _pending_owner_review_card_title(item)
        if existing is None:
            card = pm_card_service.create_card(
                PMCardCreate(
                    title=card_title,
                    owner="Neo",
                    status="review",
                    source=OWNER_REVIEW_CARD_SOURCE,
                    link_type=OWNER_REVIEW_LINK_TYPE,
                    link_id=_owner_review_link_id(),
                    payload=card_payload,
                )
            )
            created_card_ids.append(card.id)
            active_cards.append(card)
            continue
        if _pending_owner_review_card_is_current(existing, title=card_title, payload=card_payload):
            skipped_card_ids.append(existing.id)
            continue
        updated = pm_card_service.update_card(
            existing.id,
            PMCardUpdate(
                title=card_title,
                owner="Neo",
                status="review",
                source=OWNER_REVIEW_CARD_SOURCE,
                link_type=OWNER_REVIEW_LINK_TYPE,
                link_id=_owner_review_link_id(),
                payload=card_payload,
            ),
        )
        effective = updated if updated is not None else existing
        updated_card_ids.append(effective.id)
        active_cards = [card for card in active_cards if str(getattr(card, "id", "")) != str(existing.id)]
        active_cards.append(effective)

    return {
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "pending_count": len(pending_queue_ids),
        "pending_queue_ids": pending_queue_ids,
        "created_card_ids": created_card_ids,
        "updated_card_ids": updated_card_ids,
        "skipped_card_ids": skipped_card_ids,
        "closed_duplicate_card_ids": closed_duplicate_card_ids,
    }


def record_owner_decision_for_pm_card(
    card_id: str,
    decision: str,
    notes: str | None = None,
    *,
    legacy_compatibility: bool = False,
) -> dict[str, Any]:
    _require_legacy_owner_review_compatibility(legacy_compatibility)
    card = pm_card_service.get_card(card_id)
    if card is None:
        raise LinkedinOwnerReviewNotFoundError(f"PM card not found: {card_id}")
    payload = dict(card.payload or {})
    owner_review_payload = payload.get("owner_review") if isinstance(payload.get("owner_review"), dict) else {}
    queue_id = ""
    fallback_item: dict[str, Any] | None = None
    if isinstance(owner_review_payload, dict):
        queue_id = str(owner_review_payload.get("queue_id") or "").strip()
        fallback_item = _item_from_owner_review_payload(owner_review_payload)
    if not queue_id and isinstance(card.link_id, str):
        queue_id = card.link_id.strip()
    if not queue_id:
        raise ValueError(f"PM card {card_id} is not linked to an owner-review queue item.")
    payload_snapshot = list_owner_review_items()
    item = next((entry for entry in payload_snapshot.get("items", []) if entry.get("queue_id") == queue_id), None)
    if item is None:
        item = fallback_item
    if item is None:
        raise LinkedinOwnerReviewNotFoundError(f"Unknown queue item: {queue_id}")
    result = _record_owner_decision_with_lifecycle_for_item(
        item,
        decision,
        notes,
        tolerate_missing_artifacts=True,
        legacy_compatibility=True,
    )
    result["source_card_id"] = card_id
    return result


def _queue_owner_review_followup(
    item: dict[str, Any],
    *,
    decision: str,
    notes: str,
    draft_rel_path: str,
    packet_rel_path: str | None,
    reviewed_at: str,
) -> dict[str, Any]:
    queue_id = str(item.get("queue_id") or "").strip()
    identity_key = _owner_review_identity_key_for_item(item) or queue_id
    existing = pm_card_service.find_active_card_by_trigger_key(_owner_review_trigger_key(identity_key))

    if decision == "park":
        if existing is None:
            return {
                "status": "noop",
                "message": f"{queue_id} parked. No backend follow-up was active.",
            }
        existing_payload = _build_owner_review_card_payload(
            item,
            decision=decision,
            notes=notes,
            draft_rel_path=draft_rel_path,
            packet_rel_path=packet_rel_path,
            reviewed_at=reviewed_at,
            existing_payload=dict(existing.payload or {}),
        )
        execution = dict(existing_payload.get("execution") or {})
        execution.update(
            {
                "state": "cancelled",
                "last_transition_at": datetime.now(timezone.utc).isoformat(),
                "reason": f"{queue_id} was parked during owner review.",
            }
        )
        existing_payload["execution"] = execution
        pm_card_service.update_card(existing.id, PMCardUpdate(status="cancelled", payload=existing_payload))
        return {
            "status": "cancelled",
            "message": f"{queue_id} parked. Cancelled backend follow-up card {existing.id}.",
            "card_id": existing.id,
        }

    title = _owner_review_card_title(item, decision)
    payload = _build_owner_review_card_payload(
        item,
        decision=decision,
        notes=notes,
        draft_rel_path=draft_rel_path,
        packet_rel_path=packet_rel_path,
        reviewed_at=reviewed_at,
        existing_payload=dict(existing.payload or {}) if existing is not None else None,
    )

    if existing is None:
            card = pm_card_service.create_card(
                PMCardCreate(
                    title=title,
                    owner="Neo",
                    status="todo",
                    source=OWNER_REVIEW_CARD_SOURCE,
                    link_type=OWNER_REVIEW_LINK_TYPE,
                    link_id=_owner_review_link_id(),
                    payload=payload,
                )
            )
    else:
        card = pm_card_service.update_card(
            existing.id,
            PMCardUpdate(
                title=title,
                owner="Neo",
                status="todo",
                source=OWNER_REVIEW_CARD_SOURCE,
                link_type=OWNER_REVIEW_LINK_TYPE,
                link_id=_owner_review_link_id(),
                payload=payload,
            ),
        )
        if card is None:
            raise ValueError(f"Unable to update owner-review PM card for {queue_id}")

    dispatch_result = pm_card_service.dispatch_card(
        card.id,
        PMCardDispatchRequest(
            target_agent="Jean-Claude",
            lane="codex",
            requested_by="Neo",
            execution_state="queued",
        ),
    )
    if dispatch_result is None:
        raise ValueError(f"Unable to dispatch owner-review PM card for {queue_id}")
    return {
        "status": "queued",
        "message": (
            f"{queue_id} saved as {decision}. Jean-Claude follow-up is queued on backend card {dispatch_result.card.id}."
        ),
        "card_id": dispatch_result.card.id,
        "target_agent": dispatch_result.queue_entry.target_agent,
        "execution_state": dispatch_result.queue_entry.execution_state,
    }


def _split_pending_owner_review_items(items: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    pending: list[dict[str, Any]] = []
    resolved: list[dict[str, Any]] = []
    for item in items:
        decision = str(item.get("current_decision") or "").strip().lower()
        if decision:
            resolved.append(item)
        else:
            pending.append(item)
    return pending, resolved


def list_owner_review_items(*, include_resolved: bool = False) -> dict[str, Any]:
    root = _linkedin_root()
    queue_path = _queue_path(root)
    queue_text = _read_text(queue_path)
    packet_path = _latest_owner_packet(root)
    items: list[dict[str, Any]] = []
    if not queue_text:
        supplemental_items = _list_supplemental_owner_review_items(root)
        supplemental_items.extend(_list_pm_pending_owner_review_items())
        supplemental_items = _dedupe_owner_review_items(supplemental_items)
        supplemental_items = _apply_pm_owner_review_decision_overrides(supplemental_items)
        supplemental_items = [
            item for item in supplemental_items if not _is_development_only_owner_review_item(item)
        ]
        pending_items, resolved_items = _split_pending_owner_review_items(supplemental_items)
        selected_items = supplemental_items if include_resolved else pending_items
        return {
            "generated_at": datetime.now(timezone.utc).isoformat(),
            "queue_path": queue_path.relative_to(root).as_posix(),
            "owner_packet_path": packet_path.relative_to(root).as_posix() if packet_path and packet_path.exists() else None,
            "items": selected_items,
            "pending_count": len(pending_items),
            "resolved_count": len(resolved_items),
            "total_count": len(supplemental_items),
        }
    section = queue_text.split("## Queue", 1)[1] if "## Queue" in queue_text else queue_text
    for queue_id, title, block in _split_queue_blocks(section):
        fields, list_fields = _parse_bullet_fields(block)
        draft_rel_path, draft_path = _draft_path_from_status(root, fields.get("status", ""))
        if not draft_rel_path or draft_path is None or not draft_path.exists():
            continue
        items.append(_serialize_item(root, queue_id, title, fields, list_fields, packet_path))
    items.extend(_list_supplemental_owner_review_items(root))
    items.extend(_list_pm_pending_owner_review_items())
    items = _dedupe_owner_review_items(items)
    items = _apply_pm_owner_review_decision_overrides(items)
    items = [item for item in items if not _is_development_only_owner_review_item(item)]
    pending_items, resolved_items = _split_pending_owner_review_items(items)
    selected_items = items if include_resolved else pending_items
    return {
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "queue_path": queue_path.relative_to(root).as_posix(),
        "owner_packet_path": packet_path.relative_to(root).as_posix() if packet_path and packet_path.exists() else None,
        "items": selected_items,
        "pending_count": len(pending_items),
        "resolved_count": len(resolved_items),
        "total_count": len(items),
    }


def _update_queue_file(queue_path: Path, queue_id: str, decision: str, draft_rel_path: str, packet_rel_path: str | None) -> None:
    text = _read_text(queue_path)
    bounds = _section_bounds(text, QUEUE_HEADING_RE, queue_id)
    if bounds is None:
        raise ValueError(f"{queue_id} is not present in {queue_path}")
    start, end = bounds
    section = text[start:end]
    section = re.sub(
        r"^- Status: .*$",
        f"- Status: {STATUS_MAP[decision]} (`{draft_rel_path}`)",
        section,
        count=1,
        flags=re.MULTILINE,
    )
    section = re.sub(
        r"^- Approval status: .*$",
        f"- Approval status: `{APPROVAL_STATUS_MAP[decision]}`",
        section,
        count=1,
        flags=re.MULTILINE,
    )
    if packet_rel_path:
        section = re.sub(
            r"^- Owner packet: .*$",
            f"- Owner packet: `{packet_rel_path}` entry for `{queue_id}`",
            section,
            count=1,
            flags=re.MULTILINE,
        )
    updated = f"{text[:start]}{section}{text[end:]}"
    queue_path.write_text(updated, encoding="utf-8")


def _update_owner_packet(packet_path: Path, queue_id: str, decision: str, notes: str) -> None:
    text = _read_text(packet_path)
    bounds = _section_bounds(text, PACKET_HEADING_RE, queue_id)
    if bounds is None:
        raise ValueError(f"{queue_id} is not present in {packet_path}")
    start, end = bounds
    section = text[start:end]
    for candidate, label in PACKET_CHECKBOX_LINES.items():
        mark = "x" if candidate == decision else " "
        section = re.sub(
            rf"^- \[[ xX]\]\s+{re.escape(label)}\s*$",
            f"- [{mark}] {label}",
            section,
            count=1,
            flags=re.MULTILINE,
        )
    replacement_notes = notes.strip()
    section = re.sub(
        r"\*\*Revision notes \(if needed\):\*\*\n.*\Z",
        f"**Revision notes (if needed):**\n{replacement_notes}".rstrip(),
        section,
        count=1,
        flags=re.DOTALL,
    )
    updated = f"{text[:start]}{section}{text[end:]}"
    packet_path.write_text(updated, encoding="utf-8")


def _update_draft_file(draft_path: Path, decision: str, reviewed_at: str, notes: str | None = None) -> None:
    text = _read_text(draft_path)
    updated = _set_frontmatter_fields(
        text,
        {
            "publish_posture": PUBLISH_POSTURE_MAP[decision],
            "owner_decision": decision,
            "owner_reviewed_at": reviewed_at,
            "owner_review_notes": (notes or "").strip(),
        },
    )
    draft_path.write_text(updated, encoding="utf-8")


def _append_execution_log(root: Path, queue_id: str, title: str, decision: str, notes: str, draft_rel_path: str, packet_rel_path: str | None) -> None:
    log_path = _execution_log_path(root)
    log_path.parent.mkdir(parents=True, exist_ok=True)
    timestamp = datetime.now().astimezone().strftime("%Y-%m-%d %H:%M %Z")
    note_line = notes.strip() or "No additional notes."
    entry = (
        f"\n## Owner Review Decision — {timestamp}\n\n"
        f"- Queue item: `{queue_id}`\n"
        f"- Title: {title}\n"
        f"- Decision: `{decision}`\n"
        f"- Notes: {note_line}\n"
        f"- Draft: `{draft_rel_path}`\n"
        f"- Owner packet: `{packet_rel_path or 'missing'}`\n"
    )
    with log_path.open("a", encoding="utf-8") as handle:
        handle.write(entry)

def _record_owner_decision_for_item(
    item: dict[str, Any],
    decision: str,
    notes: str | None = None,
    *,
    tolerate_missing_artifacts: bool = False,
    legacy_compatibility: bool = False,
) -> dict[str, Any]:
    _require_legacy_owner_review_compatibility(legacy_compatibility)
    root = _linkedin_root()
    queue_path = _queue_path(root)
    packet_path = _latest_owner_packet(root)
    queue_id = str(item.get("queue_id") or "").strip()
    draft_rel_path = str(item.get("draft_path") or "").strip()
    artifact_warnings: list[str] = []
    if not draft_rel_path:
        if not tolerate_missing_artifacts:
            raise ValueError(f"{queue_id} does not have a draft path.")
        artifact_warnings.append(f"{queue_id} has no draft path in the current runtime payload.")
        draft_path = None
    else:
        draft_path = root / draft_rel_path
        if not draft_path.exists():
            if not tolerate_missing_artifacts:
                raise ValueError(f"Draft file is missing for {queue_id}: {draft_rel_path}")
            artifact_warnings.append(f"Draft file is missing for {queue_id}: {draft_rel_path}")
            draft_path = None
    reviewed_at = datetime.now(timezone.utc).isoformat()
    packet_rel_path = packet_path.relative_to(root).as_posix() if packet_path and packet_path.exists() else None
    workflow: dict[str, Any] | None = None

    if str(item.get("entry_kind") or "") == "queue":
        bounds = _section_bounds(_read_text(queue_path), QUEUE_HEADING_RE, queue_id)
        if bounds is not None:
            _update_queue_file(queue_path, queue_id, decision, draft_rel_path, packet_rel_path)
        elif tolerate_missing_artifacts:
            artifact_warnings.append(f"{queue_id} is not present in the live queue artifact at {queue_path.relative_to(root).as_posix()}.")
        else:
            raise ValueError(f"{queue_id} is not present in {queue_path}")
        if packet_path and packet_path.exists():
            packet_bounds = _section_bounds(_read_text(packet_path), PACKET_HEADING_RE, queue_id)
            if packet_bounds is not None:
                _update_owner_packet(packet_path, queue_id, decision, notes or "")
            elif tolerate_missing_artifacts:
                artifact_warnings.append(f"{queue_id} is not present in the live owner packet at {packet_rel_path}.")
            else:
                raise ValueError(f"{queue_id} is not present in {packet_path}")
        elif tolerate_missing_artifacts:
            artifact_warnings.append("Live owner packet is missing in this runtime, so the decision was recorded from PM payload only.")
    if draft_path is not None:
        _update_draft_file(draft_path, decision, reviewed_at, notes)
    if not _is_generated_owner_review_item(item):
        _append_execution_log(root, queue_id, str(item.get("title") or queue_id), decision, notes or "", draft_rel_path, packet_rel_path)
    try:
        workflow = _queue_owner_review_followup(
            item,
            decision=decision,
            notes=notes or "",
            draft_rel_path=draft_rel_path,
            packet_rel_path=packet_rel_path,
            reviewed_at=reviewed_at,
        )
    except Exception as exc:
        workflow = {
            "status": "error",
            "message": (
                f"{queue_id} saved as {decision}, but backend follow-up failed to queue: {exc}"
            ),
        }

    refreshed = list_owner_review_items()
    refreshed["workflow"] = workflow
    decision_recorded = not _is_generated_owner_review_item(item) or str((workflow or {}).get("status") or "") != "error"
    refreshed["owner_decision_receipt"] = {
        "schema_version": "feezie_owner_decision_receipt/v1",
        "queue_id": queue_id,
        "decision": decision,
        "reviewed_at": reviewed_at,
        "decision_recorded": decision_recorded,
        "replay": False,
    }
    if not decision_recorded:
        artifact_warnings.append(
            "The generated owner decision could not be verified as durable in PM; retry the same decision before treating it as recorded."
        )
    if artifact_warnings:
        refreshed["artifact_warnings"] = artifact_warnings
    return refreshed


def _resolved_pm_owner_review_item(queue_id: str) -> dict[str, Any] | None:
    """Recover a decided item from PM history for idempotent lifecycle retry."""

    normalized_queue_id = str(queue_id or "").strip()
    if not normalized_queue_id:
        return None
    try:
        cards = pm_card_service.list_cards(limit=250)
    except Exception:
        return None
    matches = []
    for card in cards:
        if not _is_owner_review_source(getattr(card, "source", None)):
            continue
        if str(getattr(card, "link_type", "") or "") != OWNER_REVIEW_LINK_TYPE:
            continue
        owner_review_payload = _owner_review_payload_from_card(card)
        if str(owner_review_payload.get("queue_id") or "").strip() != normalized_queue_id:
            continue
        if not str(owner_review_payload.get("decision") or "").strip():
            continue
        matches.append(card)
    if not matches:
        return None
    selected = sorted(matches, key=_owner_review_card_sort_key, reverse=True)[0]
    owner_review_payload = _owner_review_payload_from_card(selected)
    item = _item_from_owner_review_payload(owner_review_payload)
    if item is None:
        return None
    if not item.get("reviewed_at"):
        updated_at = getattr(selected, "updated_at", None)
        if isinstance(updated_at, datetime):
            item["reviewed_at"] = updated_at.astimezone(timezone.utc).isoformat()
            item["reviewed_at_source"] = "pm_updated_at_compatibility"
    return item


def _find_owner_review_item_for_decision(queue_id: str) -> dict[str, Any] | None:
    normalized_queue_id = str(queue_id or "").strip()
    if not normalized_queue_id:
        return None
    payload = list_owner_review_items(include_resolved=True)
    item = next(
        (
            entry
            for entry in payload.get("items", [])
            if isinstance(entry, dict) and str(entry.get("queue_id") or "").strip() == normalized_queue_id
        ),
        None,
    )
    if item is not None:
        return item
    item = _find_pm_pending_owner_review_item(normalized_queue_id)
    return item if item is not None else _resolved_pm_owner_review_item(normalized_queue_id)


def _normalized_reviewed_at(value: Any) -> str:
    raw = str(value or "").strip()
    if not raw:
        raise LinkedinOwnerReviewConflictError(
            "The owner decision is durable, but its review timestamp is missing; lifecycle reconciliation cannot proceed automatically."
        )
    try:
        parsed = datetime.fromisoformat(raw.replace("Z", "+00:00"))
    except ValueError as exc:
        raise LinkedinOwnerReviewConflictError(
            "The owner decision is durable, but its review timestamp is invalid; lifecycle reconciliation cannot proceed automatically."
        ) from exc
    if parsed.tzinfo is None or parsed.utcoffset() is None:
        raise LinkedinOwnerReviewConflictError(
            "The owner decision is durable, but its review timestamp lacks a timezone; lifecycle reconciliation cannot proceed automatically."
        )
    return parsed.astimezone(timezone.utc).isoformat()


def _owner_review_lifecycle_idempotency_key(
    *,
    queue_id: str,
    content_version_sha256: str,
    decision: str,
) -> str:
    seed = json.dumps(
        {
            "schema_version": OWNER_REVIEW_LIFECYCLE_EVENT_SCHEMA,
            "content_id": queue_id,
            "content_version_sha256": content_version_sha256,
            "owner_decision": decision,
        },
        sort_keys=True,
        separators=(",", ":"),
        ensure_ascii=True,
    )
    return f"feezie-owner-review-v1:{hashlib.sha256(seed.encode('utf-8')).hexdigest()}"


def _build_owner_review_lifecycle_request(
    *,
    queue_id: str,
    content_version_sha256: str,
    decision: str,
    reviewed_at: str,
) -> dict[str, Any]:
    request = LinkedinPerformanceLocalActionRequest.model_validate(
        {
            "event_type": "owner_reviewed",
            "idempotency_key": _owner_review_lifecycle_idempotency_key(
                queue_id=queue_id,
                content_version_sha256=content_version_sha256,
                decision=decision,
            ),
            "content_id": queue_id,
            "content_version_sha256": content_version_sha256,
            "occurred_at": reviewed_at,
            "owner_decision": decision,
        }
    )
    payload = request.model_dump(mode="json", exclude_none=True)
    forbidden = {
        "copy_text",
        "notes",
        "publication_url",
        "evidence_ref",
        "source_url",
        "source_path",
        "draft_path",
        "owner_packet_path",
    }
    if forbidden.intersection(payload):
        raise RuntimeError("Owner-review lifecycle request violated the privacy-minimized queue contract.")
    return payload


def _queue_owner_review_lifecycle(
    *,
    queue_id: str,
    decision: str,
    reviewed_at: str,
    content_version_sha256: str,
    replay: bool,
    legacy_compatibility: bool = False,
) -> dict[str, Any]:
    _require_legacy_owner_review_compatibility(legacy_compatibility)
    idempotency_key = _owner_review_lifecycle_idempotency_key(
        queue_id=queue_id,
        content_version_sha256=content_version_sha256,
        decision=decision,
    )
    base = {
        "schema_version": OWNER_REVIEW_LIFECYCLE_JOIN_SCHEMA,
        "event_type": "owner_reviewed",
        "content_id": queue_id,
        "content_version_sha256": content_version_sha256,
        "owner_decision": decision,
        "idempotency_key": idempotency_key,
        "decision_replay": replay,
        "approval_verified": False,
        "event_recorded": False,
        "retry": {
            "same_owner_review_endpoint": True,
            "same_decision_required": True,
            "safe_after_partial_failure": True,
        },
        "data_policy": {
            "raw_copy_included": False,
            "private_notes_included": False,
            "publication_url_included": False,
            "local_path_included": False,
        },
    }
    try:
        request = _build_owner_review_lifecycle_request(
            queue_id=queue_id,
            content_version_sha256=content_version_sha256,
            decision=decision,
            reviewed_at=reviewed_at,
        )
        card, disposition = enqueue_brain_local_action(
            "linkedin_performance_record",
            {"legacy_compatibility": True, "request": request},
        )
    except Exception as exc:
        return {
            **base,
            "queued": False,
            "status": "queue_failed",
            "pending_local_record": True,
            "retryable": True,
            "warning": (
                "The owner decision was recorded, but its private lifecycle event was not queued. "
                "Retry the same decision on this owner-review item; the exact digest and idempotency key will be reused."
            ),
            "error_type": type(exc).__name__,
        }
    return {
        **base,
        "queued": True,
        "status": "queued",
        "disposition": disposition,
        "job_id": str(card.id),
        "card_id": str(card.id),
        "pending_local_record": True,
        "retryable": True,
        "warning": (
            "The owner decision is saved, but approval is not lifecycle-verified until the local worker records this event."
            if decision == "approve"
            else "The owner decision is saved; the private lifecycle event is pending the local worker."
        ),
    }


def _record_owner_decision_with_lifecycle_for_item(
    item: dict[str, Any],
    decision: str,
    notes: str | None = None,
    *,
    tolerate_missing_artifacts: bool = False,
    legacy_compatibility: bool = False,
) -> dict[str, Any]:
    _require_legacy_owner_review_compatibility(legacy_compatibility)
    normalized_decision = str(decision or "").strip().lower()
    if normalized_decision not in STATUS_MAP:
        raise ValueError(f"Unsupported owner-review decision: {decision!r}")
    queue_id = str(item.get("queue_id") or "").strip()
    if not queue_id:
        raise LinkedinOwnerReviewNotFoundError("Owner-review queue id is missing.")
    if _is_development_only_owner_review_item(item):
        raise LinkedinOwnerReviewConflictError(
            f"{queue_id} is development-only planning material, not reviewed post copy, and cannot receive an owner-review decision."
        )

    existing_decision = str(item.get("current_decision") or "").strip().lower()
    if existing_decision and existing_decision != normalized_decision:
        raise LinkedinOwnerReviewConflictError(
            f"{queue_id} already has owner decision `{existing_decision}`; a replay must use that exact decision."
        )
    if not existing_decision and not _item_needs_owner_review(item):
        raise LinkedinOwnerReviewConflictError(
            f"{queue_id} is not in the exact owner-review-required state and cannot receive a decision."
        )
    if normalized_decision == "approve":
        _require_generated_approval_readiness(item)

    exact_copy = str(item.get("first_pass_draft") or "")
    if not exact_copy.strip():
        raise LinkedinOwnerReviewConflictError(
            f"{queue_id} has no exact operational first-pass draft; owner review cannot create lifecycle evidence."
        )
    content_version_sha256 = linkedin_content_version_sha256(exact_copy)
    replay = bool(existing_decision)

    if replay:
        reviewed_at = _normalized_reviewed_at(item.get("reviewed_at"))
        result = list_owner_review_items()
        result["workflow"] = {
            "status": "already_recorded",
            "message": (
                f"{queue_id} already records `{normalized_decision}`. The decision was not rewritten; "
                "private lifecycle reconciliation was retried."
            ),
        }
        result["owner_decision_receipt"] = {
            "schema_version": "feezie_owner_decision_receipt/v1",
            "queue_id": queue_id,
            "decision": normalized_decision,
            "reviewed_at": reviewed_at,
            "decision_recorded": True,
            "replay": True,
        }
    else:
        result = _record_owner_decision_for_item(
            item,
            normalized_decision,
            notes,
            tolerate_missing_artifacts=tolerate_missing_artifacts,
            legacy_compatibility=True,
        )
        decision_receipt = (
            result.get("owner_decision_receipt")
            if isinstance(result.get("owner_decision_receipt"), dict)
            else {}
        )
        reviewed_at = _normalized_reviewed_at(decision_receipt.get("reviewed_at"))

    decision_recorded = replay or bool((result.get("owner_decision_receipt") or {}).get("decision_recorded"))

    result["owner_decision_receipt"] = {
        **dict(result.get("owner_decision_receipt") or {}),
        "queue_id": queue_id,
        "decision": normalized_decision,
        "reviewed_at": reviewed_at,
        "decision_recorded": decision_recorded,
        "replay": replay,
        "content_version_sha256": content_version_sha256,
        "exact_copy_bound": True,
        "lifecycle_event_recorded": False,
    }
    if not decision_recorded:
        lifecycle_queue = {
            "schema_version": OWNER_REVIEW_LIFECYCLE_JOIN_SCHEMA,
            "status": "not_queued_owner_decision_unverified",
            "queued": False,
            "event_type": "owner_reviewed",
            "content_id": queue_id,
            "content_version_sha256": content_version_sha256,
            "owner_decision": normalized_decision,
            "approval_verified": False,
            "event_recorded": False,
            "pending_local_record": True,
            "retryable": True,
            "warning": (
                "The generated owner decision could not be verified as durable, so no lifecycle event was queued. "
                "Retry the same decision; a durable PM decision will reconcile without rewriting exact copy."
            ),
            "data_policy": {
                "raw_copy_included": False,
                "private_notes_included": False,
                "publication_url_included": False,
                "local_path_included": False,
            },
        }
        result["lifecycle_queue"] = lifecycle_queue
        warnings = list(result.get("warnings") or [])
        warnings.append(lifecycle_queue["warning"])
        result["warnings"] = warnings
        return result
    lifecycle_queue = _queue_owner_review_lifecycle(
        queue_id=queue_id,
        decision=normalized_decision,
        reviewed_at=reviewed_at,
        content_version_sha256=content_version_sha256,
        replay=replay,
        legacy_compatibility=True,
    )
    result["lifecycle_queue"] = lifecycle_queue
    if lifecycle_queue.get("status") == "queue_failed":
        warnings = list(result.get("warnings") or [])
        warnings.append(str(lifecycle_queue.get("warning") or "Private lifecycle queue failed."))
        result["warnings"] = warnings
    return result


def record_owner_decision(
    queue_id: str,
    decision: str,
    notes: str | None = None,
    *,
    legacy_compatibility: bool = False,
) -> dict[str, Any]:
    _require_legacy_owner_review_compatibility(legacy_compatibility)
    item = _find_owner_review_item_for_decision(queue_id)
    if item is None:
        raise LinkedinOwnerReviewNotFoundError(f"Unknown queue item: {queue_id}")
    try:
        return _record_owner_decision_with_lifecycle_for_item(
            item,
            decision,
            notes,
            legacy_compatibility=True,
        )
    except ValueError as exc:
        fallback_item = _find_pm_pending_owner_review_item(queue_id)
        if fallback_item is None or not _is_missing_owner_review_artifact_error(exc):
            raise
        return _record_owner_decision_with_lifecycle_for_item(
            fallback_item,
            decision,
            notes,
            tolerate_missing_artifacts=True,
            legacy_compatibility=True,
        )
