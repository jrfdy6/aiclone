...
def _owner_review_reason(queue_id: str, title: str, decision: str) -> str:
    if decision == "approve":
        return (
...
def _owner_review_card_title(queue_id: str, decision: str) -> str:
    if decision == "approve":
...
def _build_owner_review_card_payload(
    ...
    *,
    ...
    queue_id = str(item.get("queue_id") or "").strip()
    title = str(item.get("title") or queue_id).strip()
    reason = _owner_review_reason(queue_id, title, decision)
...
def _pending_owner_review_card_title(queue_id: str, title: str) -> str:
    return f"Owner review - {queue_id} - {title}"


def _pending_owner_review_reason(queue_id: str, title: str) -> str:
    return (
    ...
    payload.update(
        {
            ...
            "trigger_origin": "owner_review",
            "trigger_key": _owner_review_trigger_key(queue_id),
            "reason": _pending_owner_review_reason(queue_id, title),
            "instructions": [
                f"Review `{queue_id}` in PM Board before any backend worker picks it up.",
                "Read the first-pass draft and owner packet before deciding.",
...
def sync_owner_review_pm_cards() -> dict[str, Any]:
    ...
    for item in payload.get("items", []):
        ...
        card_title = _pending_owner_review_card_title(queue_id, title)
        if existing is None:
...
def _queue_owner_review_followup(
    ...
    *,
    ...
    title = _owner_review_card_title(queue_id, decision)
...
def _update_draft_file(draft_path: Path, decision: str, reviewed_at: str) -> None:
...
def record_owner_decision(queue_id: str, decision: str, notes: str | None = None) -> dict[str, Any]:
    ...
    _update_draft_file(draft_path, decision, reviewed_at)
