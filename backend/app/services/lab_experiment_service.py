...
SOCIAL_STAGE_CATALOG: List[dict[str, Any]] = [
    ...
    },
    {
        ...
        "section": "owner",
        "description": "Choose the belief frame the system thinks the owner would use here.",
        "possible_failures": [
            "belief_missing",
            "belief_overused",
    ...
    {
        "id": "owner_perspective",
        "label": "the owner Perspective",
        "section": "owner",
        "description": "Model what the owner would agree with, push back on, or add from lived experience.",
        "possible_failures": [
            "owner_reaction_not_modeled",
    ...
    },
    {
        ...
        "section": "synthesis",
        "description": "Summarize article view, the owner view, tension, and content angle before drafting.",
        "possible_failures": [
...
def _build_social_probe_stages(
    *,
    ...
    perspective_packets = [row.get("owner_perspective") or {} for row in lane_rows]
    ...
    brief_owner_views = _unique_non_empty([str(packet.get("owner_view") or "") for packet in brief_packets])
    ...
    owner_perspective_scores = [float((row.get("stage_evaluation") or {}).get("owner_perspective_score") or 0.0) for row in lane_rows]
    ...
    owner_missing_fields = []
    if not perspective_reactions:
        owner_missing_fields.append("personal_reaction")
    if not perspective_agree:
        owner_missing_fields.append("agree_with")
    if not perspective_pushback:
        owner_missing_fields.append("pushback")
    if not perspective_lived:
        owner_missing_fields.append("lived_addition")
    stages.append(
        _extend_stage(
            _social_stage_result(
                "owner_perspective",
                ...
                if owner_missing_fields
                else "pass",
                "owner_reaction_missing"
                if not perspective_reactions
                else "lived_reaction_missing"
                if owner_missing_fields
                else "owner_perspective_ready",
                (
                    "The system modeled a the owner perspective packet, but one or more reaction slices are still thin."
                    if perspective_reactions and owner_missing_fields
                    else "The system modeled what the owner agrees with, pushes back on, and adds from lived experience."
                    if perspective_reactions
                    else "The system did not return a usable the owner perspective packet."
                ),
            ),
            evidence=[
            ...
            score=_average(owner_perspective_scores) or _ratio_score(4 - len(owner_missing_fields), 4),
            missing_fields=owner_missing_fields,
        )
    )
    ...
    stages.append(
        _extend_stage(
            _social_stage_result(
                "reaction_brief",
                "fail"
                if not (brief_article_views or brief_owner_views or brief_tensions or brief_content_angles)
                else "warn"
                if not (brief_article_views and brief_owner_views and brief_tensions and brief_content_angles)
                else "pass",
                "reaction_brief_missing"
                if not (brief_article_views or brief_owner_views or brief_tensions or brief_content_angles)
                ...
                (
                    ...
                    if (brief_article_views or brief_owner_views or brief_tensions or brief_content_angles)
                    and not (brief_article_views and brief_owner_views and brief_tensions and brief_content_angles)
                    else "The system materialized article view, the owner view, tension, and content angle before drafting."
                    if (brief_article_views and brief_owner_views and brief_tensions and brief_content_angles)
                    else "The system did not return a usable reaction brief packet."
                ),
            ),
            score=_average(reaction_brief_scores) or _ratio_score(
                sum(1 for value in [brief_article_views, brief_owner_views, brief_tensions, brief_content_angles] if value),
                4,
            ),
            evidence=[
                f"article_view={brief_article_views[0] if brief_article_views else 'missing'}",
                f"owner_view={brief_owner_views[0] if brief_owner_views else 'missing'}",
    ...
    return {
        ...
        "current": {
            ...
            "metric_cards": [
                ...
                _metric_card("owner_perspective_avg", "the owner Perspective", None, "#f97316"),
...
async def run_social_response_matrix_experiment() -> dict[str, Any]:
                ...
                lane_rows.append(
                    {
                        ...
                        "owner_perspective": dict(variant.get("owner_perspective") or {}),
            ...
            results.append(
                {
                    ...
                    "stage_results": [
                        ...
                        _extend_stage(_social_stage_result("owner_perspective", "missing", "probe_error", "The probe failed before the owner perspective modeling completed."), missing_fields=["owner_perspective"]),
    ...
    owner_perspective_avg = round(
        sum(float((row.get("stage_evaluation") or {}).get("owner_perspective_score") or 0.0) for row in all_lane_rows)
        / total_lane_rows,
        1,
    )
    ...
    lived_addition_rate = round(
        (
            sum(1 for row in all_lane_rows if str(((row.get("owner_perspective") or {}).get("lived_addition")) or "").strip())
            / total_lane_rows
        )
        * 100,
        1,
    ...
    record.update(
        {
            ...
            "current": {
                ...
                "metric_cards": [
                    ...
                    _metric_card("owner_perspective_avg", "the owner Perspective", owner_perspective_avg, "#f97316"),
