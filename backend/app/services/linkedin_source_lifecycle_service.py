...
STAGE_CONFIG: dict[str, dict[str, Any]] = {
    ...
    "published": {
        ...
        "reason": "The post has a validated publication-confirmation event with its live LinkedIn URL and timestamp.",
    },
}
    ...
    if candidate_priority >= current_priority:
        for field in ("stage", "reason", "primary_surface", "primary_action", "visibility", "next_action_label", "stage_label"):
            if candidate.get(field):
                existing[field] = candidate[field]
        for field in ("scheduled_at", "published_at", "publication_id", "release_packet_path", "receipt_path"):
            if candidate.get(field):
                existing[field] = candidate[field]
...
def _add_confirmed_publications(
    items_by_key: dict[str, dict[str, Any]],
    key_index: dict[str, str],
    *,
    linkedin_root: Path,
    publication_records: list[dict[str, Any]] | None,
) -> None:
    """Project only validated, content-minimized publication confirmations."""

    for record in publication_records or []:
        if not isinstance(record, dict):
            continue
        queue_id = _clean(record.get("content_id")).upper()
        publication_id = _clean(record.get("publication_id"))
        publication_url = _clean(record.get("publication_url"))
        published_at = _clean(record.get("published_at"))
        if not queue_id or not publication_id or not publication_url or not published_at:
            continue
        try:
        ...
        if parsed_timestamp.tzinfo is None or parsed_timestamp.utcoffset() is None:
            continue
        normalized_url = _normalize_url(publication_url)
        if not normalized_url.startswith(("https://linkedin.com/posts/", "https://www.linkedin.com/posts/", "https://linkedin.com/feed/update/", "https://www.linkedin.com/feed/update/")):
            continue
        _merge_item(
            items_by_key,
            key_index,
            _candidate(
                ...
                source_kind="publication_confirmation",
                published_at=parsed_timestamp.astimezone(timezone.utc).isoformat(),
                publication_id=publication_id,
                reason="A validated publication-confirmation event proves this exact item is live on LinkedIn.",
                evidence={
                    "publication_id": publication_id,
                    "publication_url": publication_url,
                    "published_at": parsed_timestamp.astimezone(timezone.utc).isoformat(),
                    "confirmation_schema": "linkedin_publication_event/v1",
                },
            ),
        )
