...
def database_configured() -> bool:
    """Return whether this process is expected to serve durable Brain state."""

    return bool(_get_conninfo())
...
_BASE_SCHEMA_STATEMENTS = (
    ...
    """
    CREATE TABLE IF NOT EXISTS neo_guest_invites (
        id UUID PRIMARY KEY, label TEXT NOT NULL, code_digest TEXT NOT NULL UNIQUE,
        status TEXT NOT NULL DEFAULT 'active', expires_at TIMESTAMPTZ,
        created_at TIMESTAMPTZ DEFAULT NOW(), revoked_at TIMESTAMPTZ
    )
    """,
    "CREATE INDEX IF NOT EXISTS neo_guest_invites_status_idx ON neo_guest_invites(status, expires_at)",
    """
    CREATE TABLE IF NOT EXISTS neo_guest_sessions (
        id UUID PRIMARY KEY, invite_id UUID NOT NULL REFERENCES neo_guest_invites(id) ON DELETE RESTRICT,
        token_digest TEXT NOT NULL UNIQUE, visitor_name TEXT, visitor_email TEXT, visitor_phone TEXT,
        status TEXT NOT NULL DEFAULT 'active', created_at TIMESTAMPTZ DEFAULT NOW(), last_seen_at TIMESTAMPTZ DEFAULT NOW()
    )
    """,
    "CREATE INDEX IF NOT EXISTS neo_guest_sessions_invite_idx ON neo_guest_sessions(invite_id, status, last_seen_at DESC)",
    """
    CREATE TABLE IF NOT EXISTS neo_guest_messages (
        id UUID PRIMARY KEY, session_id UUID NOT NULL REFERENCES neo_guest_sessions(id) ON DELETE CASCADE,
        role TEXT NOT NULL, content TEXT NOT NULL, created_at TIMESTAMPTZ DEFAULT NOW()
    )
    """,
    "CREATE INDEX IF NOT EXISTS neo_guest_messages_session_idx ON neo_guest_messages(session_id, created_at)",
    """
    CREATE TABLE IF NOT EXISTS neo_guest_jobs (
        id UUID PRIMARY KEY, session_id UUID NOT NULL REFERENCES neo_guest_sessions(id) ON DELETE CASCADE,
        message_id UUID NOT NULL REFERENCES neo_guest_messages(id) ON DELETE CASCADE,
        client_request_id UUID,
        status TEXT NOT NULL DEFAULT 'pending', context_packet JSONB NOT NULL DEFAULT '{}'::jsonb,
        claimed_by TEXT, claim_token TEXT, terminal_claim_token_digest TEXT,
        claimed_at TIMESTAMPTZ, lease_expires_at TIMESTAMPTZ,
        completed_at TIMESTAMPTZ, failed_at TIMESTAMPTZ,
        partial_response TEXT, model_started_at TIMESTAMPTZ, first_token_at TIMESTAMPTZ, progress_at TIMESTAMPTZ,
        error_message TEXT, result_message_id UUID REFERENCES neo_guest_messages(id) ON DELETE SET NULL,
        created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW()
    )
    """,
    "ALTER TABLE neo_guest_jobs ADD COLUMN IF NOT EXISTS partial_response TEXT",
    "ALTER TABLE neo_guest_jobs ADD COLUMN IF NOT EXISTS model_started_at TIMESTAMPTZ",
    "ALTER TABLE neo_guest_jobs ADD COLUMN IF NOT EXISTS first_token_at TIMESTAMPTZ",
    "ALTER TABLE neo_guest_jobs ADD COLUMN IF NOT EXISTS progress_at TIMESTAMPTZ",
    ...
    "CREATE INDEX IF NOT EXISTS neo_guest_jobs_status_idx ON neo_guest_jobs(status, created_at)",
    "CREATE INDEX IF NOT EXISTS neo_guest_jobs_claim_idx ON neo_guest_jobs(status, lease_expires_at, created_at)",
    "CREATE UNIQUE INDEX IF NOT EXISTS neo_guest_jobs_session_client_request_uidx ON neo_guest_jobs(session_id, client_request_id)",
    """
    CREATE TABLE IF NOT EXISTS neo_meeting_requests (
        id UUID PRIMARY KEY, session_id UUID NOT NULL REFERENCES neo_guest_sessions(id) ON DELETE CASCADE,
        client_request_id UUID, request_fingerprint TEXT,
        visitor_name TEXT NOT NULL, visitor_email TEXT NOT NULL, visitor_phone TEXT NOT NULL,
        purpose TEXT NOT NULL, preferred_times JSONB NOT NULL DEFAULT '[]'::jsonb, timezone TEXT NOT NULL,
        status TEXT NOT NULL DEFAULT 'pending', owner_notes TEXT,
...
def get_pool() -> ConnectionPool:
    ...
    with _pool_lock:
        ...
        candidate = ConnectionPool(conninfo=conninfo, min_size=min_size, max_size=max_size)
        try:
