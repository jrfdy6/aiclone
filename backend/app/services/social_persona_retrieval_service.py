...
def _belief_claim_type_bonus(
    ...
    *,
    ...
    if normalized == "factual":
        if lane_id in {"ops-pm", "program-leadership", "current-role"} and any(
            token in lowered
            for token in ("platform implementation", "systems migration", "measurable outcome")
        ):
            return 0.7
        return -0.8
    return 0.0
...
def _generic_belief_penalty(
    ...
    *,
    ...
    if "intersection of education, ai systems, entrepreneurship, and style" in lowered and lane_id != "entrepreneurship":
        penalty += 2.6
    if "current-role title" in lowered and lane_id not in {"admissions", "current-role", "enrollment-management"}:
        penalty += 1.6
    if any(token in lowered for token in ("annual revenue portfolio", "system migration", "multi-system migration")) and lane_id not in {"ops-pm", "program-leadership", "current-role"}:
        penalty += 1.4
    if "private identity experience" in lowered and lane_id not in {"therapy", "referral", "admissions", "personal-story", "current-role"}:
        penalty += 0.8
...
def _experience_article_fit_bonus(
    ...
    *,
    ...
    if any(token in article_text for token in ("dashboard", "reporting", "visibility", "referral", "outreach", "meetings", "priorities")):
        if any(token in lowered for token in ("dashboard", "visibility", "reporting", "market development", "community event", "referral")):
            score += 3.0
    if any(token in article_text for token in ("family", "families", "student", "students", "education", "admissions", "trust", "school")):
        if any(token in lowered for token in ("community event", "market development", "family", "families", "student", "students", "trust")):
            score += 3.0
    ...
    if lane_id == "ai" and any(token in lowered for token in ("ai constraint breakthrough", "if you cannot rely on the output", "fail to constrain")):
        score += 1.8
    if lane_id in {"admissions", "current-role", "enrollment-management"} and any(token in lowered for token in ("community event", "market development", "admissions", "enrollment")):
        score += 1.4
...
def _generic_experience_penalty(
    ...
    *,
    ...
    if "ai clone / brain system" in lowered and lane_id != "ai" and not any(
        ...
        penalty += 2.1
    if "community event" in lowered and lane_id not in {"admissions", "current-role", "enrollment-management", "referral", "therapy"}:
        penalty += 1.4
    if "best practices initiative" in lowered and not any(
        ...
        penalty += 1.2
    if "employer market development" in lowered and lane_id not in {"admissions", "current-role", "enrollment-management", "referral"}:
        penalty += 1.0
    return penalty
    ...
    if target_kind == "belief":
        ...
        if lane_id in {"admissions", "current-role", "enrollment-management"} and any(
            ...
            for token in (
                ...
                "education changed owner's life",
            )
        ):
            score += 2.3
        if lane_id == "entrepreneurship" and any(
        ...
        if lane_id == "ai" and any(token in lowered for token in ("ai clone", "grounded content generation", "brain system")):
            score += 4.2
        if lane_id in {"admissions", "current-role", "enrollment-management"} and any(token in lowered for token in ("market development", "community event", "family", "student", "trust")):
            score += 2.4
        if lane_id == "program-leadership" and any(token in lowered for token in ("best practices", "community event", "visibility", "team")):
            score += 5.0
    ...
    def retrieve(self, signal: dict[str, Any], lane_id: str, article_understanding: dict[str, Any]) -> dict[str, Any]:
        ...
        if lane_id in {"admissions", "current-role", "enrollment-management"}:
            relational_experience = _first_candidate_matching_text(
                experience_candidates,
                ("market development", "community event", "family", "student", "trust"),
                excluded_title=_normalize_inline_text((selected_belief or {}).get("title")),
            )
