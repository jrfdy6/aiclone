...
def _thread_from_gmail_payload(payload: dict, account_email: str) -> EmailThread:
    ...
    return EmailThread(
        ...
        provider_labels=[
            ...
            for label in (payload.get("_label_names") or payload.get("_openclaw_label_names") or [])
            if str(label).strip()
        ],
    ...
    samples = [
        {
            ...
            "from_name": "Example University Procurement",
            "organization": "Example University",
            "to_addresses": ["portfolio.universities@example.com"],
            ...
            "messages": [
                inbound_message(
                    ...
                    "portfolio.universities@example.com",
                    42,
                    "Example University Procurement",
                )
            ],
        },
        {
            "subject": "Reseller onboarding follow-up",
            "from_address": "partners@example.com",
            "from_name": "Example Vendor Partner Relations",
            "organization": "Example Vendor",
            "to_addresses": ["portfolio.vendors@example.com"],
            "provider_labels": ["workspace/agc"],
            "messages": [
                inbound_message(
                    ...
                    "portfolio.vendors@example.com",
                    115,
                    "Example Vendor Partner Relations",
                )
            ],
        },
        {
            ...
            "to_addresses": ["portfolio.education@example.com"],
            "messages": [
                inbound_message(
                    ...
                    "portfolio.education@example.com",
                    205,
                    "Parent Intake",
                )
            ],
        },
        {
            ...
            "from_name": "Example Podcast Host",
            "organization": "Example Podcast",
            "to_addresses": ["portfolio.content@example.com"],
            "messages": [
                inbound_message(
                    "Podcast invitation and speaking collaboration",
                    "Would the owner be open to a podcast conversation about creator systems, public execution, and operator-led visibility?",
                    "host@example.com",
                    "portfolio.content@example.com",
                    320,
        ...
        },
        {
            ...
            "from_name": "Example Software Vendor Billing",
            "organization": "Example Software Vendor",
            "to_addresses": ["portfolio.ops@example.com"],
            "messages": [
                inbound_message(
                    ...
                    "billing@example.com",
                    "portfolio.ops@example.com",
                    18,
                    "Example Software Vendor Billing",
                )
            ],
        },
        {
            "subject": "Need help improving procurement workflow visibility",
            "from_address": "operations@example.com",
            "from_name": "Example College Operations",
            "organization": "Example College",
            "to_addresses": ["portfolio@example.com"],
            "messages": [
                inbound_message(
...
def _build_thread_list_response(
    threads: list[EmailThread],
...
def list_persisted_threads(
    ...
    """List the persisted snapshot without provider I/O, seeding, or writes."""
    return _build_thread_list_response(
        _read_threads(),
...
def get_persisted_thread(thread_id: str) -> Optional[EmailThread]:
    """Read one thread from persisted state without provider or state mutations."""
    for thread in _read_threads():
...
async def _run_content_generation_for_thread(
    ...
    *,
    ...
    request = ContentGenerationRequest(
        user_id=str(request_payload.get("user_id") or "owner"),
...
def _queue_codex_job_for_thread(
    ...
    *,
    ...
    job = create_codex_job(
        ...
        requested_by=str(request_payload.get("user_id") or "owner"),
