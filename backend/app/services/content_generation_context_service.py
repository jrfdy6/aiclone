...
from app.services.feezie_positioning_contract_service import (
    build_feezie_generation_disallowed_moves,
...
def _hydrate_content_safe_operator_lesson(item: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "metadata": {
            ...
            # These fields are already produced by the public-safe lesson distiller.
            # Keeping them typed lets the generation admission gate test for lived
            # specificity without reopening raw operator records in Railway or the UI.
...
def retrieve_content_safe_operator_lesson_chunks(
    *,
    ...
    category = normalize_feezie_intent(category, default="value")
    ...
    for raw_lesson in lessons:
        ...
        if category == "value":
            composite += 2
        elif category == "invitation" and any(token in {"trust", "customer", "proof", "results"} for token in tag_tokens):
            composite += 2
    ...
    *,
    ...
    for raw_item in items:
        ...
        if category == "value" and memory_role == "proof":
            composite += 4
        if category == "invitation" and reservoir_lane in {"proof_point", "post_seed"}:
            composite += 3
...
def _merge_content_signal_chunks(
    content_reservoir_chunks: list[dict[str, Any]],
    content_safe_operator_lesson_chunks: list[dict[str, Any]],
    *,
    limit: int,
) -> list[dict[str, Any]]:
    """Blend ranked source lanes without allowing either lane to hide the other."""
    merged: list[dict[str, Any]] = []
    seen: set[str] = set()
    largest_group = max(len(content_reservoir_chunks), len(content_safe_operator_lesson_chunks))
    for index in range(largest_group):
        for group in (content_reservoir_chunks, content_safe_operator_lesson_chunks):
            if index >= len(group):
                continue
            item = group[index]
            key = _normalized_chunk_key(item)
...
def _build_disallowed_moves(*, audience: str, grounding_mode: str) -> list[str]:
    return build_feezie_generation_disallowed_moves(
        audience=audience,
...
def build_content_generation_context(
    *,
    ...
    if normalized_source_mode in {"reservoir_ranked", "recent_signals", "selected_source"}:
        ...
        if content_reservoir_chunks and content_safe_operator_lesson_chunks:
            retrieval_source = "content_reservoir+content_safe_operator_lessons"
        elif content_reservoir_chunks:
            retrieval_source = "content_reservoir"
        elif content_safe_operator_lesson_chunks:
            retrieval_source = "content_safe_operator_lessons"
        retrieved_persona_chunks = content_signal_chunks
    ...
    print(
        ...
        f"signals={len(content_signal_chunks)} "
    ...
    if include_audit:
        ...
        audit = {
            ...
            "retrieval": {
                ...
                "content_signal_candidates": _serialize_chunk_group(content_signal_chunks, limit=8),
    ...
    return ContentGenerationContext(
        ...
        public_safe_proof_packets=public_safe_proof_packets,
        content_release_policy=content_release_policy,
        content_signal_chunks=content_signal_chunks,
        content_signal_source=retrieval_source,
