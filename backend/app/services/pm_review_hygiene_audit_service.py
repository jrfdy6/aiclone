...
def record_review_hygiene_audit(result: dict[str, Any]) -> dict[str, Any] | None:
    ...
    try:
        ...
        # Review progression is operationally authoritative; the local audit
        # mirror is useful evidence, but it must never make that progression
        # fail when the runtime filesystem is temporarily read-only.
        return None
    return entry
