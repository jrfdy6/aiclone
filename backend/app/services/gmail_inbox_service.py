...
def gmail_send_enabled() -> bool:
    # Shared inbox access is permanently read/compose-only. The separately
    # governed City Event Scout has its own local token and dedicated sender.
    return False
...
def fetch_gmail_threads() -> list[dict[str, Any]]:
    ...
    for thread_ref in thread_refs:
        ...
        item["_label_names"] = sorted(label_names)
        items.append(item)
    return items
