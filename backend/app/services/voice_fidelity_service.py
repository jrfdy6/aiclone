...
def _select_voice_exemplars(
    ...
    *,
    ...
    if not entries or limit <= 0:
        return [], {"mode": "none", "embedding_model": None}
    lexical_scores = _bm25_scores(entries, query)
    ...
    if semantic_requested and query.strip():
        embedded = _ollama_embeddings(
            [
                query,
                *[_clean_text(entry.get("text")) for entry in entries],
            ]
        )
        if embedded and len(embedded) == len(entries) + 1:
    ...
    *,
    ...
    return "\n\n".join(
        [
            ...
            "- The examples below are verified owner-written or owner-edited posts. Learn their range, paragraph rhythm, directness, and way of taking a position.",
            "- Treat every fact, name, event, organization, date, metric, and story inside an example as forbidden source material unless the approved topic/proof anchors independently provide it.",
            "- Match the voice pattern, not exact wording. Do not copy an eight-word sequence.",
            "- Do not force a catchphrase, slogan, rhetorical question, emoji, hashtag, or fragment just because one appears in an example.",
            "- Choose the example mode that fits the current intent. The owner does not sound identical in an event post, a life update, and direct commentary.",
            "- Prefer natural spoken clarity over generic LinkedIn polish or internal system jargon.",
...
def build_voice_context(
    *,
    ...
    owner_prompt = render_voice_reference_prompt(selected, fingerprint=fingerprint)
