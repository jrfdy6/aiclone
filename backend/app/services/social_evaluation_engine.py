...
class SocialEvaluationEngine:
    ...
    def evaluate_variant(
        ...
        *,
        ...
        owner_perspective: dict[str, Any] | None = None,
        ...
        role_safety_score = {"safe": 9.2, "review": 6.4, "risky": 3.8, "blocked": 1.0}.get(
            role_safety_value,
            6.0,
        )
        ...
        owner_perspective_score = float(stage_evaluation.get("owner_perspective_score") or 0.0)
        ...
        if not (persona_retrieval.get("relevant_claims") or persona_retrieval.get("top_candidates")):
            ...
            genericity_penalty += 0.6
        if not normalize_inline_text(owner_perspective.get("personal_reaction")):
            warnings.append("owner perspective is thin")
            genericity_penalty += 0.6
        ...
        if response_type_packet.get("response_type") == "personal_story" and not normalize_inline_text(owner_perspective.get("lived_addition")):
            warnings.append("personal story type lacks lived proof")
        if response_type_packet.get("response_type") == "contrarian" and not normalize_inline_text(owner_perspective.get("pushback")):
        ...
        return {
            ...
            "owner_perspective_score": round_score(clamp(owner_perspective_score, 0.0, 10.0)),
