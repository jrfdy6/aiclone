...
def _selectable_fragment_metadata(
    *,
    fragment_text: str,
    route: GeneratedFragmentRoute,
    evidence: str | None,
) -> dict[str, Any]:
    """Expose one generated fragment through the existing Brain review controls."""
    if route.kind == "framework":
        return {
            "frameworks": [
                {
                    "title": route.label,
                    "takeaway": fragment_text,
                    "evidence": evidence,
                }
            ]
        }
    if route.kind == "anecdote":
        return {
            "anecdotes": [
                {
                    "title": route.label,
                    "summary": fragment_text,
                    "evidence": evidence,
                }
            ]
        }
    if route.kind == "phrase_candidate":
        return {"phrase_candidates": [fragment_text]}
    if route.kind == "stat":
        return {"stats": [fragment_text]}
    return {"talking_points": [fragment_text]}
...
def promote_generated_fragment(
    *,
    ...
    cleaned_option = _trim_fragment(option_text)
    option_label = f"Generated FEEZIE OS option {option_index + 1}" if option_index is not None else "Generated FEEZIE OS option"
    source_excerpt = cleaned_option if cleaned_option.lower() != cleaned_fragment.lower() else f"{option_label} selected for owner review."
    metadata = {
        ...
        "generated_option_text": cleaned_option,
        "generated_option_index": option_index,
        "generated_option_brief": option_brief or {},
        "source_excerpt_clean": source_excerpt,
        "source_context_excerpt": cleaned_option or cleaned_fragment,
        ...
        "submitted_for_owner_review_at": now,
        **_selectable_fragment_metadata(
            fragment_text=cleaned_fragment,
            route=route,
            evidence=evidence,
        ),
    ...
    proposal = persona_delta_service.update_delta(
        delta.id,
        PersonaDeltaUpdate(
            status="in_review",
            metadata={
                **metadata,
                "proposed_promotion_items": [promotion_item],
                "selected_promotion_items": [],
                "selected_promotion_item_ids": [],
                "selected_promotion_count": 0,
                "promotion_state": "awaiting_owner_review",
            },
        ),
    )
    queued = proposal or delta
    ...
    return {
        ...
        "target_file": route.target_file,
        "target_label": route.target_label,
        "delta": annotate_for_brain_queue(queued).model_dump(mode="json"),
        "written_files": [],
        "message": (
            f'Already queued in Brain for owner review before {route.target_label} can change.'
            if existing is not None
            else f'Queued in Brain for owner review before {route.target_label} can change.'
        ),
    }
...
def undo_generated_fragment_promotion(*, delta_id: str) -> dict[str, Any]:
    ...
    if delta.status != "committed":
        ...
        updated = persona_delta_service.update_delta(
            ...
            PersonaDeltaUpdate(
                ...
                metadata={
                    ...
                    "proposal_withdrawn_at": reverted_at,
                    "reverted_target_files": [],
        ...
        return {
            ...
            "preserved_target_files": [],
            "message": "Removed from Brain review. Canon was unchanged.",
        }
