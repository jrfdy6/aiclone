...
Runtime authority: [SOURCE_OF_TRUTH.md](../SOURCE_OF_TRUTH.md). The active
system is Codex-native: Railway provides the authenticated remote control
plane, repository-owned `launchd` jobs schedule work on the Mac, Codex CLI
executes bounded packets with the saved ChatGPT login, and private runtime
state lives under `/Users/neo/.codex/ai-clone`. OpenClaw, Discord, and QMD are
...
- `scripts/scheduled_automation_runtime.py`
- `SOPs/codex_native_local_automation_sop.md`
...
| `scripts/*.py`, repository launchd jobs, Codex/workspace runners        |
...
| - SQLite FTS5       local durable-memory search index                    |
| - private runtime   secrets, logs, ledgers under `~/.codex/ai-clone`     |
|                                                                          |
...
+-----------------------------+-------------------------------+------------+
                              ...
                              v                               v
+-----------------------------+-----------------+   +----------------------+
| Authenticated backend API (FastAPI)           |   | Background services   |
...
+-----------------------------+-----------------+   +-----------+----------+
                              ...
                              +---------------+-----------------+
                                              ...
                                              v
+------------------------------------------------------------------------+
| Authenticated frontend surfaces (Next.js)                               |
...
| - legacy product routes may remain, but are not active control surfaces |
+------------------------------------------------------------------------+
...
Codex Chronicle bridge:
- `memory/codex_session_handoff.jsonl`
- local maintenance jobs and standups read this before acting

Remote requests never execute directly in the browser or Railway process.
The authenticated frontend proxies to the authenticated backend, the backend
queues a signed and path-bounded packet, and an allowlisted local runner claims
that packet. Local logs and ledgers remain authoritative if Railway is
...
- `scripts/**`
  You are changing local automations, `launchd` jobs, scheduled flows, or runners.
...
FLOW B: Codex live work -> Chronicle -> maintenance -> PM/standup

`live work session`
  -> high-delta Chronicle chunk in `memory/codex_session_handoff.jsonl`
  -> repository-scheduled maintenance read
...
1. Brain signal / review object
   ...
   Writers:
   source intelligence, workspace execution, Chronicle, scheduled-job outputs, owner reactions
   Readers:
