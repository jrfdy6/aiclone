...
> Authority: supporting architecture subordinate to [SOURCE_OF_TRUTH.md](../SOURCE_OF_TRUTH.md). Observed production state overrides design claims in this reference.

This document defines how the AI Clone brain should work across:
- Codex
- repository-owned `launchd` jobs
- the authenticated Railway control plane
- standups
- PM board
- workspace agents
...
Reference:
- [brain_workspace_exchange_protocol.md](./brain_workspace_exchange_protocol.md)
...
A generic `Brain Signal` is the review object for source, workspace, scheduled-job, Chronicle, PM, or meeting signal that is not automatically persona canon. Persona deltas remain a downstream identity lane, not the whole Brain substrate.
...
- Codex live work
- authenticated Railway requests and owner review
- context pruning / flush cycles
- local scheduled-job outputs
- meeting transcripts
...
- [cron-prune.md](../memory/cron-prune.md) (compatibility-named scheduled-job journal)
...
- [brain_canonical_memory_sync_contract.md](./brain_canonical_memory_sync_contract.md)
...
- repository-scheduled maintenance jobs
- standups
- PM board
...
- scheduled jobs are digestion and maintenance helpers
...
Reference:
- [chronicle_pm_promotion_boundary.md](./chronicle_pm_promotion_boundary.md)
...
## 4. Scheduled Automation Intertwining

Scheduled automations should not become a second brain.
...
4. No scheduled job should behave like a separate planner.
...
1. ingest Codex, authenticated control-plane, scheduled-job, and source-intelligence signal
2. classify by workspace, durability, and actionability
3. write Chronicle or source-intelligence digests
4. let repository-scheduled jobs consolidate durable truth
...
`intake -> digestion -> canonical memory -> scheduled consolidation -> standup decision -> PM truth -> workspace execution -> write-back`
