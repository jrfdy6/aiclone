...
# Tests all endpoints with new API keys

RAILWAY_URL="https://aiclone-production-32dc.up.railway.app"
USER_ID="test-railway-$(date +%s)"
...
# Test 2: Research Trigger (Perplexity)
echo "2️⃣ Testing Research Trigger (Perplexity API)..."
response=$(curl -s -w "\n%{http_code}" -X POST "$RAILWAY_URL/api/research/trigger" \
...
# Test 3: Prospect Discovery (Google Custom Search + Firecrawl)
echo "3️⃣ Testing Prospect Discovery (Google Search + Firecrawl)..."
response=$(curl -s -w "\n%{http_code}" -X POST "$RAILWAY_URL/api/prospects/discover" \
  -H "Content-Type: application/json" \
  -d "{
    \"user_id\": \"$USER_ID\",
    \"industry\": \"SaaS\",
...
# Test 4: Prospect Approval
if [ ! -z "$PROSPECT_ID" ]; then
    echo "4️⃣ Testing Prospect Approval..."
    response=$(curl -s -w "\n%{http_code}" -X POST "$RAILWAY_URL/api/prospects/approve" \
      -H "Content-Type: application/json" \
      -d "{
        \"user_id\": \"$USER_ID\",
...
if [ ! -z "$PROSPECT_ID" ]; then
    ...
    response=$(curl -s -w "\n%{http_code}" -X POST "$RAILWAY_URL/api/prospects/score" \
      -H "Content-Type: application/json" \
      -d "{
        \"user_id\": \"$USER_ID\",
        \"prospect_ids\": [\"$PROSPECT_ID\"]
...
if [ ! -z "$PROSPECT_ID" ]; then
    ...
    response=$(curl -s -w "\n%{http_code}" -X POST "$RAILWAY_URL/api/outreach/manual/prompts/generate" \
      -H "Content-Type: application/json" \
      -d "{
...
response=$(curl -s -w "\n%{http_code}" "$RAILWAY_URL/api/metrics/current?user_id=$USER_ID&period=weekly")
http_code=$(echo "$response" | tail -1)
body=$(echo "$response" | head -1)
...
# Test 8: Learning Patterns
echo "8️⃣ Testing Learning Patterns..."
response=$(curl -s -w "\n%{http_code}" "$RAILWAY_URL/api/learning/patterns?user_id=$USER_ID&limit=10")
http_code=$(echo "$response" | tail -1)
body=$(echo "$response" | head -1)
...
echo "======================================"
echo "✅ Railway Production Testing Complete!"
echo ""
