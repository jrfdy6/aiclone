...
export type OpsWorkspaceContextArtifactProjection = {
  kind?: 'analytics' | 'execution_log' | 'briefing' | 'dispatch';
  title?: string;
  summary?: string;
  reference?: string;
  ...
  source_state?: 'verified_preexisting' | 'changed_since_cycle';
};

export type OpsWorkspaceContextItemProjection = {
  workspace_key?: string;
  display_name?: string;
  state?: 'consumed' | 'partial' | 'unavailable';
  reason_codes?: string[];
  current_focus?: {
    title?: string;
    status?: string;
  ...
  artifacts?: OpsWorkspaceContextArtifactProjection[];
};

export type OpsWorkspaceContextProjection = {
  schema_version?: 'ops_workspace_context_projection/v1';
  generated_at?: string;
  observed_at?: string | null;
  state?: 'ready' | 'degraded' | 'unavailable';
  reason_codes?: string[];
  cycle_id?: string | null;
  workspaces?: OpsWorkspaceContextItemProjection[];
};
...
export function projectOpsWorkspaceContext(
  projection: OpsWorkspaceContextProjection | null | undefined,
  requestedWorkspaceKey: string,
): OpsWorkspaceContextItemProjection | null {
  if (
    projection?.schema_version !== 'ops_workspace_context_projection/v1'
    || !['ready', 'degraded'].includes(projection.state ?? '')
    || !Array.isArray(projection.workspaces)
  ) {
    return null;
  }
  const workspaceKey = normalizeOpsWorkspaceKey(requestedWorkspaceKey);
  const row = projection.workspaces.find(
    (item) => normalizeOpsWorkspaceKey(item?.workspace_key) === workspaceKey,
  );
  if (!row || !['consumed', 'partial', 'unavailable'].includes(row.state ?? '')) {
    return null;
  }
  const artifactKinds = new Set(['analytics', 'execution_log', 'briefing', 'dispatch']);
  const sourceStates = new Set(['verified_preexisting', 'changed_since_cycle']);
  const artifacts = Array.isArray(row.artifacts)
    ? row.artifacts.flatMap((artifact) => {
        const title = ownerSafeOpsText(artifact?.title, 180);
        if (
          !artifact
          || !artifactKinds.has(artifact.kind ?? '')
          || artifact.consumption_role !== 'reference_only'
          || !sourceStates.has(artifact.source_state ?? '')
          || !title
        ) {
          return [];
        }
        return [{
          kind: artifact.kind,
          title,
          summary: ownerSafeOpsText(artifact.summary, 480),
          source_updated_at: typeof artifact.source_updated_at === 'string' ? artifact.source_updated_at : undefined,
          consumption_role: 'reference_only' as const,
          source_state: artifact.source_state,
        }];
      })
    : [];
  const focusTitle = row.current_focus?.source_kind === 'canonical_pm_snapshot'
    ? ownerSafeOpsText(row.current_focus?.title, 220)
    : '';
  return {
    workspace_key: workspaceKey,
    display_name: displayName(workspaceKey, row.display_name),
    state: row.state,
    reason_codes: Array.isArray(row.reason_codes)
      ? row.reason_codes.map((reason) => ownerSafeOpsText(reason, 200)).filter(Boolean).slice(0, 10)
      : [],
    current_focus: focusTitle
      ? {
          title: focusTitle,
          status: ownerSafeOpsText(row.current_focus?.status, 80),
          source_kind: 'canonical_pm_snapshot',
        }
      : null,
    artifacts,
