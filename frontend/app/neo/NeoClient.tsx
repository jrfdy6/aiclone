...
const welcome = "I'm Neo, Johnnie's AI assistant. I can answer questions about his professional experience, projects, and the way he works. I can also help you request a 15-minute coffee chat—Johnnie reviews every request before anything is booked.";
...
export default function NeoClient() {
  ...
  if (!ready) return (
    <main className={styles.shell}><section className={`${styles.accessCard} ${responsiveStyles.accessCard}`}>
      <div className={styles.mark}>N</div><p className={styles.eyebrow}>A private introduction</p>
      <h1>Meet Neo</h1><p>Johnnie Fields&apos; AI assistant for professional conversations.</p>
      ...
      <small>Invite access is separate from Johnnie&apos;s private AI Clone dashboards.</small>
    </section></main>
  );

  return <main className={styles.shell}><section className={`${styles.chatCard} ${responsiveStyles.chatCard}`}>
    <header><div><p className={styles.eyebrow}>Johnnie Fields</p><h1>Neo</h1></div><span className={styles.status}><i /> Local AI assistant</span></header>
    ...
    {error && <p className={styles.error} role="alert">{error}</p>}
    <form className={styles.composer} onSubmit={send}><textarea aria-label="Ask Neo a question" value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="Ask about Johnnie's experience, projects, or approach…" rows={2} /><button type="button" className={styles.mic} onClick={toggleVoice} aria-pressed={listening}>{listening ? 'Stop' : 'Talk'}</button><button disabled={busy || Boolean(activeResponse) || !draft.trim()}>Send</button></form>
    ...
    {meetingOpen && (meetingSent ? <div id="neo-meeting-panel" ref={meetingConfirmationRef} className={styles.confirmation} role="status" aria-live="polite" aria-atomic="true" tabIndex={-1}><strong>Request sent.</strong><p>Johnnie will review it before anything is booked.</p></div> : <form id="neo-meeting-panel" className={styles.meeting} onSubmit={requestMeeting}><h2>Let Johnnie buy you a coffee</h2><p>Share a few options. This sends an approval request; it does not book the meeting.</p><div className={styles.grid}><label>Name<input name="name" required /></label><label>Email<input name="email" type="email" required /></label><label>Phone<input name="phone" type="tel" required /></label><label>Timezone<input name="timezone" placeholder="America/New_York" required /></label></div><label>Preferred dates and times<textarea name="times" placeholder="Tuesday 2–4 PM; Thursday after 11 AM" required /></label><label>What would you like to discuss?<textarea name="purpose" required /></label><button disabled={busy}>Send for Johnnie&apos;s approval</button></form>)}
  </section></main>;
}
