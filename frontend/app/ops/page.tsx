...
const AI_CLONE_PROJECT_ROOT = resolveWorkspaceRoot();
const AI_CLONE_MEMORY_ROOT = path.join(AI_CLONE_PROJECT_ROOT, 'memory');
...
function loadWorkspaceFiles(): WorkspaceFile[] {
  const roots = [
    { root: path.join(AI_CLONE_PROJECT_ROOT, 'knowledge/persona/feeze'), label: 'persona-bundle' },
    { root: path.join(AI_CLONE_PROJECT_ROOT, 'workspaces/linkedin-content-os'), label: 'linkedin-content-os' },
    { root: path.join(AI_CLONE_PROJECT_ROOT, 'workspaces'), label: 'project-workspaces' },
  ...
  return roots
    .flatMap(({ root, label }) =>
      walk(root)
        ...
        .sort()
        .map((filePath) => {
          const relativePath = path.relative(AI_CLONE_PROJECT_ROOT, filePath).replace(/\\/g, '/');
...
function loadDocEntries(): DocReference[] {
  ...
  return targets
    .map((relativePath) => path.join(AI_CLONE_PROJECT_ROOT, relativePath))
    .filter((fullPath) => fs.existsSync(fullPath))
    .map((fullPath) => {
      const raw = fs.readFileSync(fullPath, 'utf-8');
      const stat = fs.statSync(fullPath);
      const relativePath = path.relative(AI_CLONE_PROJECT_ROOT, fullPath).replace(/\\/g, '/');
      const readOrder = PINNED_DOC_PATHS.indexOf(relativePath);
      return {
        ...
        content: raw,
        updatedAt: stat.mtime.toISOString(),
        readOrder: readOrder >= 0 ? readOrder : null,
...
function loadChronicleEntries(): ChronicleEntry[] {
  const handoffPath = path.join(AI_CLONE_MEMORY_ROOT, 'codex_session_handoff.jsonl');
...
function loadStandupPreps(): StandupPrepPacket[] {
  const prepRoot = path.join(AI_CLONE_MEMORY_ROOT, 'standup-prep');
...
function loadPmRecommendations(): PMRecommendationPacket[] {
  const recommendationsRoot = path.join(AI_CLONE_MEMORY_ROOT, 'pm-recommendations');
...
function loadExecutiveArtifacts({
  ...
  maybePushArtifact(
    ...
    buildFileArtifact({
      ...
      category: 'codex',
      filePath: path.join(AI_CLONE_MEMORY_ROOT, 'codex_session_handoff.jsonl'),
  ...
  maybePushArtifact(
    ...
    buildFileArtifact({
      ...
      category: 'memory',
      filePath: path.join(AI_CLONE_MEMORY_ROOT, 'cron-prune.md'),
      summary: summarize(snippetFromTail(path.join(AI_CLONE_MEMORY_ROOT, 'cron-prune.md')), 160),
      detail: `${countOccurrences(path.join(AI_CLONE_MEMORY_ROOT, todayLogFileName()), '## Context Flush')} context flush checkpoints logged today.`,
    }),
  );
  maybePushArtifact(
    artifacts,
    buildHeartbeatArtifact(path.join(AI_CLONE_MEMORY_ROOT, 'heartbeat-state.json')),
  );
  maybePushArtifact(
    artifacts,
    buildFileArtifact({
      id: 'daily-brief',
      label: 'Morning Daily Brief (Legacy Record)',
      category: 'legacy',
      filePath: path.join(AI_CLONE_MEMORY_ROOT, 'daily-briefs.md'),
      summary: summarize(snippetFromHead(path.join(AI_CLONE_MEMORY_ROOT, 'daily-briefs.md')), 160),
      detail: 'Read-only historical brief retained for continuity; Codex-native launchd jobs own active summaries.',
    }),
  );
  maybePushArtifact(
    artifacts,
    buildFileArtifact({
      id: 'dream-cycle',
      label: 'Dream Cycle (Legacy Record)',
      category: 'legacy',
      filePath: path.join(AI_CLONE_MEMORY_ROOT, 'dream_cycle_log.md'),
      summary: summarize(snippetFromHead(path.join(AI_CLONE_MEMORY_ROOT, 'dream_cycle_log.md')), 160),
      detail: 'Read-only historical dream-cycle output retained for context, not an active scheduler dependency.',
    }),
  );
  maybePushArtifact(
    artifacts,
    buildFileArtifact({
      ...
      category: 'memory',
      filePath: path.join(AI_CLONE_MEMORY_ROOT, 'persistent_state.md'),
      summary: summarize(snippetFromHead(path.join(AI_CLONE_MEMORY_ROOT, 'persistent_state.md')), 160),
  ...
  maybePushArtifact(
    ...
    buildFileArtifact({
      ...
      category: 'local',
      filePath: path.join(AI_CLONE_MEMORY_ROOT, 'reports', 'meeting_watchdog_latest.md'),
      summary: summarize(snippetFromHead(path.join(AI_CLONE_MEMORY_ROOT, 'reports', 'meeting_watchdog_latest.md')), 160),
  ...
  maybePushArtifact(
    ...
    buildFileArtifact({
      ...
      category: 'local',
      filePath: path.join(AI_CLONE_MEMORY_ROOT, 'reports', 'post_sync_dispatch_latest.md'),
      summary: summarize(snippetFromHead(path.join(AI_CLONE_MEMORY_ROOT, 'reports', 'post_sync_dispatch_latest.md')), 160),
  ...
  maybePushArtifact(
    ...
    buildFileArtifact({
      ...
      category: 'local',
      filePath: path.join(AI_CLONE_MEMORY_ROOT, 'reports', 'accountability_sweep_latest.md'),
      summary: summarize(snippetFromHead(path.join(AI_CLONE_MEMORY_ROOT, 'reports', 'accountability_sweep_latest.md')), 160),
      detail: 'Checks whether meeting-created work is still moving through the queue.',
    }),
  );
  maybePushArtifact(
    artifacts,
    buildLatestRunnerMemoArtifact(path.join(AI_CLONE_MEMORY_ROOT, 'runner-memos/jean-claude')),
  );
  buildWorkspaceLaneArtifacts(path.join(AI_CLONE_PROJECT_ROOT, 'workspaces')).forEach((artifact) => maybePushArtifact(artifacts, artifact));
...
function buildHeartbeatArtifact(filePath: string): ExecutiveArtifact | null {
  ...
  return {
    id: 'heartbeat-state',
    label: 'Heartbeat State (Legacy Record)',
    category: 'legacy',
    path: toRelativePath(filePath),
    updatedAt: stat.mtime.toISOString(),
    ...
    detail: `${checkKeys.length} historical heartbeat checks are retained; current health comes from Railway and launchd telemetry.`,
  };
}
...
function toRelativePath(filePath: string) {
  return path.relative(AI_CLONE_PROJECT_ROOT, filePath).replace(/\\/g, '/');
}
