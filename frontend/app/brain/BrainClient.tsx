...
export type DocEntry = {
  ...
  group?: string;
  updatedAt?: string;
  readMode?: 'runtime' | 'live' | 'snapshot' | 'deployed_snapshot' | 'missing';
  resolvedPath?: string;
  authority?: string;
  status?: string;
...
// Kept as type-only compatibility for the generated, unimported workspace snapshot.
...
type AutomationMismatchReport = {
  source_of_truth?: string;
  registry_count?: number;
  registered_launchd_count?: number;
  run_count?: number;
...
type AutomationsIndexPayload = {
  success?: boolean;
  count?: number;
  run_count?: number;
  source_of_truth?: string;
  data?: Automation[];
  mismatches?: AutomationMismatchReport;
};
...
type WorkspaceWeeklyPlan = {
  ...
  strategy_contract_freshness?: {
    state?: 'current' | 'stale' | 'legacy' | 'unavailable' | string;
...
type YouTubeWatchlistPayload = {
  generated_at?: string;
  data_mode?: 'persisted' | 'configuration_only' | 'local_runner_refresh' | 'live_refresh';
  workspace?: string;
  runtime?: {
    yt_dlp?: boolean;
    ffmpeg?: boolean;
...
type BrainLocalActionQueuedResponse = {
  message?: string;
  queued?: boolean;
  state?: string;
  disposition?: string;
...
type PortfolioWorkspaceSummary = {
  ...
  counts?: {
    ...
    performance_due_actions?: number;
  };
  ...
  publication_performance?: {
    ...
    due_actions?: Record<string, number>;
    due_action_count?: number;
    topic_mix?: { status?: string; sample_size?: number; deficit_total?: number };
    intent_mix?: { status?: string; sample_size?: number; deficit_total?: number };
    actionable_gaps?: { code?: string; severity?: string; actionable?: boolean; next_action?: string }[];
  } | null;
  source_paths?: string[];
};
...
type PersonaResponseKind = 'agree' | 'disagree' | 'nuance' | 'story' | 'language';
type PersonaStandupKind = 'auto' | 'executive_ops' | 'operations' | 'weekly_review' | 'saturday_vision' | 'workspace_sync';
type PersonaMobileSheet = 'queue' | 'details' | null;
...
type PersonaReviewDraft = {
  schema: 'brain_persona_review_draft/v1';
  deltaId: string;
  reflectionText: string;
  responseKind: PersonaResponseKind;
  selectedPromotionItemIds: string[];
  promotionItemTargetOverrides: Record<string, string>;
  routeToMemory: boolean;
  routeToStandup: boolean;
  routeToPM: boolean;
  triageMemoryTargets: string[];
  triageWorkspaceKeys: string[];
  triageStandupKind: PersonaStandupKind;
  triagePMTitle: string;
  updatedAt: string;
};
...
type BrainSystemRouteResponse = {
  ...
  routes?: Array<{
    ...
    standup_reused?: boolean;
    pm_card?: {
      id: string;
      title?: string;
      status?: string;
    } | null;
    pm_card_reused?: boolean;
  }>;
  standup?: {
    id: string;
    workspace_key?: string;
...
type Tab = BrainTab;
type ResourceLoadState = 'loading' | 'ready' | 'refreshing' | 'error';
type BrainLoadTargets = {
  briefs: boolean;
  ...
  docsIndex: boolean;
  youtubeWatchlist: boolean;
  youtubeJobs: boolean;
};
type RequestedDeltaLookupState = 'idle' | 'loading' | 'ready' | 'not_found' | 'error';
...
function brainLoadTargetsForTab(tab: Tab): BrainLoadTargets {
  return {
    briefs: tab === 'briefs',
    personaDeltas: tab === 'persona',
    controlPlane: tab === 'dashboard',
    automations: tab === 'automations',
    telemetryHealth: tab === 'dashboard',
    portfolioSnapshot: tab === 'dashboard',
    docsIndex: tab === 'persona' || tab === 'docs',
    youtubeWatchlist: tab === 'dashboard',
    youtubeJobs: tab === 'dashboard',
  };
}
...
const BRAIN_DOC_INDEX_TIMEOUT_MS = 20_000;
const BRAIN_DOC_CONTENT_TIMEOUT_MS = 12_000;
const PERSONA_REVIEW_DRAFT_STORAGE_KEY = 'aiclone.brain.persona-review-drafts.v1';
const PERSONA_PHONE_MAX_WIDTH = 520;
...
export type BrainBootstrapStatus = {
  briefs: boolean;
  personaDeltas: boolean;
  controlPlane: boolean;
};

export type BrainClientInitialState = {
  briefs?: DailyBriefEntry[];
  personaDeltas?: PersonaDeltaEntry[];
  controlPlane?: BrainControlPlanePayload | null;
  bootstrapStatus?: BrainBootstrapStatus;
...
export default function BrainClient({
  ...
  const [activeTab, setActiveTab] = useState<Tab>(initialState?.requestedDeltaId ? 'persona' : 'dashboard');
  const [locationReady, setLocationReady] = useState(false);
  const [requestedDeltaId, setRequestedDeltaId] = useState<string | null>(initialState?.requestedDeltaId ?? null);
  ...
  const [briefsLoadState, setBriefsLoadState] = useState<ResourceLoadState>(initialState?.bootstrapStatus?.briefs ? 'ready' : 'loading');
  ...
  const [personaDeltasLoadState, setPersonaDeltasLoadState] = useState<ResourceLoadState>(
    initialState?.bootstrapStatus?.personaDeltas ? 'ready' : 'loading',
  );
  const [requestedDeltaLookupState, setRequestedDeltaLookupState] = useState<RequestedDeltaLookupState>('idle');
  const [requestedDeltaLookupError, setRequestedDeltaLookupError] = useState<string | null>(null);
  const [controlPlane, setControlPlane] = useState<BrainControlPlanePayload | null>(initialState?.controlPlane ?? null);
  const [controlPlaneLoadState, setControlPlaneLoadState] = useState<ResourceLoadState>(
    initialState?.bootstrapStatus?.controlPlane ? 'ready' : 'loading',
  );
  const [automations, setAutomations] = useState<Automation[]>(initialState?.controlPlane?.automations ?? []);
  const [automationsError, setAutomationsError] = useState<string | null>(null);
  const [automationsLoadState, setAutomationsLoadState] = useState<ResourceLoadState>('loading');
  const [automationsIndex, setAutomationsIndex] = useState<AutomationsIndexPayload | null>(null);
  const [controlPlaneError, setControlPlaneError] = useState<string | null>(null);
  const [controlPlaneLastVerifiedAt, setControlPlaneLastVerifiedAt] = useState<string | null>(
    initialState?.controlPlane?.generated_at ?? null,
  );
  ...
  const [portfolioSnapshot, setPortfolioSnapshot] = useState<PortfolioWorkspaceSnapshot | null>(
    initialState?.controlPlane?.portfolio_snapshot ?? null,
  );
  const [portfolioSnapshotError, setPortfolioSnapshotError] = useState<string | null>(null);
  const [portfolioSnapshotLoadState, setPortfolioSnapshotLoadState] = useState<ResourceLoadState>(
    initialState?.controlPlane?.portfolio_snapshot ? 'ready' : 'loading',
  );
  const [docs, setDocs] = useState<DocEntry[]>([]);
  const [docsIndexCount, setDocsIndexCount] = useState(0);
  const [docsIndexLoaded, setDocsIndexLoaded] = useState(false);
  const [docsLoadState, setDocsLoadState] = useState<ResourceLoadState>('loading');
  ...
  const activatedTabsRef = useRef<Set<Tab>>(new Set());
  const [viewportWidth, setViewportWidth] = useState(1600);
  const [viewportHeight, setViewportHeight] = useState(1200);
  const visibleDocCount = docsIndexLoaded ? docsIndexCount : controlPlane?.summary?.doc_count ?? docsIndexCount;
  const navigateToSection = useCallback((tab: Tab) => {
    setActiveTab(tab);
    if (typeof window === 'undefined') {
      return;
    }
    const href = buildBrainSectionHref(tab, window.location.search);
    const currentHref = `${window.location.pathname}${window.location.search}${window.location.hash}`;
    if (href !== currentHref) {
      window.history.pushState(null, '', href);
    }
  }, []);
  const selectPersonaDeltaInUrl = useCallback((deltaId: string | null, historyMode: 'push' | 'replace' = 'push') => {
    setActiveTab('persona');
    setRequestedDeltaId(deltaId);
    if (typeof window === 'undefined') {
      return;
    }
    const params = new URLSearchParams(window.location.search);
    if (deltaId) {
      params.set('delta_id', deltaId);
    } else {
      params.delete('delta_id');
    }
    const query = params.toString();
    const href = `/brain${query ? `?${query}` : ''}#brain-section-persona`;
    const currentHref = `${window.location.pathname}${window.location.search}${window.location.hash}`;
    if (href === currentHref) {
      return;
    }
  ...
  const fetchFreshJson = useCallback(async function fetchFreshJson<T>(path: string, timeoutMs: number): Promise<T> {
    ...
    return controlApiGet<T>(`${path}${separator}brain_ts=${Date.now()}`, { timeoutMs });
  }, []);

  const loadData = useCallback(async function loadData(
    isCancelled: () => boolean = () => false,
    targets: Partial<BrainLoadTargets> = {},
  ): Promise<void> {
    const shouldLoad = (target: keyof BrainLoadTargets) => targets[target] ?? true;
    const requests: Promise<void>[] = [];

    if (shouldLoad('briefs')) {
      setBriefsLoadState((current) => (current === 'ready' ? 'refreshing' : 'loading'));
      requests.push(
        fetchFreshJson<DailyBriefEntry[]>('/api/briefs/?limit=50', BRAIN_BRIEFS_TIMEOUT_MS)
          .then((items) => {
            if (isCancelled() || !Array.isArray(items)) return;
            setBriefs(items);
            setSelectedBrief((current) => items.find((entry) => entry.id === current?.id) ?? items[0] ?? null);
            setBriefsError(null);
            setBriefsLoadState('ready');
          })
          .catch((error) => {
            if (isCancelled()) return;
            console.error('Failed to load daily briefs', error);
            setBriefsError('Unable to load daily briefs right now.');
            setBriefsLoadState('error');
          }),
      );
    }
      ...
      requests.push(
        fetchFreshJson<PersonaDeltaEntry[]>('/api/persona/deltas?limit=100&view=brain_queue', BRAIN_PERSONA_TIMEOUT_MS)
          .then((items) => {
            ...
            setPersonaDeltasLoadState('ready');
          })
          .catch((error) => {
            if (isCancelled()) return;
            console.error('Failed to load persona deltas', error);
            setPersonaDeltasError('Unable to load persona deltas right now.');
            setPersonaDeltasLoadState('error');
          }),
      );
    }

    if (shouldLoad('controlPlane')) {
      setControlPlaneLoadState((current) => (current === 'ready' ? 'refreshing' : 'loading'));
      requests.push(
        fetchFreshJson<BrainControlPlanePayload>('/api/brain/control-plane', BRAIN_CONTROL_PLANE_TIMEOUT_MS)
          .then((payload) => {
            if (isCancelled()) return;
            applyControlPlanePayload(payload ?? null);
            setControlPlaneError(null);
            setControlPlaneLastVerifiedAt(payload?.generated_at ?? new Date().toISOString());
            setWorkspaceSnapshotError(null);
            setControlPlaneLoadState('ready');
          })
          .catch((error) => {
            if (isCancelled()) return;
            console.error('Failed to load Brain control plane', error);
            setControlPlaneError('Brain could not verify the current control-plane snapshot.');
            setWorkspaceSnapshotError('Unable to load shared source intelligence right now.');
            setControlPlaneLoadState('error');
          }),
      );
    }

    if (shouldLoad('automations')) {
      setAutomationsLoadState((current) => (current === 'ready' ? 'refreshing' : 'loading'));
      requests.push(
        fetchFreshJson<AutomationsIndexPayload>('/api/automations/', BRAIN_CONTROL_PLANE_TIMEOUT_MS)
          .then((payload) => {
            if (isCancelled()) return;
            setAutomations(Array.isArray(payload?.data) ? payload.data : []);
            setAutomationsIndex(payload ?? null);
            setAutomationsError(null);
            setAutomationsLoadState('ready');
          })
          .catch((error) => {
            if (isCancelled()) return;
            console.error('Failed to load automation runtime truth', error);
            setAutomationsError('Unable to verify automation runtime evidence right now.');
            setAutomationsLoadState('error');
          }),
      );
    }

    if (shouldLoad('telemetryHealth')) {
      requests.push(
        fetchFreshJson<OpenBrainHealth>('/api/open-brain/health', BRAIN_CONTROL_PLANE_TIMEOUT_MS)
          .then((payload) => {
            if (isCancelled()) return;
            setTelemetryHealth(payload ?? null);
            setTelemetryError(null);
          })
          .catch((error) => {
            if (isCancelled()) return;
            console.error('Failed to verify Open Brain health', error);
            setTelemetryError('Open Brain health could not be verified. Capture totals may still reflect the last control-plane snapshot.');
          }),
      );
    }

    if (shouldLoad('portfolioSnapshot')) {
      setPortfolioSnapshotLoadState((current) => (current === 'ready' ? 'refreshing' : 'loading'));
      requests.push(
        fetchFreshJson<PortfolioWorkspaceSnapshot>('/api/brain/portfolio-snapshot', BRAIN_CONTROL_PLANE_TIMEOUT_MS)
          .then((payload) => {
            if (isCancelled()) return;
            setPortfolioSnapshot(payload ?? null);
            setPortfolioSnapshotError(null);
            setPortfolioSnapshotLoadState('ready');
          })
          .catch((error) => {
            if (isCancelled()) return;
            console.error('Failed to verify portfolio snapshot', error);
            setPortfolioSnapshotError('Portfolio status could not be verified right now.');
            setPortfolioSnapshotLoadState('error');
          }),
      );
    }

    if (shouldLoad('docsIndex')) {
      setDocsLoadState((current) => (current === 'ready' ? 'refreshing' : 'loading'));
      requests.push(
        fetchFreshJson<BrainDocsIndexPayload>('/api/brain/docs', BRAIN_DOC_INDEX_TIMEOUT_MS)
          .then((payload) => {
            if (isCancelled()) return;
            const nextDocs = Array.isArray(payload?.docs) ? payload.docs : [];
            setDocs(nextDocs);
            setDocsIndexCount(typeof payload?.count === 'number' ? payload.count : nextDocs.length);
            setDocsIndexLoaded(true);
            setDocsError(null);
            setDocsLoadState('ready');
          })
          .catch((error) => {
            if (isCancelled()) return;
            console.error('Failed to load Brain docs index', error);
            setDocsError('Unable to load the Brain documentation index right now.');
            setDocsLoadState('error');
          }),
      );
    }

    if (shouldLoad('youtubeWatchlist')) {
      requests.push(
        fetchFreshJson<YouTubeWatchlistPayload>('/api/brain/youtube-watchlist', BRAIN_YOUTUBE_WATCHLIST_TIMEOUT_MS)
          .then((payload) => {
            if (isCancelled()) return;
    ...
    if (shouldLoad('youtubeJobs')) {
      requests.push(
        fetchFreshJson<{ jobs?: YouTubeIngestJob[] }>('/api/brain/youtube-watchlist/jobs', BRAIN_YOUTUBE_JOBS_TIMEOUT_MS)
          .then((payload) => {
            if (isCancelled()) return;
  ...
  const refreshActiveTab = useCallback(
    () => loadData(() => false, brainLoadTargetsForTab(activeTab)),
    [activeTab, loadData],
  );
  ...
  useEffect(() => {
    ...
    const syncLocation = () => {
      const locationState = parseBrainLocation(window.location.search, window.location.hash);
      setActiveTab(locationState.tab);
      setRequestedDeltaId(locationState.deltaId);
  ...
  useEffect(() => {
    if (!locationReady || typeof window === 'undefined' || (activeTab === 'persona' && requestedDeltaId)) {
      return;
    }
    const frameId = window.requestAnimationFrame(() => {
      const target = document.getElementById(`brain-section-${activeTab}`);
      if (!target) return;
  ...
  }, [activeTab, locationReady, requestedDeltaId]);

  useEffect(() => {
    if (!requestedDeltaId) {
      setRequestedDeltaLookupState('idle');
      setRequestedDeltaLookupError(null);
      return;
    }
    if (personaDeltas.some((delta) => delta.id === requestedDeltaId)) {
      setRequestedDeltaLookupState('ready');
      setRequestedDeltaLookupError(null);
      return;
    }
    if (personaDeltasLoadState === 'loading' || personaDeltasLoadState === 'refreshing') {
      return;
    }

    let cancelled = false;
    setRequestedDeltaLookupState('loading');
    setRequestedDeltaLookupError(null);
    void fetchFreshJson<PersonaDeltaEntry>(
      `/api/persona/deltas/${encodeURIComponent(requestedDeltaId)}`,
      BRAIN_PERSONA_TIMEOUT_MS,
    )
      .then((delta) => {
        if (cancelled || !delta?.id) return;
        setPersonaDeltas((current) => (current.some((item) => item.id === delta.id) ? current : [delta, ...current]));
        setRequestedDeltaLookupState('ready');
      })
      .catch((lookupError) => {
        if (cancelled) return;
        const message = ownerSafeErrorMessage(lookupError, 'Unable to load the requested persona item.');
        if (/\b404\b/.test(message)) {
          setRequestedDeltaLookupState('not_found');
          setRequestedDeltaLookupError('The requested persona item was not found.');
          return;
        }
        setRequestedDeltaLookupState('error');
        setRequestedDeltaLookupError(message);
      });
    return () => {
      cancelled = true;
    };
  }, [fetchFreshJson, personaDeltas, personaDeltasLoadState, requestedDeltaId]);
  ...
  useEffect(() => {
    ...
    const firstActivation = !activatedTabsRef.current.has(activeTab);
    activatedTabsRef.current.add(activeTab);
    const activationTargets = brainLoadTargetsForTab(activeTab);
    if (firstActivation) {
      if (initialState?.bootstrapStatus?.briefs && activeTab === 'briefs') activationTargets.briefs = false;
      if (initialState?.bootstrapStatus?.personaDeltas && activeTab === 'persona') activationTargets.personaDeltas = false;
      if (initialState?.bootstrapStatus?.controlPlane && (activeTab === 'dashboard' || activeTab === 'automations')) {
        activationTargets.controlPlane = false;
      }
    }
    void loadData(isCancelled, activationTargets);
    const pollTargets = brainLoadTargetsForTab(activeTab);
    const interval = setInterval(() => void loadData(isCancelled, pollTargets), 60_000);
    return () => {
      cancelled = true;
      clearInterval(interval);
    };
  }, [
    initialState?.bootstrapStatus?.briefs,
    initialState?.bootstrapStatus?.controlPlane,
    initialState?.bootstrapStatus?.personaDeltas,
    activeTab,
    loadData,
    locationReady,
  ]);
  ...
  async function submitLongFormIngest() {
    ...
    try {
      const payload = await controlApiPost<BrainLocalActionQueuedResponse>('/api/brain/ingest-long-form', {
          ...
          run_refresh: true,
        }, { timeoutMs: BRAIN_CONTROL_PLANE_TIMEOUT_MS });
      setLongFormIngestStatus(formatBrainLocalQueueStatus(payload, longFormIngest.title || 'Long-form source'));
      setLongFormIngest({ url: '', title: '', summary: '', notes: '', transcriptText: '' });
      await loadData(() => false, brainLoadTargetsForTab('dashboard'));
    } catch (error) {
      setLongFormIngestError(ownerSafeErrorMessage(error, 'Unable to register long-form source.'));
    } finally {
  ...
  return (
    <RuntimePage module="brain" tabs={tabs} maxWidth="min(1920px, calc(100vw - 24px))">
      <div style={{ display: 'grid', gap: '28px' }}>
        {activeTab === 'dashboard' && <section id="brain-section-dashboard" tabIndex={-1} style={{ scrollMarginTop: '96px', outline: 'none' }}>
          <DashboardPanel
            briefCount={briefsLoadState === 'ready' ? briefs.length : null}
            docCount={visibleDocCount}
            automationCount={automations.length}
            controlPlane={controlPlane}
            controlPlaneLoadState={controlPlaneLoadState}
            controlPlaneError={controlPlaneError}
            controlPlaneLastVerifiedAt={controlPlaneLastVerifiedAt}
            telemetry={telemetry}
            ...
            portfolioSnapshotError={portfolioSnapshotError}
            portfolioSnapshotLoadState={portfolioSnapshotLoadState}
            ...
            refreshBrainData={refreshActiveTab}
            onNavigate={navigateToSection}
          />
        </section>}
        {activeTab === 'briefs' && <section id="brain-section-briefs" tabIndex={-1} style={{ scrollMarginTop: '96px', outline: 'none' }}>
          <DailyBriefsPanel
            briefs={briefs}
            selected={selectedBrief}
            onSelect={setSelectedBrief}
            error={briefsError}
            loadState={briefsLoadState}
            onRefresh={refreshActiveTab}
          />
        </section>}
        {activeTab === 'persona' && <section id="brain-section-persona" tabIndex={-1} style={{ scrollMarginTop: '96px', outline: 'none' }}>
          <PersonaPanel
            ...
            loadState={personaDeltasLoadState}
            requestedDeltaId={requestedDeltaId}
            requestedDeltaLookupState={requestedDeltaLookupState}
            requestedDeltaLookupError={requestedDeltaLookupError}
            viewportWidth={viewportWidth}
            viewportHeight={viewportHeight}
            refreshBrainData={refreshActiveTab}
            onSelectDelta={selectPersonaDeltaInUrl}
            mergeUpdatedDelta={(updatedDelta) =>
              setPersonaDeltas((current) => current.map((delta) => (delta.id === updatedDelta.id ? updatedDelta : delta)))
            }
          />
        </section>}
        {activeTab === 'automations' && <section id="brain-section-automations" tabIndex={-1} style={{ scrollMarginTop: '96px', outline: 'none' }}>
          <AutomationsPanel
            automations={automations}
            error={automationsError}
            loadState={automationsLoadState}
            index={automationsIndex}
          />
        </section>}
        {activeTab === 'docs' && <section id="brain-section-docs" tabIndex={-1} style={{ scrollMarginTop: '96px', outline: 'none' }}>
          <DocsPanel docs={docs} error={docsError} loadState={docsLoadState} />
        </section>}
      </div>
    </RuntimePage>
  );
...
function DashboardPanel({
  ...
  automationCount,
  controlPlane,
  controlPlaneLoadState,
  controlPlaneError,
  controlPlaneLastVerifiedAt,
  telemetry,
  telemetryHealth,
  telemetryError,
  workspaceSnapshot,
  ...
  controlPlane: BrainControlPlanePayload | null;
  controlPlaneLoadState: ResourceLoadState;
  controlPlaneError: string | null;
  controlPlaneLastVerifiedAt: string | null;
  telemetry: CaptureTelemetry | null;
  ...
  portfolioSnapshotError: string | null;
  portfolioSnapshotLoadState: ResourceLoadState;
  ...
  return (
    <section style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <BrainControlPlanePanel
        ...
        controlPlane={controlPlane}
        loadState={controlPlaneLoadState}
...
function BrainControlPlanePanel({
  ...
  const verified = Boolean(controlPlane);
  ...
  const brainMemorySyncLive = Boolean(brainMemorySync?.sync_live && brainMemorySyncFresh);
  const brainMemorySyncLabel = brainMemorySyncLive
    ? 'Sync live'
    : brainMemorySync?.sync_live
    ? 'Sync status stale'
    : 'Sync idle';
  ...
  const executiveFrontDoor = [
    ...
    {
      label: 'Changed',
      value: verified ? (controlPlane?.summary?.brain_signal_count ?? 0) + recentSourceCount + memorySyncItems.length : '—',
      detail: verified
        ? `${controlPlane?.summary?.brain_signal_count ?? 0} Brain signals · ${recentSourceCount} recent sources · ${memorySyncItems.length} memory updates`
        : 'No unverified zero is shown.',
      tone: '#38bdf8',
    },
    {
      label: 'Learned',
      value: verified ? (brainMemorySync?.processed_count ?? 0) + (sourceIndex?.counts?.promoted ?? 0) + (sourceIndex?.counts?.routed ?? 0) : '—',
      detail: verified
        ? `${brainMemorySync?.processed_count ?? 0} processed · ${sourceIndex?.counts?.promoted ?? 0} promoted · ${sourceIndex?.counts?.routed ?? 0} routed`
        : 'No unverified zero is shown.',
      tone: '#22c55e',
    },
    {
      label: 'Running',
      value: verified ? (controlPlane?.summary?.active_automation_count ?? automationCount) + activeYouTubeJobs : '—',
      detail: verified
        ? `${controlPlane?.summary?.active_automation_count ?? automationCount} configured automations · ${activeYouTubeJobs} YouTube jobs · ${youtubeWatchlist?.counts?.channels ?? 0} channels watched`
        : 'Open Automations for observed runtime truth.',
      tone: '#a78bfa',
    },
  ];
  const truthLanes = [
    ...
    >
      <div style={{ marginBottom: '16px', display: 'flex', justifyContent: 'space-between', gap: '16px', alignItems: 'flex-start', flexWrap: 'wrap' }}>
        <div>
          ...
          <h1 style={{ color: 'white', fontSize: '32px', margin: '4px 0 10px' }}>What needs you today.</h1>
          <p style={{ color: '#cbd5f5', fontSize: '14px', lineHeight: 1.65, maxWidth: '860px', margin: 0 }}>
            Start with decisions and movement, then open the focused workspace for briefs, persona review, automations, or canonical docs.
          </p>
        </div>
          ...
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
            <InlineBadge
              label={
                degraded
                  ? verified
                    ? 'Degraded · last verified snapshot'
              ...
              tone={degraded ? '#f97316' : verified ? '#22c55e' : '#64748b'}
            />
            <button type="button" onClick={() => onNavigate('briefs')} style={{ ...brainLinkButtonStyle, cursor: 'pointer' }}>
              Daily briefs
            </button>
            <button type="button" onClick={() => onNavigate('persona')} style={{ ...brainLinkButtonStyle, cursor: 'pointer' }}>
              Persona queue
            </button>
            <button type="button" onClick={() => onNavigate('docs')} style={{ ...brainLinkButtonStyle, cursor: 'pointer' }}>
              Docs + memory
            </button>
            <button type="button" onClick={() => void onRefresh()} style={{ ...brainLinkButtonStyle, cursor: 'pointer' }}>
              Retry verification
            </button>
          </div>
          <p style={{ color: degraded ? '#fbbf24' : '#64748b', fontSize: '12px', margin: 0 }}>
            {lastVerifiedAt ? `Last verified ${formatTimestamp(lastVerifiedAt)}` : 'No verified snapshot in this session'}
          </p>
          {workspaceSnapshotError && <p style={{ color: '#fca5a5', fontSize: '12px', margin: 0 }}>{workspaceSnapshotError}</p>}
        </div>
      </div>

      {loadState === 'loading' && !controlPlane && (
        <p role="status" style={{ color: '#94a3b8', fontSize: '13px', margin: '0 0 14px' }}>
          Loading the Brain control plane…
        </p>
      )}

      {error && (
        <p role="alert" style={{ color: verified ? '#fbbf24' : '#f87171', fontSize: '13px', margin: '0 0 14px' }}>
          {error} {verified ? 'The values below are retained from the last verified response and are marked degraded.' : 'Unknown values remain blank instead of being shown as zero.'}
        </p>
      )}

      <div
        aria-label="Executive Brain summary"
        style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '12px', marginBottom: '16px' }}
      >
        {executiveFrontDoor.map((item) => (
          <ExecutiveFrontDoorCard key={item.label} {...item} />
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '12px', marginBottom: '14px' }}>
        <TelemetryStat
          ...
          value={briefCount ?? '—'}
          tone="#38bdf8"
          detail={briefCount === null ? 'Open Daily Briefs to load' : 'Saved daily briefs'}
        />
        <TelemetryStat
          label="Automations"
          value={verified ? controlPlane?.summary?.automation_count ?? automationCount : '—'}
          tone="#fbbf24"
          detail="Configured jobs; open Automations for observed health"
        />
        ...
        <TelemetryStat
          label="Source Index"
          value={controlPlane?.summary?.source_intelligence_total ?? sourceIndex?.counts?.total ?? '—'}
          tone="#34d399"
          detail="Registered source-intelligence assets"
        />
        <TelemetryStat
          label="Memory Queue"
          value={verified ? brainMemorySync?.queued_route_count ?? 0 : '—'}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 320px), 1fr))', gap: '12px', marginBottom: '14px' }}>
        <div style={{ borderRadius: '14px', border: '1px solid #1f2937', backgroundColor: '#020617', padding: '14px', display: 'grid', gap: '12px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', alignItems: 'flex-start', flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              <InlineBadge label={brainMemorySyncLabel} tone={brainMemorySyncLive ? '#22c55e' : brainMemorySync?.sync_live ? '#f59e0b' : '#64748b'} />
          ...
          {brainMemorySync?.sync_live && !brainMemorySyncFresh && (
            <p role="status" style={{ color: '#fbbf24', fontSize: '12px', lineHeight: 1.5, margin: 0 }}>
              The worker reported live, but its latest status is older than six hours. Brain is treating this snapshot as stale.
            </p>
          )}
...
function BrainPortfolioPanel({
  ...
  async function reviewBrainSignal(signal: BrainSignalEntry) {
    ...
    setRouteError(null);
    try {
      const payload = await controlApiPost<BrainLocalActionQueuedResponse>(`/api/brain/signals/${encodeURIComponent(signal.id)}/review`, {
          digest: draft.digest,
      ...
      setRouteStatus(formatBrainLocalQueueStatus(payload, 'Brain Signal review'));
      await refreshBrainData();
    } catch (error) {
      setRouteError(ownerSafeErrorMessage(error, 'Unable to review Brain Signal.'));
    } finally {
  ...
  async function routeBrainSignal(signal: BrainSignalEntry) {
    ...
    try {
      const payload = await controlApiPost<BrainLocalActionQueuedResponse>(`/api/brain/signals/${encodeURIComponent(signal.id)}/route`, {
          route: draft.route,
      ...
      setRouteStatus(formatBrainLocalQueueStatus(payload, `Brain Signal route to ${humanizeSnakeCase(draft.route)}`));
      await refreshBrainData();
    } catch (error) {
      setRouteError(ownerSafeErrorMessage(error, 'Unable to route Brain Signal.'));
    } finally {
      setRoutingSignalId(null);
    }
  }
  ...
  return (
    <section style={{ borderRadius: '16px', border: '1px solid #1f2937', backgroundColor: '#050b19', padding: '20px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', alignItems: 'flex-start', flexWrap: 'wrap', marginBottom: '14px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
          <InlineBadge
            label={error ? (portfolioSnapshot ? 'Degraded · retained snapshot' : 'Unavailable') : loadState === 'refreshing' ? 'Refreshing' : 'Verified separately'}
      ...
      {error && (
        <p role="alert" style={{ color: portfolioSnapshot ? '#fbbf24' : '#f87171', fontSize: '13px', margin: '0 0 14px' }}>
          {error} {portfolioSnapshot ? 'Showing the last verified portfolio snapshot.' : 'Portfolio counts remain unknown.'}
        </p>
      )}

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '12px', marginBottom: '14px' }}>
        <TelemetryStat label="Workspaces" value={portfolioSnapshot ? portfolioSnapshot.counts?.workspaces ?? workspaces.length : '—'} tone="#a78bfa" detail="Visible portfolio workspaces" />
        <TelemetryStat
          label="Attention"
          value={portfolioSnapshot ? portfolioSnapshot.counts?.needs_brain_attention ?? workspaces.filter((workspace) => workspace.needs_brain_attention).length : '—'}
          tone={(portfolioSnapshot?.counts?.needs_brain_attention ?? 0) > 0 ? '#f97316' : '#22c55e'}
          detail="Review, blockers, or failed work"
        />
        <TelemetryStat label="PM Cards" value={portfolioSnapshot ? portfolioSnapshot.counts?.active_pm_cards ?? 0 : '—'} tone="#fbbf24" detail="Active workspace work" />
        <TelemetryStat label="Blockers" value={portfolioSnapshot ? portfolioSnapshot.counts?.standup_blockers ?? 0 : '—'} tone="#f87171" detail="Latest standup blockers" />
        <TelemetryStat label="Signals" value={signalsVerified ? recentSignals.length : '—'} tone="#38bdf8" detail="Recent Brain Signal objects" />
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '12px', marginBottom: '14px' }}>
          {workspaces.map((workspace) => {
            ...
            const snapshotTypes = Object.entries(workspace.persisted_snapshot_types ?? {})
              ...
              .slice(0, 4);
            const performance = workspace.publication_performance;
            const performanceSummary = performance
              ? `${humanizeSnakeCase(performance.state ?? 'unknown')} · ${performance.confirmed_publications ?? 0} confirmed · ${performance.complete_feedback_posts ?? 0} feedback-complete · ${performance.due_action_count ?? 0} follow-up${(performance.due_action_count ?? 0) === 1 ? '' : 's'} due`
              : 'No canonical publication-performance projection is available.';
            return (
              <div
              ...
              >
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: '8px' }}>
                  ...
                  <TelemetryMeta label="Feedback" value={String(workspace.counts?.performance_due_actions ?? performance?.due_action_count ?? 0)} detail="Due evidence actions" />
                </div>
                ...
                <div style={{ display: 'grid', gap: '8px' }}>
                  ...
                  {workspace.workspace_key === 'feezie-os' && (
                    <PortfolioFact label="Publication Feedback Truth" value={truncateText(performanceSummary, 190)} />
                  )}
                  <PortfolioFact
      ...
      {(routeStatus || routeError || recentSignals.length > 0) && (
        <div style={{ marginTop: '14px', display: 'grid', gap: '10px' }}>
          {routeStatus && <p role="status" aria-live="polite" style={{ color: '#22c55e', fontSize: '12px', margin: 0 }}>{routeStatus}</p>}
          {routeError && <p role="alert" style={{ color: '#f87171', fontSize: '12px', margin: 0 }}>{routeError}</p>}
          {recentSignals.map((signal) => (
  ...
  return (
    <div style={{ borderRadius: '12px', border: '1px solid #1e293b', backgroundColor: '#020617', padding: '12px', display: 'grid', gap: '12px' }}>
      ...
      {draft.route === 'canonical_memory' && (
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          {canonicalMemoryRouteOptions.map((target) => {
            ...
            return (
              <button key={target} type="button" aria-pressed={active} onClick={() => onToggleMemoryTarget(target)} style={triageToggleButtonStyle(active)}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          <Link href={buildBrainSignalPostingWorkspaceHref(signal, draft)} style={brainLinkButtonStyle}>
            Draft in FEEZIE
          </Link>
          <button type="button" onClick={onReview} disabled={disabled} style={brainSmallButtonStyle(disabled)}>
            {reviewing ? 'Queueing...' : 'Queue Review on Mac'}
          </button>
          <button type="button" onClick={onRoute} disabled={routeDisabled} style={brainSmallButtonStyle(routeDisabled)}>
            {routing ? 'Queueing...' : `Queue ${humanizeSnakeCase(draft.route)} route on Mac`}
          </button>
        </div>
      </div>
...
function PortfolioFact({ label, value }: { label: string; value: string }) {
  const displayValue = normalizeBrainDisplayText(value);
  return (
    <div style={{ borderRadius: '10px', border: '1px solid #1e293b', backgroundColor: '#030712', padding: '10px' }}>
      ...
      <p style={{ color: '#dbe7ff', fontSize: '13px', lineHeight: 1.45, margin: 0 }}>{displayValue}</p>
    </div>
  );
}
...
function BrainLongFormIngestPanel({
  ...
  const watchlistHasRunnerSnapshot = youtubeWatchlist?.data_mode === 'persisted';
  const watchlistSnapshotFresh = watchlistHasRunnerSnapshot && isRecentTimestamp(youtubeWatchlist?.generated_at, 3 * 60 * 60 * 1000);
  const transcriptRuntimeReady = watchlistHasRunnerSnapshot && Boolean(youtubeWatchlist?.runtime?.can_transcribe);
  const transcriptRuntimeLabel = !watchlistHasRunnerSnapshot
    ? 'Waiting for the local transcript runtime report'
    : transcriptRuntimeReady
    ? watchlistSnapshotFresh
      ? `Transcript runtime ready (${youtubeWatchlist?.runtime?.whisper_model || 'whisper'})`
      : `Transcript runtime was ready at last snapshot (${youtubeWatchlist?.runtime?.whisper_model || 'whisper'})`
    : watchlistSnapshotFresh
      ? 'Transcript runtime unavailable'
      : 'Transcript runtime was unavailable at last snapshot';
  ...
  async function queueWatchlistVideo(video: YouTubeWatchlistVideo) {
    ...
    try {
      const payload = await controlApiPost<BrainLocalActionQueuedResponse>('/api/brain/youtube-watchlist/ingest', {
          url: video.url,
      ...
      setQueueStatus(formatBrainLocalQueueStatus(payload, video.title || 'YouTube source'));
      await refreshBrainData();
    } catch (ingestError) {
      setQueueError(ownerSafeErrorMessage(ingestError, 'Unable to queue YouTube ingest.'));
    } finally {
      setQueueingUrl(null);
    }
  }
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', alignItems: 'flex-start', flexWrap: 'wrap', marginBottom: '14px' }}>
        <div>
          ...
          <p style={{ color: '#94a3b8', fontSize: '13px', lineHeight: 1.55, maxWidth: '760px' }}>
            Queue YouTube videos, podcasts, and other long-form sources here so Brain remains the global entry point. Your signed local runner writes them into the shared source system on this Mac, then feeds briefs, planning, and persona review.
          </p>
        </div>
      ...
      <div style={{ display: 'grid', gap: '16px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 360px), 1fr))', gap: '16px' }}>
          <div style={{ display: 'grid', gap: '10px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
              <button
                ...
                disabled={longFormQueueDisabled}
                style={{
                  border: '1px solid rgba(56,189,248,0.45)',
                  borderRadius: '10px',
                  padding: '10px 14px',
                  background: longFormQueueDisabled ? '#0f172a' : 'rgba(8,47,73,0.8)',
                  color: 'white',
                  cursor: submitting ? 'progress' : longFormQueueDisabled ? 'not-allowed' : 'pointer',
                  fontWeight: 600,
                }}
              >
                {submitting ? 'Queueing…' : 'Queue Long-Form Source on Mac'}
              </button>
              {!canQueueLongForm && (
                <span style={{ color: '#94a3b8', fontSize: '12px' }}>Add a URL, transcript, or notes before queueing.</span>
              )}
              {status && <span role="status" aria-live="polite" style={{ color: '#22c55e', fontSize: '12px' }}>{status}</span>}
              {error && <span role="alert" style={{ color: '#f87171', fontSize: '12px' }}>{error}</span>}
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gap: '12px' }}>
            <BriefOverlayBlock
              ...
              items={[
                'Queues the Mac runner to write a normalized long-form source into the local knowledge system.',
                'Refreshes the authenticated shared snapshot used by Brain, briefs, planner, and Ops.',
        ...
        <section style={{ borderRadius: '14px', border: '1px solid #1e293b', backgroundColor: '#020617', padding: '16px', display: 'grid', gap: '14px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap', alignItems: 'flex-start' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              <InlineBadge label={transcriptRuntimeLabel} tone={transcriptRuntimeReady && watchlistSnapshotFresh ? '#22c55e' : '#f59e0b'} />
              <InlineBadge
                label={
                  !watchlistHasRunnerSnapshot
                    ? 'Configuration only — local runner has not reported yet'
                    : youtubeWatchlist?.generated_at
                      ? `Last reported ${formatTimestamp(youtubeWatchlist.generated_at)}`
                      : 'No watchlist snapshot yet'
                }
                tone={watchlistSnapshotFresh ? '#38bdf8' : '#f59e0b'}
              />
              <InlineBadge
                      ...
                      {source.videos.map((video, videoIndex) => (
                        ...
                        >
                          <div style={{ display: 'flex', justifyContent: 'space-between', gap: '10px', flexWrap: 'wrap', alignItems: 'flex-start' }}>
                            ...
                            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                              {safeExternalHttpsUrl(video.url) && (
                                <a href={safeExternalHttpsUrl(video.url) ?? undefined} target="_blank" rel="noreferrer" style={brainLinkButtonStyle}>
                                  Open video
                                </a>
                              )}
                              <button
...
function WorkspaceMirrorsPanel({ workspaceSnapshot }: { workspaceSnapshot: BrainWorkspaceSnapshot | null }) {
  ...
  const contractHash = weeklyPlan?.strategy_contract?.contract_hash;
  const contractState = weeklyPlan?.strategy_contract_freshness?.state ?? (contractHash ? 'provenance recorded' : 'legacy');
  const pillarCounts = Object.entries(weeklyPlan?.pillar_coverage?.counts ?? {});
  const pillarWarnings = weeklyPlan?.pillar_coverage?.warnings ?? [];
      ...
      <div style={{ display: 'grid', gap: '12px' }}>
        <WorkspaceMirrorCard
          ...
          meta={[
            ...
            `Strategy ${humanizeSnakeCase(contractState)}`,
            ...(contractHash ? [`Contract ${contractHash.slice(0, 8)}`] : []),
          ]}
        >
          <div style={{ display: 'grid', gap: '12px' }}>
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'grid', gap: '8px' }}>
                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                  {pillarCounts.map(([pillar, count]) => (
                    <InlineBadge key={pillar} label={`${pillar} ${count}`} tone="#a78bfa" />
                  ))}
                </div>
                {pillarWarnings.map((warning) => (
                  <p key={warning} style={{ color: '#fbbf24', fontSize: '12px', margin: 0 }}>{warning}</p>
                ))}
              </div>
            )}
            <div style={{ display: 'grid', gap: '10px' }}>
                ...
                recommendations.map((item, index) => (
                  <MirrorItemCard
                    ...
                    meta={[
                      ...
                      item.canonical_pillar || null,
                      item.employer_safety ? `safety ${humanizeSnakeCase(item.employer_safety)}` : null,
                      item.proof_posture ? `proof ${humanizeSnakeCase(item.proof_posture)}` : null,
                      item.development_status ? humanizeSnakeCase(item.development_status) : null,
                      typeof item.score === 'number' ? `score ${item.score}` : null,
                    ]}
                    detail={item.audience_consequence || null}
                    href={safeExternalHttpsUrl(item.source_url)}
                    hrefLabel="Open source"
                    composerHref={buildWorkspaceMirrorPostingHref({
                      originType: 'workspace_recommendation',
                      originId: item.source_path || item.source_url || `${item.title}-${index}`,
                      title: item.title,
                      ...
                      routeReason: item.rationale,
                      publishPosture: item.publish_posture,
                      canonicalPillar: item.canonical_pillar,
                      careerSignal: item.career_signal,
                      employerProximity: item.employer_proximity,
                      employerSafety: item.employer_safety,
                      proofPosture: item.proof_posture,
                      audience: item.audience,
                      audienceConsequence: item.audience_consequence,
                      distinctThesis: item.distinct_thesis,
                      whyNow: item.why_now,
              ...
              {feedItems.length > 0 ? (
                feedItems.map((item) => (
                  <MirrorItemCard
                    ...
                    hrefLabel="Open post"
                    composerHref={buildWorkspaceMirrorPostingHref({
                      originType: 'workspace_feed',
                      originId: item.id,
                      title: item.title,
                      summary: item.summary || item.why_it_matters,
                      hook: item.standout_lines?.[0],
                      sourceUrl: item.source_url,
                      sourceKind: item.platform || 'social_feed',
                      routeReason: item.why_it_matters,
                    })}
                  />
                ))
              ) : (
...
function MirrorItemCard({
  ...
  const displayTitle = normalizeBrainDisplayText(title);
  const displayBody = normalizeBrainDisplayText(body);
  return (
    <div style={{ borderRadius: '12px', border: '1px solid #1f2937', backgroundColor: '#030712', padding: '12px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: '8px', alignItems: 'flex-start', marginBottom: '6px', flexWrap: 'wrap' }}>
        <p style={{ color: 'white', fontSize: '14px', fontWeight: 600, margin: 0 }}>{displayTitle}</p>
        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
          {composerHref && (
            <Link href={composerHref} style={{ color: '#22c55e', fontSize: '12px', textDecoration: 'none' }}>
              Draft in FEEZIE
            </Link>
          )}
          {safeExternalHttpsUrl(href) && (
            <a href={safeExternalHttpsUrl(href) ?? undefined} target="_blank" rel="noreferrer" style={{ color: '#38bdf8', fontSize: '12px', textDecoration: 'none' }}>
              {hrefLabel || 'Open'}
            </a>
          )}
        </div>
      </div>
      <p style={{ color: '#cbd5f5', fontSize: '13px', lineHeight: 1.55, margin: '0 0 8px' }}>{displayBody}</p>
      {detail ? <p style={{ color: '#94a3b8', fontSize: '12px', lineHeight: 1.5, margin: '0 0 8px' }}>{normalizeBrainDisplayText(detail)}</p> : null}
      <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
      ...
      <div style={{ flex: '999 1 520px', minWidth: 0 }}>
        {selected ? (
          <div style={{ display: 'grid', gap: '14px' }}>
            ...
            <details style={{ borderRadius: '14px', border: '1px solid #1f2937', backgroundColor: '#020617', padding: '14px' }}>
              <summary style={{ color: '#94a3b8', cursor: 'pointer', fontSize: '12px', fontWeight: 600 }}>
                How this brief was assembled and routed
              </summary>
          ...
          <p style={{ color: '#475569' }}>
            {loadState === 'loading' ? 'Loading the latest saved brief…' : 'Select a brief to read the saved markdown.'}
          </p>
        )}
      </div>
    </section>
...
function BriefStreamPanel({
  ...
  const itemKeySignature = items.map((item) => item.item_key?.trim() || item.title?.trim() || '').join('|');
  ...
  useEffect(() => {
    ...
    setMessage(null);
    setError(null);
  }, [brief.id]); // eslint-disable-line react-hooks/exhaustive-deps -- A new brief resets the owner's draft; polling the same brief must not.
  ...
  async function submitReaction() {
    ...
    setError(null);
    try {
      await controlApiPost('/api/briefs/reactions', {
          brief_id: brief.id,
      ...
      setError(ownerSafeErrorMessage(submitError, 'Unable to save this brief reaction right now.'));
    } finally {
      setSaving(false);
    }
  }
        ...
        {items.map((item) => {
          ...
          return (
            ...
            >
              ...
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                <Link
                  href={buildPostingWorkspaceHref(brief, item, 'post')}
                  style={{
                    borderRadius: '999px',
                ...
                <Link
                  href={buildPostingWorkspaceHref(brief, item, 'comment')}
                  style={{
                    borderRadius: '999px',
                ...
                {safeExternalHttpsUrl(item.source_url) && (
                  <a
                    href={safeExternalHttpsUrl(item.source_url) ?? undefined}
                      target="_blank"
                      rel="noreferrer"
                      style={{
                        borderRadius: '999px',
              ...
              {item.source_path && !item.source_url && (
                <p style={{ color: '#475569', fontSize: '11px', margin: 0 }}>
                  {displayBrainPath(item.source_path)}
                </p>
              )}
        ...
        <div style={{ display: 'grid', gap: '6px' }}>
          {items.map((item, index) => (
            <p key={`${title}-${index}`} style={{ color: '#cbd5f5', fontSize: '13px', lineHeight: 1.45 }}>
              {normalizeBrainDisplayText(item)}
            </p>
          ))}
        </div>
      )}
...
function PersonaPanel({
  ...
  loadState,
  requestedDeltaId,
  requestedDeltaLookupState,
  requestedDeltaLookupError,
  viewportWidth,
  viewportHeight,
  refreshBrainData,
  onSelectDelta,
  ...
  docsLoadState: ResourceLoadState;
  error: string | null;
  loadState: ResourceLoadState;
  requestedDeltaId: string | null;
  requestedDeltaLookupState: RequestedDeltaLookupState;
  requestedDeltaLookupError: string | null;
  viewportWidth: number;
  viewportHeight: number;
  refreshBrainData: () => Promise<void>;
  onSelectDelta: (deltaId: string | null, historyMode?: 'push' | 'replace') => void;
  mergeUpdatedDelta: (updatedDelta: PersonaDeltaEntry) => void;
}) {
  const handledRequestedDeltaId = useRef<string | null>(null);
  const [personaContextByPath, setPersonaContextByPath] = useState<Record<string, DocEntry>>({});
  const [personaContextLoadingPath, setPersonaContextLoadingPath] = useState<string | null>(null);
  const [personaContextError, setPersonaContextError] = useState<string | null>(null);
  ...
  const activeReviewDeltas = useMemo(
    () => deltas.filter((delta) => !completedDeltaIds.includes(delta.id) && personaDeltaStage(delta) === 'brain_pending_review'),
    [completedDeltaIds, deltas],
  );
  const workspaceSavedDeltas = useMemo(() => deltas.filter((delta) => personaDeltaStage(delta) === 'workspace_saved'), [deltas]);
  const pendingPromotionDeltas = useMemo(() => deltas.filter((delta) => personaDeltaStage(delta) === 'pending_promotion'), [deltas]);
  const committedDeltas = useMemo(() => deltas.filter((delta) => personaDeltaStage(delta) === 'committed'), [deltas]);
  ...
  const requestedDelta = useMemo(
    () => (requestedDeltaId ? deltas.find((delta) => delta.id === requestedDeltaId) ?? null : null),
    [deltas, requestedDeltaId],
  );
  const requestedDeltaStage = requestedDelta ? personaDeltaStage(requestedDelta) : null;
  const requestedLifecycleView = lifecycleViewForDeltaStage(requestedDeltaStage);
  const requestedDeltaMissing = Boolean(requestedDeltaId && !requestedDelta && requestedDeltaLookupState === 'not_found');
  const requestedDeltaUnavailable = Boolean(requestedDeltaId && !requestedDelta && requestedDeltaLookupState === 'error');
  const requestedDeltaPending = Boolean(
    requestedDeltaId &&
      !requestedDelta &&
      (loadState === 'loading' || loadState === 'refreshing' || requestedDeltaLookupState === 'idle' || requestedDeltaLookupState === 'loading'),
  );
  ...
  const [personaRefreshState, setPersonaRefreshState] = useState<{ tone: 'idle' | 'success' | 'error'; message: string }>({
    tone: 'idle',
    message: '',
  });
  const [isQueueingPersonaRefresh, setIsQueueingPersonaRefresh] = useState(false);
  ...
  async function queuePersonaReviewRefresh() {
    setIsQueueingPersonaRefresh(true);
    setPersonaRefreshState({ tone: 'idle', message: '' });
    try {
      const payload = await controlApiPost<BrainLocalActionQueuedResponse>('/api/brain/refresh-persona-review', {});
      setPersonaRefreshState({
        tone: 'success',
        message: formatBrainLocalQueueStatus(payload, 'Persona review refresh'),
      });
    } catch (queueError) {
      setPersonaRefreshState({
        tone: 'error',
        message: ownerSafeErrorMessage(queueError, 'Unable to queue the Persona review refresh.'),
      });
    } finally {
      setIsQueueingPersonaRefresh(false);
    }
  }
  ...
  const previousPersonaDeltaId = useMemo(
    () => adjacentPersonaReviewDeltaId(personaSourceGroups, selectedDelta?.id, 'previous'),
    [personaSourceGroups, selectedDelta?.id],
  );
  ...
  const selectActiveDelta = useCallback(
    (deltaId: string | null, historyMode: 'push' | 'replace' = 'push') => {
      setSelectedDeltaId(deltaId ?? '');
      onSelectDelta(deltaId, historyMode);
    },
    [onSelectDelta],
  );
  ...
  const [selectedResponseKind, setSelectedResponseKind] = useState<PersonaResponseKind>('nuance');
  ...
  const personaContextDoc = useMemo(() => findPersonaDocForTarget(docs, contextTargetFile), [contextTargetFile, docs]);
  const personaContextDocPath = personaContextDoc?.path ?? null;
  const loadedPersonaContextDoc = personaContextDocPath ? personaContextByPath[personaContextDocPath] ?? null : null;
  ...
  const relatedOwnerPositions = useMemo(
    () => metadataArray(selectedDelta?.metadata, 'related_owner_positions').filter(isUnknownRecord),
    [selectedDelta],
  );
  const ownerResponseHistory = useMemo(
    () => metadataArray(selectedDelta?.metadata, 'owner_response_history').filter(isUnknownRecord),
    [selectedDelta],
  );
  ...
  const sourceThumbnailUrl = youtubeThumbnailUrl(sourceUrl);
  const sourcePath = metadataText(selectedDelta?.metadata, 'source_path');
  const sourceExcerpt =
  ...
  const sourceDisplayTitle = normalizeBrainDisplayText(sourceTitle);
  const sourceDisplayExcerpt = normalizeBrainDisplayText(sourceExcerpt || selectedDelta?.trait || '');
  const sourceDisplayContextExcerpt = sourceContextExcerpt ? normalizeBrainDisplayText(sourceContextExcerpt) : null;
  ...
  const personaPostingHref = useMemo(
    () =>
      selectedDelta
        ? buildPersonaPostingWorkspaceHref(selectedDelta, {
            sourceTitle,
            sourceExcerpt,
            sourceUrl,
            sourcePath,
            sourceKind: reviewSource || sourceChannel || 'persona_delta',
            ownerReaction: reflectionText.trim() || savedResponseExcerpt || '',
          })
        : '/workspace/posting',
    [reflectionText, reviewSource, savedResponseExcerpt, selectedDelta, sourceChannel, sourceExcerpt, sourcePath, sourceTitle, sourceUrl],
  );
  ...
  const activeLifecycleItems = useMemo(() => {
    const items = activeLifecycleGroup?.items ?? [];
    if (!requestedDeltaId) return items.slice(0, 12);
    const requestedItem = items.find((item) => item.id === requestedDeltaId);
    if (!requestedItem) return items.slice(0, 12);
    return [requestedItem, ...items.filter((item) => item.id !== requestedDeltaId)].slice(0, 12);
  }, [activeLifecycleGroup, requestedDeltaId]);
  const requestedLifecycleTitle = requestedLifecycleView
  ...
  const activeReviewEmptyState =
    loadState === 'loading'
      ? {
          eyebrow: 'Loading',
          title: 'Loading persona review items…',
          message: 'Brain is checking the current review and canon lifecycle queues.',
          tone: '#38bdf8',
        }
      : error
      ? {
          eyebrow: 'Unavailable',
          title: 'The persona queue could not be verified.',
          message: 'The last refresh failed, so an empty review rail does not mean you are done.',
          tone: '#f87171',
        }
      : requestedDelta && requestedLifecycleTitle
      ? {
          eyebrow: 'Requested item opened',
          title: `This item is in ${requestedLifecycleTitle}.`,
          message: 'Its lifecycle record is highlighted in the Canon Audit below; it is not an active review item.',
          tone: '#38bdf8',
        }
      : {
  ...
  useEffect(() => {
    if (!personaContextDocPath || personaContextDoc?.content || personaContextByPath[personaContextDocPath]) {
      setPersonaContextLoadingPath(null);
      setPersonaContextError(null);
      return;
    }
    let cancelled = false;
    setPersonaContextLoadingPath(personaContextDocPath);
    setPersonaContextError(null);
    void controlApiGet<DocEntry>(`/api/brain/docs/content?path=${encodeURIComponent(personaContextDocPath)}`, {
      timeoutMs: BRAIN_DOC_CONTENT_TIMEOUT_MS,
      cache: 'no-store',
    })
      .then((doc) => {
        if (cancelled) return;
        setPersonaContextByPath((current) => ({ ...current, [personaContextDocPath]: doc }));
      })
      .catch((contextError) => {
        if (cancelled) return;
        setPersonaContextError(ownerSafeErrorMessage(contextError, 'Unable to load current canon context.'));
      })
      .finally(() => {
        if (cancelled) return;
        setPersonaContextLoadingPath((current) => (current === personaContextDocPath ? null : current));
      });
    return () => {
      cancelled = true;
    };
  }, [personaContextByPath, personaContextDoc?.content, personaContextDocPath]);

  useEffect(() => {
    if (!requestedDeltaId) {
      handledRequestedDeltaId.current = null;
      return;
    }
    if (!requestedDelta) {
      if (requestedDeltaLookupState === 'idle' || requestedDeltaLookupState === 'loading') return;
      const timeoutId = window.setTimeout(() => {
        document.getElementById('brain-persona-request-status')?.focus({ preventScroll: true });
      }, 0);
      return () => window.clearTimeout(timeoutId);
    }
    if (handledRequestedDeltaId.current === requestedDeltaId) return;

    if (requestedDeltaStage === 'brain_pending_review') {
      if (shouldMuteActivePersonaDelta(requestedDelta, promotionSignalCount(requestedDelta.metadata))) {
        setShowMutedActive(true);
      }
      setSelectedDeltaId(requestedDeltaId);
    } else if (requestedLifecycleView) {
      setLifecycleView(requestedLifecycleView);
      setShowLifecycleAudit(true);
    }
    handledRequestedDeltaId.current = requestedDeltaId;
    const timeoutId = window.setTimeout(() => {
      const target = document.getElementById(personaDeltaElementId(requestedDeltaId));
  ...
  useEffect(() => {
    ...
    const recommendedIds = recommendedSelectableItems.map((item) => item.id);
    const defaultIds = existingIds.length > 0 ? existingIds : recommendedIds;
    const existingResponseKind = metadataText(selectedDelta?.metadata, 'owner_response_kind');

    setSelectedPromotionItemIds(
      (draft ? draft.selectedPromotionItemIds : defaultIds).filter((itemId) => availableIds.has(itemId)),
    );
    setSelectedResponseKind(
      draft?.responseKind ?? (isPersonaResponseKind(existingResponseKind) ? existingResponseKind : 'nuance'),
    );
    setReflectionText(draft?.reflectionText ?? metadataText(selectedDelta?.metadata, 'owner_response_excerpt') ?? '');
    setPromotionItemTargetOverrides(draft?.promotionItemTargetOverrides ?? {});
    ...
    setTriagePMTitle(draft?.triagePMTitle ?? (selectedDelta ? defaultBrainPMTitle(selectedDelta) : ''));
    setHasUnconfirmedPersonaDraft(Boolean(draft));
  ...
  useEffect(() => {
    if (!selectedDelta || personaDraftReadyDeltaId !== selectedDelta.id || !hasUnconfirmedPersonaDraft) return;
    const timeoutId = window.setTimeout(() => {
  ...
  async function postPromotionCommit(deltaId: string) {
    return controlApiPost<BrainPromotionResponse>(`/api/brain/persona-promote/${encodeURIComponent(deltaId)}`, null, {
  ...
  async function commitPromotion(delta: PersonaDeltaEntry) {
    ...
    if (
      ...
      !window.confirm(
        `Commit ${gateSummary.selectedCount} approved canon fragment${gateSummary.selectedCount === 1 ? '' : 's'} to the canonical persona bundle?`,
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (selectedDeltaId === delta.id) {
        selectActiveDelta(reviewQueue[0]?.id ?? null, 'replace');
      }
    } catch (error) {
      setPromotionState({
        tone: 'error',
        message: ownerSafeErrorMessage(error, 'Unable to commit this promotion right now.'),
      });
    } finally {
  ...
  async function reroutePromotion(delta: PersonaDeltaEntry, targetFile: string) {
    ...
    try {
      const payload = await controlApiPost<BrainPromotionRerouteResponse>(
        `/api/brain/persona-reroute/${encodeURIComponent(delta.id)}`,
      ...
      setPromotionState({
        tone: 'error',
        message: ownerSafeErrorMessage(error, 'Unable to reroute this promotion right now.'),
      });
    } finally {
  ...
  function markPersonaReviewDraftUnconfirmed() {
    setHasUnconfirmedPersonaDraft(true);
    setPersonaDraftStorageError(null);
  ...
  function queueTemplate(kind: PersonaResponseKind) {
    if (!selectedDelta) {
      return;
    }
    markPersonaReviewDraftUnconfirmed();
  ...
  async function saveReflection(
    mode: 'reviewed' | 'approved' = 'reviewed',
    options: {
      routeAfterSave?: boolean;
      advanceAfterSave?: boolean;
      nextDeltaId?: string | null;
      advanceLabel?: 'claim' | 'source' | 'review item';
    } = {},
  ) {
    ...
    const effectiveReflection =
      ...
      (mode === 'approved' && effectivePromotionItems.length > 0
        ...
        : '');
    const keepSelectableSourceOpen = mode === 'reviewed' && selectableItems.length > 0 && !routeAfterSave && !advanceAfterSave;
    ...
    try {
      ...
      const result = await controlApiPost<CaptureResponsePayload>('/api/capture/', payload);
      if (selectedDelta) {
        const reviewPayload = {
          mode,
        ...
        if (routeAfterSave) {
          try {
            ...
            routeFailureMessage =
              ownerSafeErrorMessage(routeError, 'Unable to route this reviewed signal right now.');
            setTriageState({
              tone: 'error',
  ...
  function buildRouteResultMessage(result: BrainSystemRouteResponse) {
    ...
    const standupRoutes = (result.routes || []).filter((entry) => entry.standup?.id);
    if (standupRoutes.length > 0 || result.standup?.id) {
      const allReused = standupRoutes.length > 0 && standupRoutes.every((entry) => entry.standup_reused);
      const someReused = standupRoutes.some((entry) => entry.standup_reused);
      routeBits.push(
        `${allReused ? 'existing standup reused' : someReused ? 'standups ready (new and existing)' : 'standup queued'} for ${triageWorkspaceSelection.length} workspace${triageWorkspaceSelection.length === 1 ? '' : 's'}`,
      );
    }
    const pmRoutes = (result.routes || []).filter((entry) => entry.pm_card?.id);
    if (pmRoutes.length > 0 || result.pm_card?.id) {
      const allReused = pmRoutes.length > 0 && pmRoutes.every((entry) => entry.pm_card_reused);
      const someReused = pmRoutes.some((entry) => entry.pm_card_reused);
      routeBits.push(
        `${allReused ? 'existing PM work reused' : someReused ? 'PM work ready (new and existing)' : 'PM work created'} for ${triageWorkspaceSelection.length} workspace${triageWorkspaceSelection.length === 1 ? '' : 's'}`,
      );
    }
    return routeBits.length > 0
      ...
      : ownerSafeErrorMessage(result.message, 'Brain triage updated.');
  }
  ...
  async function submitRouteForDelta(deltaId: string, effectiveReflection: string, promotionItems: PromotionItem[] = selectedPromotionItems) {
    const result = await controlApiPost<BrainSystemRouteResponse>(
      `/api/brain/system-route/${encodeURIComponent(deltaId)}`,
      {
  ...
  async function routeSelectedSignal() {
    ...
    try {
      ...
      setTriageState({
        tone: 'error',
        message: ownerSafeErrorMessage(routeError, 'Unable to route this reviewed signal right now.'),
      });
    } finally {
      setIsRoutingSignal(false);
    }
  ...
  async function finalizeReviewedSignal(
    options: { advanceAfterSave?: boolean; nextDeltaId?: string | null; advanceLabel?: 'claim' | 'source' | 'review item' } = {},
  ) {
    if (!selectedDelta) {
    ...
    if (
      ...
      typeof window !== 'undefined' &&
      !window.confirm(
        `Finalize and write ${canonActionItems.length} approved canon fragment${canonActionItems.length === 1 ? '' : 's'} to the canonical persona bundle now?`,
      )
    ) {
      return;
    }
    await saveReflection(canonActionItems.length > 0 ? 'approved' : 'reviewed', {
      ...
      advanceAfterSave: options.advanceAfterSave,
      nextDeltaId: options.nextDeltaId,
      advanceLabel: options.advanceLabel,
    });
  }

  function persistCurrentPersonaDraftNow() {
    if (!selectedDelta || !hasUnconfirmedPersonaDraft) return true;
    const stored = writePersonaReviewDraft({
      schema: 'brain_persona_review_draft/v1',
      deltaId: selectedDelta.id,
      reflectionText,
  ...
  function selectMobilePersonaDelta(deltaId: string | null) {
    if (!deltaId) return;
    if (deltaId === selectedDelta?.id) {
      setMobilePersonaSheet(null);
      return;
    }
  ...
  async function saveMobilePersonaReview() {
    await saveReflection('reviewed', {
      ...
      nextDeltaId: nextPersonaDeltaId,
      advanceLabel: mobileAdvanceLabel,
    });
  }

  async function finalizeMobilePersonaReview() {
    setMobilePersonaSheet(null);
    await finalizeReviewedSignal({
      advanceAfterSave: true,
      nextDeltaId: nextPersonaDeltaId,
      advanceLabel: mobileAdvanceLabel,
    });
  ...
  const mobileClaimNumber = (selectedPersonaPosition?.claimIndex ?? 0) + 1;
      ...
      >
        ...
        >
          ...
          <div style={{ minWidth: 0, textAlign: 'center' }}>
            ...
            <p style={{ color: '#64748b', fontSize: '11px', margin: '2px 0 0' }}>
              Source {mobileSourceNumber} of {personaSourceGroups.length || 1}
            </p>
          </div>
          <span style={{ color: '#38bdf8', fontSize: '12px', whiteSpace: 'nowrap' }}>
            {selectedDelta ? `${mobileClaimNumber} of ${mobileClaimCount}` : '—'}
          </span>
        </header>
        ...
        {selectedDelta ? (
          <div style={{ minHeight: 0, overflowY: 'auto', padding: '8px 9px', display: 'grid', alignContent: 'start', gap: '6px' }}>
            ...
            >
              ...
              >
                {sourceThumbnailUrl ? (
                  <Image src={sourceThumbnailUrl} alt="" fill sizes="92px" style={{ objectFit: 'cover' }} />
                ) : (
                  sourceChannel || 'Source'
                )}
              </div>
              <div style={{ minWidth: 0 }}>
                <p style={{ color: '#38bdf8', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.1em', margin: '0 0 3px' }}>
                  {sourceChannel || 'Source'}
                </p>
                <p
                  style={{
              ...
              {safeExternalHttpsUrl(sourceUrl) ? (
                <a
                  href={safeExternalHttpsUrl(sourceUrl) ?? undefined}
                  target="_blank"
                  rel="noreferrer"
                  aria-label="Open source"
                  style={{ ...personaPhoneIconButtonStyle(), display: 'grid', placeItems: 'center', textDecoration: 'none' }}
                >
                  ↗
                </a>
              ) : (
            ...
            <div
              role="progressbar"
              aria-label="Claim progress for this source"
              aria-valuemin={1}
              aria-valuemax={Math.max(1, mobileClaimCount)}
              aria-valuenow={mobileClaimNumber}
              style={{ height: '4px', overflow: 'hidden', borderRadius: '999px', backgroundColor: '#1e293b' }}
            >
            ...
            <article
              onTouchStart={(event) => {
                personaSwipeStartX.current = event.changedTouches[0]?.clientX ?? null;
              }}
              onTouchEnd={(event) => {
                if (personaSwipeStartX.current === null) return;
                const endX = event.changedTouches[0]?.clientX ?? personaSwipeStartX.current;
                const distance = endX - personaSwipeStartX.current;
                personaSwipeStartX.current = null;
                if (Math.abs(distance) < 45) return;
                selectMobilePersonaDelta(distance < 0 ? nextPersonaDeltaId : previousPersonaDeltaId);
              }}
              style={{
                boxSizing: 'border-box',
                borderRadius: '12px',
            ...
            >
              <p style={{ color: '#67e8f9', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.1em', margin: 0 }}>
                {reviewSource === 'long_form_media.segment' ? 'External source claim' : 'Persona claim'}
              </p>
              <p style={{ color: '#f8fafc', fontSize: '15px', lineHeight: 1.4, margin: 0 }}>
            ...
            <section style={{ display: 'grid', gap: '5px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: '8px', alignItems: 'baseline' }}>
                <p style={{ color: '#e2e8f0', fontSize: '12px', fontWeight: 700, margin: 0 }}>AI found these useful</p>
                <span style={{ color: '#94a3b8', fontSize: '11px' }}>{selectedPromotionItems.length} selected</span>
              </div>
              <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap' }}>
                {rankedSelectableItems.slice(0, 3).map((item) => {
                  ...
                  return (
                    <button
                      ...
                      style={personaPhoneChipStyle(checked)}
                    >
                      {humanizePromotionKind(item.kind)}
                    </button>
                  );
                })}
                {rankedSelectableItems.length === 0 && <span style={{ color: '#64748b', fontSize: '11px' }}>No canon fragments suggested.</span>}
                {rankedSelectableItems.length > 3 && (
                  <button type="button" onClick={() => setMobilePersonaSheet('details')} style={personaPhoneChipStyle(false)}>
                    +{rankedSelectableItems.length - 3} details
                  </button>
                )}
              </div>
            </section>
            ...
            <section style={{ display: 'grid', gap: '5px' }}>
              <p style={{ color: '#e2e8f0', fontSize: '12px', fontWeight: 700, margin: 0 }}>Your response</p>
              <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap' }}>
                {([
                  ['agree', 'Agree'],
                  ['disagree', 'Disagree'],
                  ...
                  <button key={kind} type="button" aria-pressed={selectedResponseKind === kind} onClick={() => queueTemplate(kind)} style={personaPhoneChipStyle(selectedResponseKind === kind)}>
                    {label}
                  </button>
                ))}
              </div>
              <textarea
                aria-label="Your persona review response"
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: '8px', alignItems: 'center' }}>
              <button type="button" onClick={() => setMobilePersonaSheet('details')} style={personaPhoneChipStyle(false)}>
                Review details · {selectedPromotionItems.length} selected
              </button>
              <span style={{ color: '#64748b', fontSize: '10px', textAlign: 'right' }}>Canon + routing</span>
            </div>

            <p
              role={personaDraftStorageError || reflectionState.tone === 'error' ? 'alert' : 'status'}
              aria-live="polite"
              style={{
                minHeight: '16px',
                color: personaDraftStorageError || reflectionState.tone === 'error' ? '#f87171' : '#38bdf8',
                fontSize: '11px',
                lineHeight: 1.35,
                margin: 0,
              }}
          ...
          <div style={{ display: 'grid', placeItems: 'center', padding: '24px', textAlign: 'center' }}>
            <div>
              <p style={{ color: activeReviewEmptyState.tone, fontSize: '12px', textTransform: 'uppercase', letterSpacing: '0.12em', margin: '0 0 8px' }}>
                {activeReviewEmptyState.eyebrow}
              </p>
              <h3 style={{ color: '#f8fafc', fontSize: '22px', margin: '0 0 8px' }}>{activeReviewEmptyState.title}</h3>
              <p style={{ color: '#94a3b8', fontSize: '13px', lineHeight: 1.55, margin: 0 }}>{activeReviewEmptyState.message}</p>
            </div>
          </div>
        )}
        ...
        >
          ...
          <button
            ...
            style={{
              ...
              fontSize: '14px',
              fontWeight: 700,
              cursor: mobileSaveDisabled ? 'not-allowed' : 'pointer',
            }}
          >
            {mobileSaveLabel}
          </button>
        ...
        {mobilePersonaSheet && (
          <div
            ...
            onClick={(event) => {
              if (event.currentTarget === event.target) setMobilePersonaSheet(null);
            }}
          >
            <section
              role="dialog"
              aria-modal="true"
              aria-label={mobilePersonaSheet === 'queue' ? 'Persona source queue' : 'Persona review details'}
              style={{
                boxSizing: 'border-box',
                width: '100%',
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: '8px', alignItems: 'center' }}>
                <div>
                  <p style={{ color: '#38bdf8', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.1em', margin: '0 0 3px' }}>
                    {mobilePersonaSheet === 'queue' ? 'Source queue' : 'Advanced controls'}
                  </p>
                  <h3 style={{ color: '#f8fafc', fontSize: '18px', margin: 0 }}>
                    {mobilePersonaSheet === 'queue' ? `${personaSourceGroups.length} sources ready` : 'Review details'}
                  </h3>
                </div>
                <button type="button" aria-label="Close" onClick={() => setMobilePersonaSheet(null)} style={personaPhoneIconButtonStyle()}>
                  ×
                </button>
              </div>

              {mobilePersonaSheet === 'queue' ? (
                <>
                  <div style={{ display: 'grid', gap: '8px' }}>
                    {personaSourceGroups.map((group, sourceIndex) => {
                      const representative = group.deltas[0];
                      const active = group.deltas.some((delta) => delta.id === selectedDelta?.id);
                      return (
                        <button
                          key={group.key}
                          type="button"
                          aria-pressed={active}
                          onClick={() => selectMobilePersonaDelta(active ? selectedDelta?.id ?? representative.id : representative.id)}
                          style={{
                            borderRadius: '12px',
                            border: `1px solid ${active ? '#38bdf8' : '#1f2937'}`,
                            backgroundColor: active ? '#082f49' : '#010617',
                            color: '#e2e8f0',
                            padding: '10px 11px',
                            textAlign: 'left',
                            display: 'grid',
                        ...
                        >
                          <span style={{ color: '#38bdf8', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.1em' }}>
                            Source {sourceIndex + 1} · {group.deltas.length} claim{group.deltas.length === 1 ? '' : 's'}
                          </span>
                          <span style={{ fontSize: '13px', fontWeight: 700 }}>{normalizeBrainDisplayText(reviewQueueCardTitle(representative))}</span>
                          <span style={{ color: '#94a3b8', fontSize: '11px', lineHeight: 1.45 }}>{truncateText(normalizeBrainDisplayText(reviewQueueCardSummary(representative)), 140)}</span>
                        </button>
                      );
                    })}
                  </div>
                  <button
                    ...
                    disabled={isQueueingPersonaRefresh}
                    style={brainSmallButtonStyle(isQueueingPersonaRefresh)}
                  >
                    {isQueueingPersonaRefresh ? 'Queueing…' : 'Refresh Persona Queue on Mac'}
                  </button>
                  {personaRefreshState.message && (
                    <p
                      role={personaRefreshState.tone === 'success' ? 'status' : 'alert'}
                      aria-live={personaRefreshState.tone === 'success' ? 'polite' : undefined}
                      style={{
                        color: personaRefreshState.tone === 'success' ? '#22c55e' : '#f87171',
                        fontSize: '11px',
                        lineHeight: 1.5,
                        margin: 0,
                      }}
                ...
                <>
                  <section style={{ display: 'grid', gap: '7px', paddingTop: '9px', borderTop: '1px solid #1f2937' }}>
                    <p style={{ color: '#e2e8f0', fontSize: '12px', fontWeight: 700, margin: 0 }}>Source call</p>
                    <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                  ...
                  {rankedSelectableItems.length > 0 && (
                    <section style={{ display: 'grid', gap: '7px', paddingTop: '9px', borderTop: '1px solid #1f2937' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', gap: '8px' }}>
                        <p style={{ color: '#e2e8f0', fontSize: '12px', fontWeight: 700, margin: 0 }}>Canon fragments</p>
                        <span style={{ color: '#94a3b8', fontSize: '11px' }}>{selectedPromotionItems.length} selected</span>
                      </div>
                      <div style={{ display: 'grid', gap: '7px' }}>
                        {rankedSelectableItems.map((item) => {
                          const checked = selectedPromotionItemIds.includes(item.id);
                          return (
                            <button
                              key={item.id}
                              ...
                              style={{
                                borderRadius: '11px',
                                border: `1px solid ${checked ? '#38bdf8' : '#334155'}`,
                                backgroundColor: checked ? '#082f49' : '#010617',
                                color: '#e2e8f0',
                                padding: '9px 10px',
                                textAlign: 'left',
                                display: 'grid',
                            ...
                            >
                              <span style={{ fontSize: '12px', fontWeight: 700 }}>{checked ? '✓ ' : ''}{humanizePromotionKind(item.kind)}</span>
                              <span style={{ color: '#94a3b8', fontSize: '11px', lineHeight: 1.45 }}>{truncateText(normalizeBrainDisplayText(item.content), 180)}</span>
                            </button>
                          );
                        })}
                      </div>
                  ...
                  <section style={{ display: 'grid', gap: '7px', paddingTop: '9px', borderTop: '1px solid #1f2937' }}>
                    <p style={{ color: '#e2e8f0', fontSize: '12px', fontWeight: 700, margin: 0 }}>Optional routing</p>
                    <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                      <button type="button" aria-pressed={routeToMemory} onClick={() => toggleRouteTarget('memory')} style={personaPhoneChipStyle(routeToMemory)}>Canonical memory</button>
                      <button type="button" aria-pressed={routeToStandup} onClick={() => toggleRouteTarget('standup')} style={personaPhoneChipStyle(routeToStandup)}>Standup queue</button>
                      <button type="button" aria-pressed={routeToPM} onClick={() => toggleRouteTarget('pm')} style={personaPhoneChipStyle(routeToPM)}>PM queue</button>
                    </div>
                    {routeToStandup && (
                      <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap' }}>
                        {brainStandupKindOptions.map((kind) => (
                          <button
                            key={kind}
                            type="button"
                            aria-pressed={triageStandupKind === kind}
                            onClick={() => {
                              markPersonaReviewDraftUnconfirmed();
                              setTriageStandupKind(kind);
                            }}
                            style={personaPhoneChipStyle(triageStandupKind === kind)}
                          >
                    ...
                    {routeToPM && (
                      <input
                        ...
                        onChange={(event) => {
                          markPersonaReviewDraftUnconfirmed();
                          setTriagePMTitle(event.target.value);
                        }}
                        aria-label="PM title"
                  ...
                  {(sourceContextExcerpt || relatedOwnerPositions.length > 0 || ownerResponseHistory.length > 1) && (
                    <details style={{ paddingTop: '9px', borderTop: '1px solid #1f2937' }}>
                      <summary style={{ color: '#38bdf8', fontSize: '12px', cursor: 'pointer' }}>Context and prior positions</summary>
                      {sourceDisplayContextExcerpt && <p style={{ color: '#cbd5f5', fontSize: '12px', lineHeight: 1.5 }}>{truncateText(sourceDisplayContextExcerpt, 700)}</p>}
                      {relatedOwnerPositions.slice(0, 3).map((position, index) => (
                        <p key={readRecordString(position, 'delta_id') || index} style={{ color: '#94a3b8', fontSize: '12px', lineHeight: 1.5 }}>
                          Position {index + 1}: {truncateText(readRecordString(position, 'excerpt') || 'Saved owner position', 260)}
                        </p>
                      ))}
                    </details>
                  )}
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', paddingTop: '4px' }}>
                    <button type="button" onClick={() => setMobilePersonaSheet(null)} style={personaPhoneChipStyle(false)}>Done</button>
                    <button
                      type="button"
                      onClick={() => void finalizeMobilePersonaReview()}
                      ...
                      style={{
                        ...
                        minHeight: '38px',
                        fontWeight: 700,
                        cursor: isFinalizePending || finalizeActionDisabled ? 'not-allowed' : 'pointer',
                      }}
                    >
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: compactPersonaChrome ? '10px' : '16px', alignItems: 'flex-start', flexWrap: 'wrap' }}>
          ...
          <div
            style={{
              display: 'grid',
              justifyItems: compactPersonaChrome ? 'start' : 'end',
              gap: '8px',
        ...
        {personaRefreshState.message && (
          <p
            role={personaRefreshState.tone === 'success' ? 'status' : 'alert'}
            aria-live={personaRefreshState.tone === 'success' ? 'polite' : undefined}
            style={{ color: personaRefreshState.tone === 'success' ? '#22c55e' : '#f87171', fontSize: '12px', margin: 0 }}
          >
            {personaRefreshState.message}
          </p>
        )}
        ...
        {requestedDeltaId && (
          <div
            id="brain-persona-request-status"
            role={requestedDeltaMissing || requestedDeltaUnavailable ? 'alert' : 'status'}
            aria-live="polite"
            tabIndex={-1}
            style={{
              borderRadius: '12px',
              border: `1px solid ${requestedDeltaMissing || requestedDeltaUnavailable ? '#7f1d1d' : '#0e7490'}`,
              backgroundColor: requestedDeltaMissing || requestedDeltaUnavailable ? '#450a0a55' : '#082f4955',
              color: requestedDeltaMissing || requestedDeltaUnavailable ? '#fecaca' : '#bae6fd',
              padding: '10px 12px',
              fontSize: '13px',
              lineHeight: 1.55,
              outline: 'none',
          ...
          >
            {requestedDeltaPending
              ? 'Loading the requested persona item…'
              : requestedDeltaUnavailable
              ? `The requested persona item could not be loaded. ${requestedDeltaLookupError || 'Retry when the connection recovers.'}`
              : requestedDeltaMissing
              ? 'The requested persona item is not in the current 50-item Brain queue. It may have moved, been resolved, or fallen outside this view.'
              : requestedDeltaStage === 'brain_pending_review'
              ? 'The requested persona item is selected in Active Review.'
              : requestedLifecycleTitle
              ? `The requested persona item is highlighted in ${requestedLifecycleTitle} below.`
              : 'The requested persona item loaded, but its current lifecycle state is not shown in this queue.'}
          </div>
        )}
        ...
        {selectedDelta ? (
              ...
              <div style={{ minHeight: 0, overflowY: 'auto', paddingRight: '4px' }}>
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', alignItems: 'flex-start', flexWrap: 'wrap' }}>
                    ...
                    {safeExternalHttpsUrl(sourceUrl) && (
                      <a href={safeExternalHttpsUrl(sourceUrl) ?? undefined} target="_blank" rel="noreferrer" style={{ ...brainLinkButtonStyle, justifySelf: 'start', whiteSpace: 'nowrap' }}>
                        Open source
                      </a>
                    )}
                  </div>
                  ...
                  {relatedOwnerPositions.length > 0 && (
                    <div style={{ borderRadius: '12px', border: '1px solid #0e7490', backgroundColor: '#082f4933', padding: '10px 12px', display: 'grid', gap: '8px' }}>
                      <div>
                        <p style={{ color: '#67e8f9', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.1em', margin: '0 0 5px' }}>Prior owner positions</p>
                        <p style={{ color: '#94a3b8', fontSize: '12px', lineHeight: 1.55, margin: 0 }}>
                          These are your saved responses, not external claims or automatic canon. Use them to say what held, changed, or became more nuanced.
                        </p>
                      </div>
                      {relatedOwnerPositions.map((position, index) => {
                        const positionSequence = typeof position.position_sequence === 'number' ? position.position_sequence : index + 1;
                        const sourceTitle = readRecordString(position, 'source_title') || 'Related source';
                        const responseKind = readRecordString(position, 'response_kind') || 'nuance';
                        const excerpt = readRecordString(position, 'excerpt');
                        const createdAt = readRecordString(position, 'created_at');
                        return (
                          <div key={readRecordString(position, 'delta_id') || `${sourceTitle}-${index}`} style={{ borderRadius: '10px', border: '1px solid #164e63', backgroundColor: '#020617', padding: '9px 10px' }}>
                            <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginBottom: '6px' }}>
                              <InlineBadge label={`Position ${positionSequence}`} tone="#38bdf8" />
                              <InlineBadge label={humanizeSavedResponseKind(responseKind)} tone="#818cf8" />
                              {createdAt && <InlineBadge label={formatTimestamp(createdAt)} tone="#64748b" />}
                            </div>
                            <p style={{ color: '#e2e8f0', fontSize: '12px', fontWeight: 600, margin: '0 0 4px' }}>{sourceTitle}</p>
                            {excerpt && <p style={{ color: '#cbd5f5', fontSize: '12px', lineHeight: 1.55, margin: 0 }}>{truncateText(excerpt, 360)}</p>}
                          </div>
                        );
                      })}
                    </div>
                  )}
                  {ownerResponseHistory.length > 1 && (
                    <div style={{ borderRadius: '12px', border: '1px solid #4338ca', backgroundColor: '#1e1b4b22', padding: '10px 12px', display: 'grid', gap: '8px' }}>
                      <div>
                        <p style={{ color: '#a5b4fc', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.1em', margin: '0 0 5px' }}>This response over time</p>
                        <p style={{ color: '#94a3b8', fontSize: '12px', lineHeight: 1.55, margin: 0 }}>
                          Earlier answers remain visible. The system does not decide whether they conflict, reinforce one another, or represent a changed view.
                        </p>
                      </div>
                      {ownerResponseHistory.map((entry, index) => {
                        const revision = typeof entry.revision === 'number' ? entry.revision : index + 1;
                        const responseKind = readRecordString(entry, 'response_kind') || 'nuance';
                        const excerpt = readRecordString(entry, 'excerpt');
                        const recordedAt = readRecordString(entry, 'recorded_at');
                        return (
                          <div key={`${revision}-${recordedAt || index}`} style={{ borderRadius: '10px', border: '1px solid #312e81', backgroundColor: '#020617', padding: '9px 10px' }}>
                            <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginBottom: excerpt ? '6px' : 0 }}>
                              <InlineBadge label={`Revision ${revision}`} tone="#818cf8" />
                              <InlineBadge label={humanizeSavedResponseKind(responseKind)} tone="#a78bfa" />
                              {recordedAt && <InlineBadge label={formatTimestamp(recordedAt)} tone="#64748b" />}
                            </div>
                            {excerpt && <p style={{ color: '#cbd5f5', fontSize: '12px', lineHeight: 1.55, margin: 0 }}>{truncateText(excerpt, 360)}</p>}
                          </div>
                        );
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '10px' }}>
                      ...
                      <button
                        ...
                        onClick={() => {
                          markPersonaReviewDraftUnconfirmed();
                          setSelectedPromotionItemIds(recommendedSelectableItems.map((item) => item.id));
                        }}
                ...
                <details>
                  ...
                  <div style={{ marginTop: '12px' }}>
                    {personaContextDocPath && personaContextLoadingPath === personaContextDocPath ? (
                      <p role="status" style={{ color: '#94a3b8', fontSize: '13px', lineHeight: 1.55 }}>
                        Loading the matching canon file through authenticated Brain Docs…
                      </p>
                    ) : personaContextError ? (
                      <p role="alert" style={{ color: '#f87171', fontSize: '13px', lineHeight: 1.55 }}>
                        Current canon could not be loaded: {personaContextError}
                      </p>
                    ) : activeContext ? (
                      <>
                        <p style={{ color: '#94a3b8', fontSize: '12px', marginBottom: '8px', overflowWrap: 'anywhere', wordBreak: 'break-word' }}>
                          {activeContextPath ? displayBrainPath(activeContextPath) : 'Canonical bundle excerpt'}
                        </p>
                      ...
                      <p role="status" style={{ color: '#94a3b8', fontSize: '13px', lineHeight: 1.55 }}>
                        Loading the authenticated canon index…
                      </p>
                    ) : docsError ? (
                      <p role="alert" style={{ color: '#f87171', fontSize: '13px', lineHeight: 1.55 }}>
                        Current canon is unavailable because the Brain Docs index could not be loaded.
                      </p>
                    ) : contextTargetFile && !personaContextDoc ? (
                      <p style={{ color: '#64748b', fontSize: '13px', lineHeight: 1.55 }}>
                        No authenticated canon document matches {displayBrainPath(contextTargetFile)}. Brain will not substitute an embedded snapshot.
                      </p>
                    ) : (
                      <p style={{ color: '#475569', fontSize: '13px', lineHeight: 1.55 }}>
                        No canonical target is attached to this item yet. Use your response to say where it belongs and how it should be phrased.
                      </p>
                    )}
                  </div>
                </details>
              ...
              >
                ...
                <div style={{ display: 'grid', gap: '8px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                    ...
                    <QuickFillButton label="Post idea" onClick={() => seedSourceDecision('post_seed')} />
                    <QuickFillButton label="Memory / canon" onClick={() => seedSourceDecision('canon_candidate')} />
                    <Link href={personaPostingHref} style={brainLinkButtonStyle}>
                      Draft in FEEZIE
                    </Link>
                  </div>
                </div>
                ...
                <div style={{ display: 'grid', gap: '10px', padding: '10px', borderRadius: '12px', border: '1px solid #1f2937', backgroundColor: '#010617' }}>
                    ...
                    <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                      {brainWorkspaceOptions.map((workspace) => (
                        <button
                          key={workspace.key}
                          type="button"
                          aria-pressed={triageWorkspaceSelection.includes(workspace.key)}
                  ...
                  <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                    <button type="button" aria-pressed={routeToMemory} onClick={() => toggleRouteTarget('memory')} style={triageToggleButtonStyle(routeToMemory)}>
                      Canonical Memory
                    </button>
                    <button type="button" aria-pressed={routeToStandup} onClick={() => toggleRouteTarget('standup')} style={triageToggleButtonStyle(routeToStandup)}>
                      Standup Queue
                    </button>
                    <button type="button" aria-pressed={routeToPM} onClick={() => toggleRouteTarget('pm')} style={triageToggleButtonStyle(routeToPM)}>
                      PM Queue
                    </button>
                  </div>
                  {routeToMemory && (
                      ...
                      <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                        {canonicalMemoryRouteOptions.map((target) => (
                          <button
                            key={target}
                            type="button"
                            aria-pressed={triageMemoryTargets.includes(target)}
          ...
          >
            <div style={{ maxWidth: '560px' }}>
              <p style={{ color: activeReviewEmptyState.tone, fontSize: '14px', textTransform: 'uppercase', letterSpacing: '0.18em', marginBottom: '10px' }}>
                {activeReviewEmptyState.eyebrow}
              </p>
              <h3 style={{ color: 'white', fontSize: '28px', margin: '0 0 10px' }}>{activeReviewEmptyState.title}</h3>
              <p style={{ color: '#94a3b8', fontSize: '14px', lineHeight: 1.65 }}>
                {activeReviewEmptyState.message}
              </p>
              {(error || requestedDeltaMissing) && (
                <button
                  type="button"
                  onClick={() => void refreshBrainData()}
                  disabled={loadState === 'loading' || loadState === 'refreshing'}
                  style={brainSmallButtonStyle(loadState === 'loading' || loadState === 'refreshing')}
                >
                  Retry queue refresh
                </button>
              )}
              {reflectionState.message && (
                <p
                  role={reflectionState.tone === 'success' ? 'status' : 'alert'}
                  aria-live={reflectionState.tone === 'success' ? 'polite' : undefined}
                  style={{ color: reflectionState.tone === 'success' ? '#22c55e' : '#f87171', fontSize: '12px', marginTop: '12px' }}
                >
                  {reflectionState.message}
                </p>
              )}
          ...
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: '16px', alignItems: 'flex-start', flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                <button
                  type="button"
                  aria-expanded={showLifecycleAudit}
            ...
            <div style={{ display: 'grid', gap: '8px', minHeight: 0, overflowY: 'auto', paddingRight: '2px' }}>
              {activeLifecycleItems.length === 0 ? (
                ...
                activeLifecycleItems.map((item) => (
                  ...
                  <div
                    key={item.id}
                    id={personaDeltaElementId(item.id)}
                    tabIndex={-1}
                    style={{
                      borderRadius: '14px',
                      border: `1px solid ${item.id === requestedDeltaId ? '#f59e0b' : isRecentlyCommitted ? '#22c55e' : isRecentlyQueued ? '#38bdf8' : '#1f2937'}`,
                      padding: '10px',
                      backgroundColor: isRecentlyCommitted ? '#052e1b' : isRecentlyQueued ? '#082f49' : '#0b1220',
                      boxShadow: item.id === requestedDeltaId ? '0 0 0 2px #f59e0b33' : 'none',
                      outline: 'none',
                    }}
                  >
...
function CaptureTelemetryPanel({
  ...
  if (error) {
    return (
      <section style={{ borderRadius: '16px', border: '1px solid #7c2d12', backgroundColor: '#050b19', padding: '20px' }}>
        <p role="alert" style={{ color: '#fbbf24', margin: 0 }}>{error}</p>
        {metrics && <p style={{ color: '#94a3b8', fontSize: '12px', margin: '8px 0 0' }}>The last capture metrics are retained, but service health is unknown rather than offline.</p>}
      </section>
    );
  }

  const storageLabel = health?.storage_backend === 'pgvector' ? 'Native vector mode' : health?.storage_backend === 'jsonb' ? 'JSON fallback' : 'Unknown';
  const serviceLabel = health ? (health.search_ready ? 'Search ready' : health.database_connected ? 'Memory store online' : 'Memory store offline') : 'Health not verified';
  const subtitle =
  ...
  return (
    <section style={{ borderRadius: '16px', border: '1px solid #1f2937', backgroundColor: '#050b19', padding: '20px' }}>
      <div style={{ marginBottom: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
        ...
        <p style={{ color: health ? (health.database_connected ? '#22c55e' : '#f87171') : '#94a3b8', fontSize: '12px' }}>
          {serviceLabel}
        </p>
      </div>
...
function TelemetryMeta({ label, value, detail }: { label: string; value: string; detail: string }) {
  const displayValue = normalizeBrainDisplayText(value);
  const displayDetail = normalizeBrainDisplayText(detail);
  return (
    <div style={{ padding: '12px 14px', borderRadius: '14px', border: '1px solid #1f2937', backgroundColor: '#020617' }}>
      <p style={{ color: '#94a3b8', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.1em' }}>{label}</p>
      <p style={{ color: '#e2e8f0', fontSize: '18px', fontWeight: 600 }}>{displayValue}</p>
      <p style={{ color: '#475569', fontSize: '12px' }}>{displayDetail}</p>
    </div>
  );
}

function ExecutiveFrontDoorCard({ label, value, detail, tone }: { label: string; value: number | string; detail: string; tone: string }) {
  return (
    <div
      style={{
        padding: '14px',
        borderRadius: '14px',
        border: `1px solid ${tone}44`,
        background: `linear-gradient(145deg, ${tone}18, rgba(2,6,23,0.72))`,
      }}
    >
      <p style={{ color: tone, fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.12em', margin: '0 0 6px' }}>{label}</p>
      <p style={{ color: 'white', fontSize: '28px', fontWeight: 700, margin: '0 0 6px' }}>{value}</p>
      <p style={{ color: '#94a3b8', fontSize: '12px', lineHeight: 1.45, margin: 0 }}>{detail}</p>
    </div>
  );
}

function TelemetryStat({ label, value, detail, tone }: { label: string; value: number | string; detail: string; tone: string }) {
  return (
    <div style={{ padding: '12px 14px', borderRadius: '14px', border: '1px solid #1f2937', backgroundColor: '#020617' }}>
      <p style={{ color: '#94a3b8', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.1em' }}>{label}</p>
...
function AutomationsPanel({
  ...
  error: string | null;
  loadState: ResourceLoadState;
  index: AutomationsIndexPayload | null;
}) {
    ...
    <section style={{ borderRadius: '16px', border: '1px solid #1f2937', backgroundColor: '#050b19', padding: '20px', display: 'grid', gap: '14px' }}>
      <div style={{ marginBottom: '12px', display: 'flex', justifyContent: 'space-between', gap: '12px', alignItems: 'flex-start', flexWrap: 'wrap' }}>
        <div>
          ...
          <p style={{ color: '#64748b', fontSize: '13px' }}>Configuration and observed runtime are shown separately. “Configured” never means a job was installed, ran, or delivered successfully.</p>
        </div>
        <InlineBadge
          label={error ? (hasRows ? 'Degraded · retained evidence' : 'Runtime unverified') : loadState === 'refreshing' ? 'Refreshing runtime evidence' : 'Observed run ledger'}
          tone={error ? '#f97316' : '#22c55e'}
        />
      </div>
      ...
      <div style={{ overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {!hasRows && loadState !== 'loading' && (
              <tr>
                <td colSpan={7} style={{ padding: '12px 0', color: '#475569' }}>No verified automation contracts were returned.</td>
              </tr>
            )}
            {hasRows &&
              automations.map((job) => (
                <tr key={job.id}>
                  ...
                  <td style={{ padding: '10px 0' }}>{statusBadge(job.last_status || 'unknown')}</td>
                  <td style={{ padding: '10px 0', color: job.last_delivered === true ? '#22c55e' : job.last_delivered === false ? '#f87171' : '#64748b' }}>
...
function DocsPanel({
  ...
  useEffect(() => {
    ...
    void controlApiGet<DocEntry>(`/api/brain/docs/content?path=${encodeURIComponent(selectedDoc.path)}`, {
      timeoutMs: BRAIN_DOC_CONTENT_TIMEOUT_MS,
      cache: 'no-store',
    })
      .then((doc) => {
      ...
      .catch((contentError) => {
        if (cancelled) return;
        setDocContentError(ownerSafeErrorMessage(contentError, 'Unable to load document content.'));
      })
      .finally(() => {
        if (cancelled) return;
      ...
      <div style={{ width: 'min(100%, 320px)', flex: '1 1 280px', borderRight: '1px solid #0f172a', paddingRight: '12px', maxHeight: '520px', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'grid', gap: '10px', marginBottom: '14px' }}>
          <input
            ...
            aria-label="Search Brain documentation"
            placeholder="Search names, paths, headings, or loaded text"
            style={brainInputStyle}
          />
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            <button
          ...
          <div style={{ marginBottom: '16px' }}>
            ...
            <div style={{ display: 'grid', gap: '8px' }}>
              {recentDocs.slice(0, 3).map((doc) => (
                ...
                >
                  <p style={{ fontSize: '13px', fontWeight: 600 }}>{normalizeBrainDisplayText(doc.name)}</p>
                  {doc.snippet && (
                    <p style={{ fontSize: '12px', color: '#94a3b8', lineHeight: 1.45 }}>{truncateText(normalizeBrainDisplayText(doc.snippet), 90)}</p>
                  )}
                </button>
              ))}
            </div>
                ...
                return (
                  ...
                  >
                    <p style={{ fontSize: '13px', fontWeight: 600 }}>
                      {normalizeBrainDisplayText(doc.name)}
                      {doc.readMode && doc.readMode !== 'live' ? ` (${humanizeSnakeCase(doc.readMode)})` : ''}
                    </p>
                    {(doc.authority || doc.status) && (
                      <p style={{ color: doc.authority === 'binding' ? '#38bdf8' : '#64748b', fontSize: '11px', marginTop: '3px' }}>
                        {[doc.authority, doc.status].filter(Boolean).map((value) => humanizeSnakeCase(String(value))).join(' · ')}
                      </p>
                    )}
                    {doc.snippet && <p style={{ fontSize: '12px', color: '#94a3b8', lineHeight: 1.45 }}>{normalizeBrainDisplayText(doc.snippet)}</p>}
                  </button>
                );
              })}
            </div>
          ...
          <div>
            <div style={{ marginBottom: '12px', display: 'flex', justifyContent: 'space-between', gap: '12px', alignItems: 'flex-start', flexWrap: 'wrap' }}>
              <div>
                ...
                <h2 style={{ color: 'white', fontSize: '24px', marginBottom: '6px' }}>{normalizeBrainDisplayText(selectedDoc.name)}</h2>
                <p style={{ color: '#64748b', fontSize: '12px' }}>
                  {displayBrainPath(selectedDoc.path)}
                  {selectedDoc.readMode && ` · ${humanizeSnakeCase(selectedDoc.readMode)}`}
                  {selectedDoc.authority && ` · ${humanizeSnakeCase(selectedDoc.authority)}`}
                  {selectedDoc.status && ` · ${humanizeSnakeCase(selectedDoc.status)}`}
                  {selectedDoc.resolvedPath && selectedDoc.resolvedPath !== selectedDoc.path ? ` · reads ${displayBrainPath(selectedDoc.resolvedPath)}` : ''}
                </p>
              </div>
            ...
            >
              {selectedDocIsLoading
                ? 'Loading document content...'
                : docContentError
                  ? docContentError
                  : normalizeBrainDisplayText(
                      selectedDocContent?.trim() || selectedDoc.snippet || 'No preview is available for this document.',
                    )}
            </div>
          </div>
        ) : (
          <p style={{ color: error ? '#f87171' : '#475569' }}>
            {loadState === 'loading'
...
function personaPhoneChipStyle(active: boolean) {
  return {
    borderRadius: '9px',
    border: active ? '1px solid #38bdf8' : '1px solid #334155',
    backgroundColor: active ? '#082f49' : '#020617',
...
function personaPhoneIconButtonStyle(disabled = false) {
  return {
    borderRadius: '9px',
    border: '1px solid #334155',
    backgroundColor: '#020617',
...
function isPersonaResponseKind(value: unknown): value is PersonaResponseKind {
  return value === 'agree' || value === 'disagree' || value === 'nuance' || value === 'story' || value === 'language';
}

function isPersonaStandupKind(value: unknown): value is PersonaStandupKind {
  return (
    value === 'auto' ||
    value === 'executive_ops' ||
    value === 'operations' ||
    value === 'weekly_review' ||
    value === 'saturday_vision' ||
    value === 'workspace_sync'
  );
}

function readPersonaReviewDraft(deltaId: string): PersonaReviewDraft | null {
  if (typeof window === 'undefined') return null;
  try {
    const parsed = JSON.parse(window.localStorage.getItem(PERSONA_REVIEW_DRAFT_STORAGE_KEY) ?? '{}') as Record<string, unknown>;
    const raw = parsed[deltaId];
    if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return null;
    const draft = raw as Record<string, unknown>;
    if (draft.schema !== 'brain_persona_review_draft/v1' || draft.deltaId !== deltaId || !isPersonaResponseKind(draft.responseKind)) return null;
    const standupKind = isPersonaStandupKind(draft.triageStandupKind) ? draft.triageStandupKind : 'auto';
    const targetOverrides: Record<string, string> =
      draft.promotionItemTargetOverrides && typeof draft.promotionItemTargetOverrides === 'object' && !Array.isArray(draft.promotionItemTargetOverrides)
        ? Object.fromEntries(
            Object.entries(draft.promotionItemTargetOverrides as Record<string, unknown>)
              .filter((entry): entry is [string, string] => entry[0].length <= 200 && typeof entry[1] === 'string' && entry[1].length <= 500)
              .slice(0, 25),
          )
        : {};
    return {
      schema: 'brain_persona_review_draft/v1',
      deltaId,
      reflectionText: typeof draft.reflectionText === 'string' ? draft.reflectionText.slice(0, 20_000) : '',
      responseKind: draft.responseKind,
      selectedPromotionItemIds: Array.isArray(draft.selectedPromotionItemIds)
        ? draft.selectedPromotionItemIds.filter((value): value is string => typeof value === 'string').slice(0, 25)
        : [],
      promotionItemTargetOverrides: targetOverrides,
      routeToMemory: draft.routeToMemory === true,
      routeToStandup: draft.routeToStandup === true,
      routeToPM: draft.routeToPM === true,
      triageMemoryTargets: Array.isArray(draft.triageMemoryTargets)
        ? draft.triageMemoryTargets.filter((value): value is string => typeof value === 'string').slice(0, 10)
        : ['persistent_state'],
      triageWorkspaceKeys: Array.isArray(draft.triageWorkspaceKeys)
        ? draft.triageWorkspaceKeys.filter((value): value is string => typeof value === 'string').slice(0, 12)
        : ['shared_ops'],
      triageStandupKind: standupKind,
      triagePMTitle: typeof draft.triagePMTitle === 'string' ? draft.triagePMTitle.slice(0, 500) : '',
...
function writePersonaReviewDraft(draft: PersonaReviewDraft): boolean {
  if (typeof window === 'undefined') return false;
  try {
    const parsed = JSON.parse(window.localStorage.getItem(PERSONA_REVIEW_DRAFT_STORAGE_KEY) ?? '{}') as Record<string, unknown>;
    parsed[draft.deltaId] = draft;
    const retained = Object.entries(parsed)
      .filter(([, value]) => value && typeof value === 'object' && !Array.isArray(value))
      .sort((left, right) => {
        const leftUpdated = String((left[1] as Record<string, unknown>).updatedAt ?? '');
        const rightUpdated = String((right[1] as Record<string, unknown>).updatedAt ?? '');
        return rightUpdated.localeCompare(leftUpdated);
      })
      .slice(0, 50);
    window.localStorage.setItem(PERSONA_REVIEW_DRAFT_STORAGE_KEY, JSON.stringify(Object.fromEntries(retained)));
    return true;
  } catch {
    return false;
  }
...
function removePersonaReviewDraft(deltaId: string): void {
  if (typeof window === 'undefined') return;
  try {
    const parsed = JSON.parse(window.localStorage.getItem(PERSONA_REVIEW_DRAFT_STORAGE_KEY) ?? '{}') as Record<string, unknown>;
    delete parsed[deltaId];
    window.localStorage.setItem(PERSONA_REVIEW_DRAFT_STORAGE_KEY, JSON.stringify(parsed));
  } catch {
    // A completed server-side review remains authoritative even if local cleanup is unavailable.
  }
}
...
function formatPromotionSuccessMessage(
  ...
  const normalized = ownerSafeErrorMessage(baseMessage, '');
...
function findPersonaDocForTarget(docs: DocEntry[], targetFile: string | null) {
  if (!targetFile) return null;
  const normalizedTarget = displayBrainPath(targetFile).replace(/^knowledge\/persona\/feeze\//, '').replace(/^\/+/, '');
  if (!normalizedTarget) return null;
  return (
    docs.find((doc) => {
      const normalizedDocPath = displayBrainPath(doc.path).replace(/^knowledge\/persona\/feeze\//, '').replace(/^\/+/, '');
      return normalizedDocPath === normalizedTarget || normalizedDocPath.endsWith(`/${normalizedTarget}`);
    }) ?? null
  );
}

function personaPackKeyFromPath(docPath: string) {
  const normalized = displayBrainPath(docPath).replace(/^knowledge\/persona\/feeze\//, '').replace(/^\/+/, '');
  return normalized.split('/')[0] || null;
}

function stripBrainDocFrontmatter(content: string | null) {
  if (!content) return null;
  if (!content.startsWith('---\n')) return content.trim() || null;
  const closingMarker = content.indexOf('\n---\n', 4);
  if (closingMarker < 0) return content.trim() || null;
  return content.slice(closingMarker + 5).trim() || null;
}
...
function buildPostingWorkspaceHref(brief: DailyBriefEntry, item: BriefSourceIntelligenceCandidate, mode: 'post' | 'comment') {
  const params = new URLSearchParams();
  params.set('mode', mode);
  params.set('autoplay', '1');
  params.set('originType', 'daily_brief_item');
  params.set('originId', item.item_key?.trim() || brief.id);
  params.set('briefId', brief.id);
  params.set('returnUrl', '/brain#brain-section-briefs');
  ...
  const latestOwnerReaction = (item.existing_reactions ?? [])
    .map((reaction) => reaction.text?.trim() || '')
    .filter(Boolean)
    .at(-1);
  if (latestOwnerReaction) params.set('ownerReaction', latestOwnerReaction);
  return `/workspace/posting?${params.toString()}`;
}

function setPostingParam(params: URLSearchParams, key: string, value: string | null | undefined) {
  const normalized = value?.trim();
  if (normalized) params.set(key, normalized);
}

function buildBrainSignalPostingWorkspaceHref(signal: BrainSignalEntry, draft: BrainSignalRouteDraft) {
  const params = new URLSearchParams({
    mode: 'post',
    autoplay: '1',
    originType: 'brain_signal',
    originId: signal.id,
    itemKey: signal.id,
    returnUrl: '/brain#brain-section-dashboard',
  });
  const sourceRef = signal.source_ref?.trim() || '';
  setPostingParam(params, 'title', draft.digest || signal.digest || signal.raw_summary || humanizeSnakeCase(signal.source_kind));
  setPostingParam(params, 'summary', draft.summary || signal.raw_summary || signal.digest);
  setPostingParam(params, 'hook', draft.yodaMeaning || draft.neoSystemImpact);
  setPostingParam(params, 'ownerReaction', draft.routeReason || draft.summary);
  setPostingParam(params, 'priorityLane', draft.workspaceKey);
  setPostingParam(params, 'sourceKind', signal.source_kind);
  setPostingParam(params, 'routeReason', draft.routeReason || defaultBrainSignalRouteReason(draft.route));
  if (/^https?:\/\//i.test(sourceRef)) {
    params.set('sourceUrl', sourceRef);
  } else {
    setPostingParam(params, 'sourcePath', sourceRef);
  }
  return `/workspace/posting?${params.toString()}`;
}

function buildPersonaPostingWorkspaceHref(
  delta: PersonaDeltaEntry,
  source: {
    sourceTitle: string;
  ...
  const params = new URLSearchParams({
    mode: 'post',
    autoplay: source.ownerReaction ? '1' : '0',
    originType: 'persona_delta',
    originId: delta.id,
    itemKey: delta.id,
    returnUrl: `/brain?delta_id=${encodeURIComponent(delta.id)}#brain-section-persona`,
  });
  setPostingParam(params, 'title', source.sourceTitle || delta.trait);
  setPostingParam(params, 'summary', source.sourceExcerpt || delta.notes || delta.trait);
  setPostingParam(params, 'hook', metadataStringArray(delta.metadata, 'talking_points')[0]);
  setPostingParam(params, 'ownerReaction', source.ownerReaction);
  setPostingParam(params, 'sourceUrl', source.sourceUrl);
  setPostingParam(params, 'sourcePath', source.sourcePath);
  setPostingParam(params, 'sourceKind', source.sourceKind);
  setPostingParam(params, 'routeReason', metadataText(delta.metadata, 'why_showing') || metadataText(delta.metadata, 'system_hypothesis'));
  setPostingParam(params, 'targetFile', metadataText(delta.metadata, 'target_file'));
  return `/workspace/posting?${params.toString()}`;
}

function buildWorkspaceMirrorPostingHref(seed: {
  originType: string;
  originId: string;
  title?: string | null;
  summary?: string | null;
  ...
  publishPosture?: string | null;
  canonicalPillar?: string | null;
  careerSignal?: string | null;
  employerProximity?: string | null;
  employerSafety?: string | null;
  proofPosture?: string | null;
  ...
  const params = new URLSearchParams({
    ...
    itemKey: seed.originId,
    returnUrl: '/brain#brain-section-dashboard',
  });
  setPostingParam(params, 'title', seed.title);
  setPostingParam(params, 'summary', seed.summary);
  setPostingParam(params, 'hook', seed.hook);
  setPostingParam(params, 'sourceUrl', seed.sourceUrl);
  setPostingParam(params, 'sourcePath', seed.sourcePath);
  setPostingParam(params, 'priorityLane', seed.priorityLane);
  setPostingParam(params, 'sourceKind', seed.sourceKind);
  setPostingParam(params, 'routeReason', seed.routeReason);
  setPostingParam(params, 'publishPosture', seed.publishPosture);
  setPostingParam(params, 'canonicalPillar', seed.canonicalPillar);
  setPostingParam(params, 'careerSignal', seed.careerSignal);
  setPostingParam(params, 'employerProximity', seed.employerProximity);
  setPostingParam(params, 'employerSafety', seed.employerSafety);
  setPostingParam(params, 'proofPosture', seed.proofPosture);
  setPostingParam(params, 'audience', seed.audience);
  setPostingParam(params, 'audienceConsequence', seed.audienceConsequence);
  setPostingParam(params, 'distinctThesis', seed.distinctThesis);
  setPostingParam(params, 'whyNow', seed.whyNow);
  setPostingParam(params, 'developmentStatus', seed.developmentStatus);
  return `/workspace/posting?${params.toString()}`;
}
...
function briefConsumerLaneGuidance(lane: BriefConsumerLane) {
  ...
  if (lane === 'persona_candidate') {
    return 'This may reveal durable truth about the owner. Use Persona to decide whether it becomes identity, worldview, or canon.';
  }
  if (lane === 'post_candidate') {
...
function isRecentTimestamp(value: string | null | undefined, maxAgeMs: number) {
...
function formatBrainLocalQueueStatus(payload: BrainLocalActionQueuedResponse, subject: string) {
  const jobId = payload.card_id || payload.job_id;
  const jobSuffix = jobId && /^[A-Za-z0-9._-]{1,80}$/.test(jobId) ? ` Job ${jobId}.` : '';
  if (payload.disposition === 'already_active') {
    return `${subject} is already active in the signed queue on your Mac.${jobSuffix}`;
  }
  const backendMessage = ownerSafeErrorMessage(payload.message, '').replace(/\.$/, '');
  return `${backendMessage || `${subject} queued for the signed local runner`}. It will run on your Mac without a model API call.${jobSuffix}`;
}

function displayBrainPath(value: string) {
  const normalized = normalizeBrainDisplayText(value).replace(/^\.\//, '');
  const anchors = ['workspaces/', 'knowledge/', 'docs/', 'memory/', 'SOPs/'];
  const anchorPositions = anchors
    .map((anchor) => ({ anchor, index: normalized.indexOf(anchor) }))
    .filter((entry) => entry.index >= 0)
    .sort((left, right) => left.index - right.index);
  return anchorPositions[0] ? normalized.slice(anchorPositions[0].index) : normalized;
}

function inferDocGroup(path: string) {
  const normalizedPath = displayBrainPath(path).replace(/^(\.\.\/)+/, '');
...
function compareDocEntries(left: DocEntry, right: DocEntry) {
  const leftOrder = typeof left.readOrder === 'number' ? left.readOrder : Number.POSITIVE_INFINITY;
  const rightOrder = typeof right.readOrder === 'number' ? right.readOrder : Number.POSITIVE_INFINITY;
