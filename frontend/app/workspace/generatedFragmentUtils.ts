...
export function codexJobStatusHint(status?: string | null) {
  switch ((status || '').trim().toLowerCase()) {
    ...
    case 'running':
      return 'Codex Terminal is generating the options through the signed-in Codex session on this Mac.';
    case 'completed':
      return 'The Mac worker completed this job and wrote the verified result back to the workspace.';
    case 'failed':
