...
export function normalizeLocalVoiceReviewText(value?: string | null) {
  return String(value ?? '').replace(/\r\n/g, '\n').replace(/\r/g, '\n').trim();
}

export function isExactOperationalVoiceReviewCopy(
  operationalText?: string | null,
  browserLocalText?: string | null,
) {
  const normalizedOperationalText = normalizeLocalVoiceReviewText(operationalText);
  return Boolean(normalizedOperationalText) && normalizedOperationalText === normalizeLocalVoiceReviewText(browserLocalText);
}
...
export function buildLocalVoiceReviewPacket(
  ...
  const generatedText = normalizeLocalVoiceReviewText(source.generatedText);
  if (!generatedText) {
    throw new Error('The exact generated draft is required for local voice feedback.');
  }
  const editedText = source.decision === 'park' ? '' : normalizeLocalVoiceReviewText(source.editedText);
  const generationJobId = normalizeLocalVoiceReviewText(source.generationJobId) || null;
  const generationOptionIndex = Number.isInteger(source.generationOptionIndex)
    ? Number(source.generationOptionIndex)
    : null;
  const lane = normalizeLocalVoiceReviewText(source.lane).toLowerCase();
  return {
    ...
    context: {
      ...
      channel: 'linkedin',
      post_type: 'owner_review',
      topic: normalizeLocalVoiceReviewText(source.topic),
      topic_tags: lane ? [lane] : [],
      owner_notes: normalizeLocalVoiceReviewText(source.ownerNotes),
    },
  };
}
