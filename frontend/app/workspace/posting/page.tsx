...
function PostingWorkspaceClient() {
  ...
  const [audience, setAudience] = useState<string>(mapAudienceFromLane(initialQuery.priorityLane));
  ...
  const handleTopicSourceModeChange = useCallback(
    ...
    [initialQuery.hook, initialQuery.ownerReaction, initialQuery.priorityLane, initialQuery.routeReason, initialQuery.summary, initialQuery.title],
  );
  ...
  }, [audience, category, effectiveSourceMode, resolvePostInputs, sourceCard]);
                    ...
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                      {reviewHandoffs[index]?.queue_id ? (
                        ...
                        <button
                          ...
                          disabled={reviewActionLoading !== null || !localJobCompleted}
                          style={secondaryButtonStyle('#22c55e')}
                        >
                          {reviewActionLoading === index ? 'Sending…' : 'Send to owner review'}
                        </button>
                      )}
                      <button type="button" onClick={() => void handleCopy(option, `Post option ${index + 1}`)} style={secondaryButtonStyle('#38bdf8')}>
                        Copy
                      </button>
                    </div>
                  </div>
