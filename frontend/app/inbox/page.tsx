...
function InboxPageContent() {
  ...
  const initialWorkspaceFilter = normalizeWorkspaceFilter(searchParams.get('workspace')) ?? 'all';
      ...
      <section style={{ display: 'grid', gap: '18px' }}>
        <header style={{ display: 'grid', gap: '10px' }}>
          ...
          <p style={{ color: '#cbd5e1', fontSize: '13px', margin: 0 }}>
            Neo guest conversations and coffee-chat approvals live in the <Link href="/inbox/neo" style={{ color: '#fbbf24' }}>Neo Inbox</Link>.
          </p>
          {workspaceFilter === 'agc' ? (
...
export default function InboxPage() {
  return (
    <Suspense fallback={<main style={{ padding: '32px', color: '#cbd5e1' }}>Loading inbox…</main>}>
      <InboxPageContent />
    </Suspense>
  );
}
