...
type LabExperiment = {
  ...
  sample_runs: {
    ...
    matrix_rows?: {
      ...
      owner_perspective?: Record<string, unknown>;
                  ...
                  {item.matrix_rows && item.matrix_rows.length > 0 && (
                    <div style={{ marginTop: '12px', display: 'grid', gap: '8px' }}>
                      ...
                      {item.matrix_rows.map((row) => {
                        ...
                        const perspective = objectValue(row.owner_perspective);
                        ...
                        const ownerView =
                          stringValue(reactionBrief.owner_view) || stringValue(row.context_snapshot?.owner_view);
                        const tension =
                        ...
                        const perspectiveScore = numberValue(stageEvaluation.owner_perspective_score);
                            ...
                            <div style={{ display: 'grid', gap: '6px' }}>
                              ...
                              {expanded && (
                                <>
                                  ...
                                  {(row.belief_used || row.experience_anchor || articleView || ownerView || tension || contentAngle || personalReaction) && (
                                    <div style={{ display: 'grid', gap: '4px' }}>
                                      ...
                                      {ownerView && <p style={{ color: '#94a3b8', fontSize: '12px' }}>the owner view: {ownerView}</p>}
...
function LiveSourceAuditSection({
  ...
  const candidateSamples = useMemo(() => audit.candidate_samples ?? [], [audit.candidate_samples]);
  const assetSamples = useMemo(() => audit.asset_samples ?? [], [audit.asset_samples]);
  const fragmentSamples = useMemo(() => audit.fragment_samples ?? [], [audit.fragment_samples]);
...
function groupStageResults(
  ...
  const order = ['source', 'article', 'classification', 'owner', 'synthesis', 'decision', 'draft', 'evaluation'];
