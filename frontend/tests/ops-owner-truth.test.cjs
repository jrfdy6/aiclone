    ...
    workspaces: [
      {
        ...
        reason_codes: [],
        current_focus: {
          title: 'Advance the current AGC opportunity lane',
          status: 'failed',
        ...
        artifacts: [
          {
            kind: 'analytics',
            title: 'AGC GDIT Partner-Path Capability Experiment',
            summary: 'Advance the bounded partner-path experiment.',
            reference: 'workspace://agc/analytics/capability-experiment.md',
            source_updated_at: '2026-08-20T06:14:00Z',
            source_sha256: 'a'.repeat(64),
            consumption_role: 'reference_only',
            source_state: 'verified_preexisting',
          },
        ],
      },
    ],
...
test('workspace context exposes recognizable owner-safe evidence without transport identifiers', () => {
  const context = projectOpsWorkspaceContext(contextProjection(), 'agc');

  assert.equal(context.workspace_key, 'agc');
  assert.equal(context.current_focus.title, 'Advance the current AGC opportunity lane');
  assert.equal(context.artifacts[0].title, 'AGC GDIT Partner-Path Capability Experiment');
  assert.equal(context.artifacts[0].summary, 'Advance the bounded partner-path experiment.');
  assert.equal(context.artifacts[0].consumption_role, 'reference_only');
  assert.equal(context.artifacts[0].reference, undefined);
  assert.equal(context.artifacts[0].source_sha256, undefined);
});

test('workspace context rejects unsupported authority and artifact roles', () => {
  const wrongSchema = contextProjection();
  wrongSchema.schema_version = 'ops_workspace_context_projection/v999';
  assert.equal(projectOpsWorkspaceContext(wrongSchema, 'agc'), null);

  const wrongRole = contextProjection();
  wrongRole.workspaces[0].artifacts[0].consumption_role = 'completed_work';
  const context = projectOpsWorkspaceContext(wrongRole, 'agc');
