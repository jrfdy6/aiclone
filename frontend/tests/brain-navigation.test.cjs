...
const deployScript = fs.readFileSync(path.join(repoRoot, 'scripts', 'deploy_railway_service.sh'), 'utf8');
...
test('redacts private host paths and credential names from every Brain display string', () => {
  assert.equal(
    normalizeBrainDisplayText('Result in /Users/operator/.private-runtime/jobs/result.json.'),
    'Result in [local path].',
  );
  assert.equal(
    normalizeBrainDisplayText('Updated /Users/operator/project/workspaces/agc/analytics/report.md.'),
