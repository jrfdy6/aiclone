...
def _select_next_entry(entries: list[dict[str, Any]], *, card_id: str | None = None) -> dict[str, Any] | None:
    ...
    for item in entries:
        if not _entry_has_runnable_execution_gate(item):
            continue
    ...
    if not filtered:
        return None
    return sorted(filtered, key=_jean_claude_queue_sort_key)[0]
...
def _jean_claude_queue_sort_key(entry: dict[str, Any]) -> tuple[int, float]:
    standup_rank = 0 if _is_standup_backed_entry(entry) else 1
    timestamp = _parse_datetime(entry.get("queued_at")) or _parse_datetime(entry.get("last_transition_at"))
    if timestamp is None:
        time_key = float("inf")
    else:
        epoch = timestamp.timestamp()
        time_key = -epoch if standup_rank == 0 else epoch
    return standup_rank, time_key
    ...
    try:
        raise SystemExit(
            execute_with_runner_lock(
                lock_name="jean_claude_execution_dispatch",
                automation_id="jean_claude_execution_dispatch",
                automation_name="Jean-Claude Execution Dispatch",
