...
def _build_bundle(args: argparse.Namespace, run_id: str, selected_entry: dict[str, Any], card: dict[str, Any], registry: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "pm_card": card,
        "registry_item": registry.get(workspace_key) or {},
...
def _build_sop(bundle: dict[str, Any]) -> dict[str, Any]:
    ...
    if direct:
        ...
        objective = f"Jean-Claude should brief {target_agent} and keep the work contained inside `{workspace_key}`."
...
def _update_card(imports: dict[str, Any], api_url: str, card: dict[str, Any], run_id: str, sop_path: Path, briefing_path: Path, *, dry_run: bool) -> dict[str, Any] | None:
    if dry_run:
        return None
...
def main() -> int:
    ...
    briefing_lines = [
        ...
        "",
        f"- Workspace: `{bundle['primary_workspace_key']}`",
    ...
    chronicle_entry = {
        ...
        "memory_promotions": [],
        "pm_candidates": [],
        "artifacts": [str(sop_path), str(briefing_path)],
