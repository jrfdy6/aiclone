...
def _build_workspace_execution_contract(
    *,
    ...
    local_artifacts = _merge_unique_strings(
        [
            f"Ground the result against `{manager_briefing_path}`." if manager_briefing_path else "",
            (
                f"Cite or update the latest workspace briefing at `{latest_workspace_briefing_path}`."
                if latest_workspace_briefing_path
                else ""
            ),
            (
                f"Cite the latest workspace execution log at `{execution_log_path}` or state why it was unavailable."
                if execution_log_path
                else "State directly when no workspace execution log was available."
            ),
            "Return absolute file paths for changed workspace artifacts, or cite the reviewed local artifact paths when no files changed.",
        ]
    )
    ...
    return {
        ...
        "artifacts_expected": local_artifacts[:8],
...
def _is_standup_backed_entry(entry: dict[str, Any]) -> bool:
    source = str(entry.get("source") or "").strip().lower()
    link_type = str(entry.get("link_type") or "").strip().lower()
    return source.startswith("post-sync-dispatch:") or link_type == "standup"


def _entry_needs_workspace_pickup(entry: dict[str, Any], *, allow_existing_packet: bool) -> bool:
    ...
    return allow_existing_packet or not packet_path


def _workspace_pickup_sort_key(entry: dict[str, Any]) -> tuple[int, int, float]:
    execution_state = str(entry.get("execution_state") or "").strip().lower()
    queued_rank = 0 if execution_state == "queued" else 1
    standup_rank = 0 if _is_standup_backed_entry(entry) else 1
    timestamp = _parse_datetime(entry.get("last_transition_at")) or _parse_datetime(entry.get("queued_at"))
    if timestamp is None:
        time_key = float("inf")
    else:
        epoch = timestamp.timestamp()
        time_key = -epoch if standup_rank == 0 else epoch
    return queued_rank, standup_rank, time_key
...
def main() -> int:
    ...
    work_order = {
        ...
        "execution_gate_intent_hash": selected_entry.get("execution_gate_intent_hash"),
        ...
        "artifacts_expected": _merge_unique_strings(artifacts_expected + list(artifact_context.get("artifacts_expected") or []))[:8],
