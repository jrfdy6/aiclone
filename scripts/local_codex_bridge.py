...
from runtime_http import runtime_secret_value
...
DEFAULT_WORKSPACE_ROOT = os.getenv("LOCAL_CODEX_BRIDGE_WORKSPACE_ROOT", "/Users/neo/Documents/Codex/AI-Clone")
...
DEFAULT_CONTENT_EXECUTION_POLICY = "codex_terminal_required"
    ...
    *,
    ...
    if not base_prompt_available:
        _fail_job(
            ...
            job_id=job_id,
            worker_id=worker_id,
            error_message="Claimed Codex job did not include the prompt packet required for Codex Terminal execution.",
        )
        return True
...
def parse_args() -> argparse.Namespace:
    ...
    worker_suffix = (
        str(os.getenv("LOCAL_CODEX_WORKER_SUFFIX") or "codex-bridge").strip()
        or "codex-bridge"
    )
    parser.add_argument("--worker-id", default=f"{socket.gethostname()}-{worker_suffix}")
...
def main() -> int:
    ...
    if not workspace_root.exists():
        ...
        return 1
    token = args.token or runtime_secret_value("LOCAL_CODEX_BRIDGE_TOKEN")
    if not token:
        print("LOCAL_CODEX_BRIDGE_TOKEN is required for the local Codex bridge.", file=sys.stderr)
        return 1
