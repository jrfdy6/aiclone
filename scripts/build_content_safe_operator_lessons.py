...
from app.services.content_safe_operator_lesson_service import (  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the public-safe operator-lesson lane from operator story signals.")
...
def _sync_payload(api_url: str, payload: dict[str, Any], source: str) -> dict[str, Any]:
    request_payload = {
        ...
        "lesson_count": (payload.get("counts") or {}).get("total", 0),
        "source_snapshot_type": payload.get("source_snapshot_type"),
        "source_generated_at": payload.get("source_generated_at"),
        "counts": payload.get("counts") or {},
        "lessons": payload.get("lessons") or [],
    }
    request = urllib.request.Request(
        f"{api_url.rstrip('/')}/api/brain/content-safe-operator-lessons/sync",
...
def main() -> int:
    args = parse_args()
    report_path = Path(args.operator_story_report).expanduser()
    payload = build_content_safe_operator_lessons_payload(
        workspace_key=args.workspace_key,
        operator_story_report_path=report_path,
    )
