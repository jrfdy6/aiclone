...
from automation_run_mirror import latest_successful_run_ms
...
def _latest_progress_pulse_run_ms() -> int | None:
    return latest_successful_run_ms("progress_pulse", RUN_LOG)
...
def build_report() -> dict[str, Any]:
    last_run_ms = _latest_progress_pulse_run_ms()
    new_handoffs = _new_handoff_entries(last_run_ms)
    material_handoffs = [row for row in new_handoffs if entry_has_material_signal(row)]
    persistent_state_newer = _persistent_state_newer(last_run_ms)
    ...
    reasons: list[str] = []
    if last_run_ms is None and material_handoffs:
        reasons.append("no prior successful Progress Pulse run found")
