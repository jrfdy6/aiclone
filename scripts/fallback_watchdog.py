...
def _durable_checks(workspace_keys: list[str]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[str]]:
    ...
    for workspace_key in workspace_keys:
        ...
        index_result_count = int(context.get("index_result_count") or 0)
        memory_index_result_count = int(context.get("memory_index_result_count") or index_result_count)
        check = {
            ...
            "memory_index_result_count": memory_index_result_count,
            "warnings": warnings,
        ...
        if not check["fallback_active"]:
            continue
        severity = "critical" if warnings and index_result_count == 0 else "warning"
    ...
    *,
    ...
    contract = build_execution_contract(
        ...
        instructions=[
            ...
            "Repair the root cause so canonical memory reads return to runtime/live expectations and durable retrieval returns to the local Codex SQLite index first.",
...
def _markdown_report(report: dict[str, Any]) -> str:
    ...
    for item in report.get("durable_checks") or []:
        lines.append(
            ...
            f"(index={item.get('index_result_count', 0)}, filesystem={item['filesystem_result_count']})"
        )
