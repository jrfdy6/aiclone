...
if [ -z "$PYTHON_BIN" ]; then
  if [ -x "$HOME/.codex/ai-clone/venv/bin/python" ]; then
    PYTHON_BIN="$HOME/.codex/ai-clone/venv/bin/python"
  else
    PYTHON_BIN="python3"
  fi
fi
...
stage_repo_docs() {
  ...
  rsync_if_exists "$REPO_ROOT/CODEX_STARTUP.md" "$target_root/CODEX_STARTUP.md"
  rsync_if_exists "$REPO_ROOT/SOURCE_OF_TRUTH.md" "$target_root/SOURCE_OF_TRUTH.md"
  rsync_if_exists "$REPO_ROOT/IDENTITY.md" "$target_root/IDENTITY.md"
  rsync_if_exists "$REPO_ROOT/CHARTER.md" "$target_root/CHARTER.md"
  rsync_if_exists "$REPO_ROOT/SOUL.md" "$target_root/SOUL.md"
  rsync_if_exists "$REPO_ROOT/USER.md" "$target_root/USER.md"
  rsync_if_exists "$REPO_ROOT/MEMORY.md" "$target_root/MEMORY.md"
  rsync_if_exists "$REPO_ROOT/memory/roadmap.md" "$target_root/memory/roadmap.md"
  rsync_if_exists "$REPO_ROOT/README.md" "$target_root/README.md"

  for workspace_dir in shared-ops linkedin-content-os fusion-os easyoutfitapp ai-swag-store agc work-life-tools
  do
...
stage_frontend_brain_sources() {
  ...
  # Railway reads current standup state from Postgres. Package only the small
  # workspace-lane evidence folders needed by snapshot fallbacks; the historic
  # executive/operations packet archive is local-only and can exceed 170 MB.
  for standup_lane in agc ai-swag-store easyoutfitapp fusion-os linkedin-os
  do
    rsync_if_exists "$DATA_ROOT/memory/standup-prep/$standup_lane/" "$target_root/memory/standup-prep/$standup_lane/"
  done
  ...
  latest_daily_log="$(find "$DATA_ROOT/memory" -maxdepth 1 -type f -name '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9].md' 2>/dev/null | sort | tail -n 1 || true)"
...
# The frontend receives private operating data only through the authenticated
# backend proxy. Never package memory, persona, dispatch, or workspace files
# into the browser-facing image.
if [ "$SERVICE_NAME" = "aiclone-frontend" ] && [ -n "$(find "$STAGE_DIR/$CHILD_DIR" -type f -name 'workspaceSnapshot.*' -print -quit)" ]; then
  echo "error: a private workspace snapshot entered the frontend deployment stage" >&2
  exit 1
fi
