...
def _finding_lines(
    ...
    runtime = heartbeat_report.get("runtime") or {}
    counts = runtime.get("status_counts") or {}
    ...
    if error_count or action_required_count:
        lines.append(
            f"Local automation health needs attention ({error_count} error run(s), {action_required_count} action-required run(s) in the last {runtime.get('window_hours', 36)}h)."
        )
    elif entries:
...
def _action_lines(
    ...
    action_required_count = int(runtime.get("action_required_count", 0) or 0)
    if error_count or action_required_count:
        lines.append("Resolve the local automation errors and action-required runs before treating runtime health as calm.")
    else:
