...
- Status: first runtime slice is now live. `backend/app/services/social_long_form_signal_service.py` classifies long-form segments onto shared response modes, `workspace_snapshot_service.py` persists `long_form_routes`, `/ops` shows the route mix and top routed candidates, `social_persona_review_service.py` now consumes the same shared candidate set for worldview review instead of re-parsing long-form assets separately, and the weekly-plan snapshot now reads routed `post_seed` / `belief_evidence` outputs from that same contract. The current follow-on is planner metadata cleanup: production now shows the correct routed media counts and candidate lists, but `weekly_plan.generated_at` still reflects the base file-backed plan timestamp instead of the augmented planner state.
- Next validation target: planner payload should expose routed media fields and planner metadata consistently in the same response. Specifically, when `media_post_seeds` / `belief_evidence_candidates` are present, the planner-facing timestamp and freshness readout should no longer imply a stale pre-routing state.
- Source files:
  - `docs/source_expansion_implementation_plan.md`
...
- Benchmark gate:
  ...
  - Brain primary review queue defaults to high-signal items first, while lower-confidence long-form items are still reachable through the muted view
- Source files:
  - `docs/source_expansion_implementation_plan.md`
  - `backend/app/services/social_belief_engine.py`
  - `frontend/app/brain/BrainClient.tsx`
