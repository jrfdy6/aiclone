...
- the weekly-plan snapshot now also consumes that same shared route set for planner-visible `media_post_seeds` and `belief_evidence_candidates`, so planner reuse is extending the same upstream contract instead of branching into a second media planner
